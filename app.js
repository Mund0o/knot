...
function clearServerNativeScreen(state,{keepChannel=false}={}){
  if(!state)return;state.nativeScreenPlayer?.destroy();state.nativeScreenPlayer=null;state.nativeScreenAudioExpected=false;if(state.screenAudio){try{state.screenAudio.pause();state.screenAudio.srcObject=null;state.screenAudio.remove()}catch{}state.screenAudio=null}if(state.screen&&!state.screen.srcObject){try{state.screen.pause();state.screen.remove()}catch{}state.screen=null}if(!keepChannel&&state.nativeReceiveChannel){try{state.nativeReceiveChannel.onmessage=null;state.nativeReceiveChannel.close()}catch{}state.nativeReceiveChannel=null}
}
function beginServerNativeScreen(peerId,state,meta,channel){
  ...
  const fallback=()=>{try{if(channel.readyState==='open')channel.send(JSON.stringify({t:'native-screen-fallback',serverId:state.context.serverId}))}catch{}};try{state.nativeScreenPlayer=createNativeScreenPlayer(video,meta.codec||'AV1',fallback,meta)}catch(error){fallback();clearServerNativeScreen(state,{keepChannel:true});setServerStatus(error.message);return false}channel._nativeReceive={fragments:new Map(),complete:new Map(),nextSeq:0,player:state.nativeScreenPlayer};renderServerVoiceUI();return true
}
...
function wireServerNativeScreenChannel(peerId,state,channel,{remote=false}={}){
  ...
  state.nativeSendChannel=channel;channel.onmessage=event=>{if(typeof event.data!=='string')return;try{const value=JSON.parse(event.data);if(value.t==='native-screen-fallback'&&value.serverId===state.context.serverId)fallbackServerNativeToWebRtc()}catch{}};channel.onopen=()=>announceServerNativeChannel(channel);channel.onclose=()=>{if(state.nativeSendChannel===channel)state.nativeSendChannel=null}
}
async function ensureServerPeer(peerId,context={type:'server',serverId:activeServerId,channelId:activeChannelId}){
  ...
  if(voicePeerAllowed(peerId)){
    for(const track of serverVoiceStream.getTracks())state.voiceSender=connection.addTrack(track,serverVoiceStream);
...
function announceServerNativeChannel(channel){if(channel.readyState!=='open'||!serverNativeScreenSession)return false;channel._nativeSendSeq=0;channel._nativeInitSent=false;channel.send(JSON.stringify({t:'native-screen-meta',serverId:joinedVoiceServerId,codec:'AV1',fps:serverNativeScreenSession.fps,width:serverNativeScreenSession.width,height:serverNativeScreenSession.height,encoder:serverNativeScreenSession.encoder,latencyTargetMs:serverNativeScreenSession.latencyTargetMs,audio:false}));return true}
async function sendServerNativeItem(channel,item){
  if(channel.readyState!=='open'||!serverNativeScreenSession)return false;if(channel._nativeSendSeq===undefined)announceServerNativeChannel(channel);if(item.kind==='init'){if(channel._nativeInitSent)return true;channel._nativeInitSent=true;return sendNativeScreenSegment(channel,{...item,seq:channel._nativeSendSeq++})}if(!channel._nativeInitSent){if(!serverNativeScreenInit)return false;channel._nativeInitSent=true;if(!await sendNativeScreenSegment(channel,{kind:'init',data:serverNativeScreenInit,seq:channel._nativeSendSeq++}))return false}return sendNativeScreenSegment(channel,{...item,seq:channel._nativeSendSeq++})
}
async function ensureServerNativeChannel(peerId,state){
  if(!serverNativeScreenSession||!state||state.pc.connectionState!=='connected')return null;let channel=state.nativeSendChannel;if(!channel||channel.readyState==='closed'){channel=state.pc.createDataChannel('knot-server-screen-native',{ordered:true,priority:'low'});wireServerNativeScreenChannel(peerId,state,channel)}if(!await waitNativeScreenChannel(channel))return null;announceServerNativeChannel(channel);if(serverNativeScreenInit)await sendServerNativeItem(channel,{kind:'init',data:serverNativeScreenInit});if(serverNativeScreenAudioStream&&!state.screenSenders.some(sender=>sender.track===serverNativeScreenAudioStream.getAudioTracks()[0]))try{await attachServerNativeAudioToPeer(peerId,state)}catch{try{channel.send(JSON.stringify({t:'native-screen-audio',serverId:state.context.serverId,active:false}))}catch{}}return channel
}
async function attachServerNativeAudioToPeer(peerId,state){const track=serverNativeScreenAudioStream?.getAudioTracks?.()[0];if(!track)throw new Error('screen audio ended');if(state.nativeSendChannel?.readyState==='open')state.nativeSendChannel.send(JSON.stringify({t:'native-screen-audio',serverId:state.context.serverId,active:true}));const sender=state.pc.addTrack(track,serverNativeScreenAudioStream);state.screenSenders.push(sender);try{const parameters=sender.getParameters();if(!parameters.encodings?.length)parameters.encodings=[{}];parameters.encodings[0].maxBitrate=192000;parameters.encodings[0].priority='high';parameters.encodings[0].networkPriority='high';await sender.setParameters(parameters);if(!await renegotiateServerPeer(peerId,state))throw new Error('screen audio negotiation did not start');return sender}catch(error){try{state.pc.removeTrack(sender)}catch{}state.screenSenders=state.screenSenders.filter(value=>value!==sender);throw error}}
...
async function startCall(){
  ...
  try{
    ...
    try{const p=sender.getParameters();if(p){if(!p.encodings||!p.encodings.length)p.encodings=[{}];p.encodings[0].maxBitrate=256000;if(p.codecs)p.codecs.forEach(c=>{if(c.mimeType.toLowerCase()==='audio/opus'){c.maxptime=20;c.ptime=10;if(c.parameters){c.parameters.maxaveragebitrate=256000;c.parameters.maxplaybackrate=48000;c.parameters.maxptime=20;c.parameters.minptime=10;c.parameters.useinbandfec=1;c.parameters.usedtx=0;c.parameters.cbr=1;c.parameters.stereo=1;c.parameters['sprop-stereo']=1;c.parameters.spropmaxcapturerate=48000}}});await sender.setParameters(p)}}catch(e){console.warn('opus params:',e)}
...
async function configureScreenVideoSender(sender,track,fps){
  ...
  const parameters=sender.getParameters();if(!parameters.encodings?.length)parameters.encodings=[{}];const encoding=parameters.encodings[0];encoding.maxBitrate=maxBitrate;delete encoding.minBitrate;encoding.maxFramerate=requestedFps;encoding.scaleResolutionDownBy=1;encoding.priority='high';encoding.networkPriority='high';parameters.degradationPreference='maintain-resolution';await sender.setParameters(parameters);
}
...
const NATIVE_SCREEN_PACKET=0x4b4e5331,NATIVE_SCREEN_PART=60*1024,NATIVE_SCREEN_BUFFER_HIGH=8*1024*1024;
...
function createWebCodecsNativeScreenPlayer(video,codec,onError=()=>{},options={}){
  ...
  const allowSoftwareFallback=options.allowSoftwareFallback!==false,fps=Number(options.fps)||60,configuredWidth=Number(options.width)||0,configuredHeight=Number(options.height)||0;
  let canvas,context,decoder=null,config=null,configured=false,destroyed=false,decoderDisabled=false,failureReported=false,latencyExceeded=false,playbackActive=true,decodedFrames=0,paintedFrames=0,firstPaintAt=0,frameWidth=0,frameHeight=0,softwareFallback=false,hardwareUnavailable=false,replay=[],queuedSinceOutput=0,configuredAt=0,lastOutputAt=0;const arrivalTimes=new Map(),latencySamples=[];
  const fail=error=>{if(destroyed||failureReported)return;failureReported=true;decoderDisabled=true;try{decoder?.close()}catch{};onError(error instanceof Error?error:new Error(String(error||'Native AV1 low-latency decode failed')))};
  ...
  const output=frame=>{
    ...
    if(playbackActive){
      ...
      try{context.drawImage(frame,0,0,canvas.width,canvas.height);const paintedAt=performance.now(),arrivedAt=arrivalTimes.get(frame.timestamp);paintedFrames++;if(!firstPaintAt)firstPaintAt=paintedAt;if(arrivedAt!==undefined){latencySamples.push(paintedAt-arrivedAt);if(latencySamples.length>360)latencySamples.shift();if(softwareFallback&&latencySamples.length>=120&&decodedFrames%15===0){const recent=latencySamples.slice(-60).sort((a,b)=>a-b),p95=recent[Math.ceil(recent.length*.95)-1];if(p95>100){latencyExceeded=true;fail(new Error('AV1 software decode exceeded the 100ms latency target'))}}}}catch(error){fail(error)}
    }
    arrivalTimes.delete(frame.timestamp);
    frame.close();
  };
  const disableLocalDecoder=()=>{hardwareUnavailable=true;decoderDisabled=true;try{decoder?.close()}catch{}replay=[];arrivalTimes.clear();drawPreviewUnavailable();return true};
  const startSoftwareDecoder=error=>{
    ...
    softwareFallback=true;try{decoder?.close()}catch{}
    try{decoder=makeDecoder('prefer-software');decoder.configure({...config,hardwareAcceleration:'prefer-software'});configuredAt=performance.now();lastOutputAt=configuredAt;queuedSinceOutput=replay.length;for(const frame of replay)decoder.decode(new EncodedVideoChunk(frame));return true}catch(fallbackError){fail(fallbackError);return false}
  };
  const makeDecoder=hardwareAcceleration=>{
    const instance=new VideoDecoder({output,error:error=>{
    ...
    append(data){
      ...
      try{
        if(bytes.length>=4&&bytes[0]===0x1f&&bytes[1]===0x43&&bytes[2]===0xb6&&bytes[3]===0x75){
          ...
          for(const frame of parser.webmAv1Frames(bytes,fps)){if(frame.type==='key')replay=[];replay.push(frame);if(replay.length>Math.max(120,fps*2))replay.shift();arrivalTimes.set(frame.timestamp,performance.now());queuedSinceOutput++;decoder.decode(new EncodedVideoChunk(frame))}
...
function cleanupRemoteNativeScreen({keepChannel=false}={}){
  nativeRemotePlayer?.destroy();nativeRemotePlayer=null;if(!keepChannel&&remoteNativeScreenChannel){try{remoteNativeScreenChannel.onmessage=null;remoteNativeScreenChannel.close()}catch{}remoteNativeScreenChannel=null}if(nativeRemoteAudio){try{nativeRemoteAudio.pause();nativeRemoteAudio.srcObject=null}catch{}}try{remoteScreen.removeAttribute('src');remoteScreen.load()}catch{}
}
function beginRemoteNativeScreen(meta,channel){
  ...
  try{nativeRemotePlayer=createNativeScreenPlayer(remoteScreen,meta.codec||'AV1',requestFallback,meta)}catch(error){screenStatus.textContent=error.message;requestFallback();return false}channel._nativeReceive={fragments:new Map(),complete:new Map(),nextSeq:0,player:nativeRemotePlayer};screenStatus.textContent='Friend sharing · '+(meta.codec||'AV1')+' · '+(meta.width||'source')+'×'+(meta.height||'source')+' · '+(meta.fps||60)+'fps';updateScreenLayout();return true;
}
function receiveNativeScreenPacket(channel,data){
  const state=channel._nativeReceive,bytes=data instanceof ArrayBuffer?new Uint8Array(data):ArrayBuffer.isView(data)?new Uint8Array(data.buffer,data.byteOffset,data.byteLength):null;if(!state||!bytes||bytes.byteLength<12)return;const view=new DataView(bytes.buffer,bytes.byteOffset,bytes.byteLength);if(view.getUint32(0)!==NATIVE_SCREEN_PACKET)return;const seq=view.getUint32(4),part=view.getUint16(8),total=view.getUint16(10);if(!total||total>512||part>=total||seq>state.nextSeq+512)return;state.pendingBytes=Number(state.pendingBytes)||0;let entry=state.fragments.get(seq);if(!entry){if(state.fragments.size>=32)return;entry={parts:new Array(total),count:0,bytes:0};state.fragments.set(seq,entry)}if(entry.parts.length!==total||entry.parts[part])return;entry.parts[part]=bytes.slice(12);entry.count++;entry.bytes+=bytes.byteLength-12;state.pendingBytes+=bytes.byteLength-12;if(entry.bytes>8*1024*1024||state.pendingBytes>24*1024*1024){state.fragments.clear();state.complete.clear();state.pendingBytes=0;try{channel.close()}catch{}return}if(entry.count===total){const joined=new Uint8Array(entry.bytes);let offset=0;for(const value of entry.parts){joined.set(value,offset);offset+=value.byteLength}state.fragments.delete(seq);state.complete.set(seq,joined);while(state.complete.has(state.nextSeq)){const complete=state.complete.get(state.nextSeq);state.player?.append(complete);state.pendingBytes=Math.max(0,state.pendingBytes-complete.byteLength);state.complete.delete(state.nextSeq++)}}
}
function wireNativeScreenChannel(channel,{remote=false}={}){
  channel.binaryType='arraybuffer';if(remote){remoteNativeScreenChannel=channel;channel.onmessage=event=>{if(typeof event.data==='string'){try{const value=JSON.parse(event.data);if(value.t==='native-screen-meta')beginRemoteNativeScreen(value,channel);else if(value.t==='native-screen-end')clearRemoteScreenShare()}catch{}return}receiveNativeScreenPacket(channel,event.data)};channel.onclose=()=>{if(remoteNativeScreenChannel===channel&&remoteScreenExpected)clearRemoteScreenShare()};return}
  nativeScreenChannel=channel;channel.onmessage=event=>{if(typeof event.data!=='string')return;try{const value=JSON.parse(event.data);if(value.t==='native-screen-fallback')fallbackNativeScreenToWebRtc()}catch{}};
}
...
async function nativeChannelBackpressure(channel){if(channel.readyState!=='open')return false;if(channel.bufferedAmount<NATIVE_SCREEN_BUFFER_HIGH)return true;channel.bufferedAmountLowThreshold=3*1024*1024;return new Promise(resolve=>{let done=false;const finish=value=>{if(done)return;done=true;clearTimeout(timer);channel.removeEventListener('bufferedamountlow',low);channel.removeEventListener('close',closed);resolve(value)};const low=()=>finish(true),closed=()=>finish(false),timer=setTimeout(()=>finish(channel.readyState==='open'&&channel.bufferedAmount<NATIVE_SCREEN_BUFFER_HIGH),5000);channel.addEventListener('bufferedamountlow',low,{once:true});channel.addEventListener('close',closed,{once:true})})}
async function sendNativeScreenSegment(channel,item){
  const data=item.data instanceof Uint8Array?item.data:new Uint8Array(item.data),total=Math.max(1,Math.ceil(data.byteLength/NATIVE_SCREEN_PART));for(let part=0;part<total;part++){if(!await nativeChannelBackpressure(channel))return false;const start=part*NATIVE_SCREEN_PART,end=Math.min(data.byteLength,start+NATIVE_SCREEN_PART),packet=new Uint8Array(12+end-start),view=new DataView(packet.buffer);view.setUint32(0,NATIVE_SCREEN_PACKET);view.setUint32(4,item.seq);view.setUint16(8,part);view.setUint16(10,total);packet.set(data.subarray(start,end),12);channel.send(packet.buffer)}return true;
}
...
function targetNativeAv1BitrateKbps(width,height,fps){const pixels=Math.max(1,(Number(width)||1920)*(Number(height)||1080)),cadence=Number(fps)===30?.65:1,targetMbps=Math.max(8,10*(pixels/(1920*1080))*cadence);return Math.round(Math.min(screenBitrateMbps,targetMbps)*1000)}
