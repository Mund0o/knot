...
let screenTransceiver=null,screenActive=false,screenStarting=false,screenStream=null,screenGen=0,screenSenders=[],screenStatsTimer=null,screenStatsLast=null,screenRecoveryLevel=0,screenBadSamples=0,screenGoodSamples=0,remoteScreenExpected=false,remoteScreenSuppressed=false,screenAudioDebug='';
let nativeScreenSession=null,nativeScreenChannel=null,remoteNativeScreenChannel=null,nativeLocalPlayer=null,nativeRemotePlayer=null,nativeRemoteAudio=null,nativeScreenBypassOnce=false,nativeScreenAnnounced=false,nativeScreenAudioStream=null;
...
let speakingAudioCtx=null;const speakingMonitors=new Map();
...
function stopSpeakingMonitor(key){const monitor=speakingMonitors.get(key);if(!monitor)return;clearInterval(monitor.timer);try{monitor.source.disconnect()}catch{}try{monitor.analyser.disconnect()}catch{}try{monitor.sink?.disconnect()}catch{}speakingMonitors.delete(key);paintSpeaking(key,false);if(!speakingMonitors.size&&speakingAudioCtx){try{speakingAudioCtx.close()}catch{}speakingAudioCtx=null}}
function monitorSpeaking(key,stream){
  stopSpeakingMonitor(key);const track=stream?.getAudioTracks?.()[0];if(!track)return;
  try{
    ...
    const source=speakingAudioCtx.createMediaStreamSource(new MediaStream([track])),analyser=speakingAudioCtx.createAnalyser(),sink=speakingAudioCtx.createGain(),samples=new Float32Array(256),monitor={source,analyser,sink,timer:null,speaking:false,noiseFloor:.003,holdUntil:0};
    ...
    speakingMonitors.set(key,monitor);track.addEventListener?.('ended',()=>stopSpeakingMonitor(key),{once:true});
...
function applyRemoteCallState(active,session=''){
  if(active){remoteCallSessionId=String(session||remoteCallSessionId||'legacy');friendLeftNotified=false;setFriendPresence(true,{animate:true,sound:false});renderCallPeerProfile();setRemoteCallAudio(callActive);return}
...
// Knot has exactly one recipient. A 128 MiB window keeps a fast wired link full
...
function setupPeer(){
  ...
  pc.ontrack=e=>{logCallEvent('Diag: ontrack kind='+e.track.kind);try{const stream=e.streams[0]||new MediaStream([e.track]);
    ...
    const isVoiceAudio=e.track.kind==='audio'&&e.transceiver?.sender===audioTransceiver;
...
function monitorRemoteScreenDecode(receiver,track,requestFallback){
  ...
  const sample=async()=>{try{
    ...
    if(stalls>=2&&/video\/AV1/i.test(codec?.mimeType||'')){
      screenStatus.textContent='AV1 decoder stalled — switching to H.264';
...
let screenCursor='always',screenContentHint='motion',screenBitrateMbps=60,screenCodec='auto',shareResolution='1080',shareFrameRate=60;
...
function addScreenShareSettings(){
  ...
  page.innerHTML='<div><h3>Screen sharing</h3><p>Knot preserves the resolution and frame-rate preset you select instead of silently lowering it.</p></div><label class="settings-field"><span>Video codec</span><select id="screenCodecSetting"><option value="auto">Automatic — hardware-friendly</option><option value="H264">H.264 — widest support</option><option value="AV1">AV1 — best compression</option><option value="VP9">VP9</option><option value="VP8">VP8</option></select></label><label class="settings-field"><span>Maximum video bitrate <output id="screenBitrateValue">60 Mbps</output></span><input id="screenBitrateSetting" type="range" min="8" max="80" value="60" step="1" /></label><label class="settings-field"><span>Content optimization</span><select id="screenContentHintSetting"><option value="motion">Motion — games and video</option><option value="detail">Detail — text and documents</option></select></label><label class="settings-field"><span>Cursor</span><select id="screenCursorSetting"><option value="always">Always show</option><option value="motion">Show while moving</option><option value="never">Hide cursor</option></select></label><p class="settings-hint">Native AV1 4K60 targets 24 Mbps and drops stale video before it can delay your microphone. If a hardware decoder cannot sustain AV1 under 100 ms, Knot switches that viewer to H.264 instead of showing a black or delayed stream.</p><div class="settings-inline-actions"><button id="testScreenAudio" type="button">Test isolated computer audio</button></div><p id="screenAudioTestStatus" class="settings-hint" aria-live="polite">Checks the same isolated audio route used by a real share.</p>';
  ...
  const updateBitrate=()=>{screenBitrateMbps=Math.max(8,Math.min(80,Number(bitrate.value)||60));bitrateValue.textContent=screenBitrateMbps+' Mbps';bitrate.style.setProperty('--range-fill',((screenBitrateMbps-8)/72*100)+'%');ssSet('screenBitrate',String(screenBitrateMbps))};
  ...
  return async()=>{
    const savedBitrate=Number(await ss('screenBitrate'));screenBitrateMbps=Number.isFinite(savedBitrate)&&savedBitrate>=8&&savedBitrate<=80?savedBitrate:60;bitrate.value=String(screenBitrateMbps);updateBitrate();
    ...
    const savedResolution=await ss('shareResolution');shareResolution=['source','720','1080','1440','2160'].includes(savedResolution)?savedResolution:'1080';
    const savedFps=Number(await ss('shareFrameRate'));shareFrameRate=savedFps===30?30:60;syncScreenPreset();
  };
}
...
let conversationHistories={},historySaveTimer=null,serverVoiceStream=null,serverScreenStream=null,serverNativeScreenSession=null,serverNativeLocalPlayer=null,serverNativeScreenAudioStream=null,serverNativeScreenInit=null,serverNativeScreenBypassOnce=false,serverVoiceMuted=false,serverScreenStarting=false,serverScreenGen=0,joinedVoiceServerId='',joinedVoiceChannelId='',joinedVoiceAt=0,voiceElapsedTimer=null,draggedChannelId='';const serverPeers=new Map();
...
function showFriends({expand=true}={}){if(expand)setSocialSidebarCollapsed(false);activeServerId='';activeChannelId='';if(!dmCallOngoing()&&!serverVoiceStream){callBtn.textContent='Start call';callStatus.textContent='Voice off';callStatus.className='call-status'}$('#friendsNavigation').hidden=false;$('#serverNavigation').hidden=true;$('#serverMemberPanel').hidden=true;document.body.classList.remove('server-view');document.querySelectorAll('#serverList .rail-button').forEach(button=>button.classList.remove('active'));$('#homeButton').classList.add('active');if(!serverVoiceStream)closeServerMesh();else{$('#serverVoiceStage').hidden=true;renderServerVoiceUI()}if(!activePeerId)showFriendsLanding();renderDmVoiceUI()}
...
function renderDmVoiceUI(){const dock=$('#dmVoiceDock');if(!dock)return;const connected=callActive||friendInCall;dock.hidden=!connected;document.body.classList.toggle('dm-call-connected',connected);if(!connected)return;$('#dmVoiceDockName').textContent=directoryUser(dmCallPeerId)?.name||friendName||'Direct call';$('#dmVoiceDockTime').textContent=voiceElapsed(callStart||Date.now());$('#dmVoiceMute').classList.toggle('active',micMuted);$('#dmVoiceMute').disabled=!callActive;$('#dmVoiceShare').classList.toggle('active',screenActive||screenStarting);$('#dmVoiceShare').disabled=!callActive||!pc}
...
function selectServer(id){const server=directorySnapshot.servers?.find(item=>item.id===id);if(!server)return;setSocialSidebarCollapsed(false);if(pc&&!dmCallOngoing())disconnectRoom();if(serverVoiceStream||joinedVoiceChannelId)stopServerVoice();activePeerId='';activeServerId=id;document.body.classList.add('server-view');$('#friendsNavigation').hidden=true;$('#serverNavigation').hidden=false;$('#serverMemberPanel').hidden=false;$('#homeButton').classList.remove('active');renderServers();renderChannels();renderDmVoiceUI();const first=server.channels?.find(channel=>channel.type==='text')||server.channels?.[0];if(first)selectServerChannel(id,first.id)}
async function selectServerChannel(serverId,channelId){const server=directorySnapshot.servers?.find(item=>item.id===serverId),channel=server?.channels?.find(item=>item.id===channelId);if(!server||!channel)return;activeServerId=serverId;activeChannelId=channelId;activePeerId='';roomTitle.textContent=channel.name;$('#chatTitle').textContent=channel.name;$('#roomContextLabel').textContent=server.name.toUpperCase();$('#chatModePill').textContent=channel.type==='voice'?'VOICE':'P2P MESH';messageInput.placeholder='Message #'+channel.name;openConversation('server:'+serverId+':'+channelId);renderChannels();if(channel.type==='voice'&&!serverVoiceStream){setServerStatus('Double-click '+channel.name+' to join voice');callBtn.textContent='Join voice'}else setServerStatus('Connecting to online server members…');syncServerMesh();messageInput.disabled=false;messageForm.querySelector('.send').disabled=false;fileInput.disabled=true}
function serverOnlineMembers(){const server=activeServer();return server?(server.members||[]).filter(id=>id!==directoryUserId&&directorySnapshot.members?.[id]?.online):[]}
...
function addServerVoiceAudio(peerId,state,track,stream){
  const audio=document.createElement('audio');audio.autoplay=true;audio.srcObject=stream;audio.hidden=true;document.body.append(audio);state.audios.push(audio);monitorSpeaking('server:'+peerId,stream);audio.play().catch(()=>{});track.onended=()=>{stopSpeakingMonitor('server:'+peerId);audio.remove();state.audios=state.audios.filter(item=>item!==audio)};
}
function addServerScreenVideo(peerId,state,track,stream){
  ...
  const video=document.createElement('video');video.autoplay=false;video.playsInline=true;video.srcObject=stream;video.dataset.peerId=peerId;video.muted=true;$('#serverVoiceScreens').append(video);state.screen=video;state.screenStreamId=stream.id;
...
function beginServerNativeScreen(peerId,state,meta,channel){
  if(meta.serverId!==state.context.serverId)return false;clearServerNativeScreen(state,{keepChannel:true});try{state.screen?.pause();state.screen?.remove()}catch{}const video=document.createElement('video');video.autoplay=false;video.playsInline=true;video.dataset.peerId=peerId;video.muted=true;$('#serverVoiceScreens').append(video);state.screen=video;state.screenStreamId='native';state.nativeScreenAudioExpected=!!meta.audio;
  const fallback=()=>{try{if(channel.readyState==='open')channel.send(JSON.stringify({t:'native-screen-fallback',serverId:state.context.serverId}))}catch{}};try{state.nativeScreenPlayer=createNativeScreenPlayer(video,meta.codec||'AV1',fallback,meta)}catch(error){fallback();clearServerNativeScreen(state,{keepChannel:true});setServerStatus(error.message);return false}channel._nativeReceive=nativeScreenReceiveState(state.nativeScreenPlayer,meta,fallback);try{channel.send(JSON.stringify({t:'native-screen-ready',serverId:state.context.serverId,transportVersion:NATIVE_SCREEN_PROTOCOL}))}catch{}renderServerVoiceUI();return true
}
function addServerNativeScreenAudio(state,track,stream){
  if(state.screenAudio){try{state.screenAudio.remove()}catch{}}const audio=document.createElement('audio');audio.autoplay=true;audio.hidden=true;audio.srcObject=stream;audio.volume=remoteScreen.volume;audio.muted=serverFocusedShareId!==stream._knotPeerId&&serverFocusedShareId!==state.screen?.dataset.peerId;document.body.append(audio);state.screenAudio=audio;state.nativeScreenAudioExpected=false;track.onended=()=>{if(state.screenAudio===audio){audio.remove();state.screenAudio=null}};renderServerShareExperience()
}
function wireServerNativeScreenChannel(peerId,state,channel,{remote=false}={}){
  channel.binaryType='arraybuffer';if(remote){state.nativeReceiveChannel=channel;channel.onmessage=event=>{if(typeof event.data==='string'){try{const value=JSON.parse(event.data);if(value.t==='native-screen-meta')beginServerNativeScreen(peerId,state,value,channel);else if(value.t==='native-screen-audio'&&value.serverId===state.context.serverId)state.nativeScreenAudioExpected=!!value.active;else if(value.t==='native-screen-end')clearServerNativeScreen(state,{keepChannel:true})}catch{}return}receiveNativeScreenPacket(channel,event.data)};channel.onclose=()=>{if(state.nativeReceiveChannel===channel){clearServerNativeScreen(state,{keepChannel:true});state.nativeReceiveChannel=null;renderServerVoiceUI()}};return}
  state.nativeSendChannel=channel;channel.onmessage=event=>{if(typeof event.data!=='string')return;try{const value=JSON.parse(event.data);if(value.t==='native-screen-ready'&&value.serverId===state.context.serverId){channel._nativePeerProtocol=Number(value.transportVersion)||0;clearTimeout(channel._nativeProtocolTimer)}else if(value.t==='native-screen-fallback'&&value.serverId===state.context.serverId)fallbackServerNativeToWebRtc()}catch{}};channel.onopen=()=>announceServerNativeChannel(channel);channel.onclose=()=>{clearTimeout(channel._nativeProtocolTimer);if(state.nativeSendChannel===channel)state.nativeSendChannel=null}
}
async function ensureServerPeer(peerId,context={type:'server',serverId:activeServerId,channelId:activeChannelId}){
  ...
  if(voicePeerAllowed(peerId)){
    ...
    if(serverScreenStream)for(const track of serverScreenStream.getTracks()){
      const sender=connection.addTrack(track,serverScreenStream);state.screenSenders.push(sender);if(track.kind==='video'){applyScreenCodecPreference(connection,sender);await configureScreenVideoSender(sender,track,shareFrameRate)}
    }
  }
...
function closeServerMesh(){for(const [peerId,state] of serverPeers){stopSpeakingMonitor('server:'+peerId);try{state.channel?.close()}catch{}clearServerNativeScreen(state);try{state.nativeSendChannel?.close()}catch{}try{state.pc.close()}catch{}for(const audio of state.audios||[])try{audio.remove()}catch{}try{state.screen?.remove()}catch{}}serverPeers.clear()}
...
function announceServerNativeChannel(channel){if(channel.readyState!=='open'||!serverNativeScreenSession)return false;if(channel._nativeSend?.sessionId===serverNativeScreenSession.id)return true;try{initializeNativeScreenSender(channel,{t:'native-screen-meta',serverId:joinedVoiceServerId,codec:'AV1',fps:serverNativeScreenSession.fps,width:serverNativeScreenSession.width,height:serverNativeScreenSession.height,encoder:serverNativeScreenSession.encoder,latencyTargetMs:serverNativeScreenSession.latencyTargetMs,audio:false},serverNativeScreenSession.id,()=>fallbackServerNativeToWebRtc());return true}catch{return false}}
...
async function attachServerNativeScreenAudio(gen){
  if(!screenAudioOn||!serverNativeScreenSession||gen!==serverScreenGen)return;const track=await linuxShareAudioTrack();if(!track||!serverNativeScreenSession||gen!==serverScreenGen){try{track?.stop()}catch{};return}serverNativeScreenAudioStream=new MediaStream([track]);try{track.contentHint='music'}catch{};const peers=[...serverPeers].filter(([peerId])=>voicePeerAllowed(peerId)),results=await Promise.allSettled(peers.map(([peerId,state])=>attachServerNativeAudioToPeer(peerId,state))),failed=results.some(result=>result.status==='rejected')||!serverNativeScreenSession||gen!==serverScreenGen,label=serverNativeScreenSession?.encoder||'GPU';if(failed){for(const [peerId,state] of peers){const sender=state.screenSenders.find(value=>value.track===track);if(sender){try{state.pc.removeTrack(sender)}catch{}state.screenSenders=state.screenSenders.filter(value=>value!==sender)}if(state.nativeSendChannel?.readyState==='open')try{state.nativeSendChannel.send(JSON.stringify({t:'native-screen-audio',serverId:state.context.serverId,active:false}))}catch{}renegotiateServerPeer(peerId,state).catch(()=>{})}try{track.stop()}catch{}serverNativeScreenAudioStream=null;window.pairEnv.stopLinuxShareAudio?.();setServerStatus('Sharing · '+label+' AV1 · computer sound unavailable',true)}else setServerStatus('Sharing · '+label+' AV1 · computer sound live',true)
}
async function pumpServerNativeScreen(gen,session){
  let audioStarted=false;while(serverNativeScreenSession?.id===session.id&&gen===serverScreenGen){const item=await window.pairNativeScreen.read(session.id);if(serverNativeScreenSession?.id!==session.id||gen!==serverScreenGen)break;if(item?.data){if(item.kind==='init')serverNativeScreenInit=item.data instanceof Uint8Array?item.data.slice():new Uint8Array(item.data);serverNativeLocalPlayer?.append(item.data);const channels=[...serverPeers.values()].map(state=>state.nativeSendChannel).filter(channel=>channel?.readyState==='open');const sent=await Promise.allSettled(channels.map(channel=>sendServerNativeItem(channel,item)));sent.forEach((result,index)=>{if(result.status==='rejected'||result.value===false)try{channels[index].close()}catch{}});if(!audioStarted){audioStarted=true;void attachServerNativeScreenAudio(gen)}continue}if(!item?.active){if(item?.error)setServerStatus('Native share stopped: '+item.error);break}}
  if(serverNativeScreenSession?.id===session.id&&gen===serverScreenGen)await stopServerScreenShare()
}
async function startServerNativeScreenShare(){
  serverScreenStarting=true;const gen=++serverScreenGen;renderServerVoiceUI();try{const [width,height]=selectedNativeDimensions(),fps=shareFrameRate===30?30:60,session=await window.pairNativeScreen.start({codec:'av1',fps,width,height,bitrateKbps:targetNativeAv1BitrateKbps(width,height,fps),cursor:screenCursor});if(!session||session.error)throw new Error(session?.error||'GPU AV1 capture did not start');serverNativeScreenSession=session;serverNativeScreenInit=null;const preview=$('#serverVoiceScreenPreview');preview.hidden=false;preview.muted=true;serverNativeLocalPlayer=createNativeScreenPlayer(preview,'AV1',()=>{}, {...session,decode:false});await Promise.all([...serverPeers].filter(([peerId])=>voicePeerAllowed(peerId)).map(([peerId,state])=>ensureServerNativeChannel(peerId,state)));if(gen!==serverScreenGen||serverNativeScreenSession?.id!==session.id)return true;setServerStatus('Choose a display · starting '+(session.encoder||'GPU')+' AV1…',true);serverFocusedShareId=directoryUserId;renderServerVoiceUI();void pumpServerNativeScreen(gen,session);return true}catch(error){if(serverNativeScreenSession)window.pairNativeScreen?.stop(serverNativeScreenSession.id);serverNativeScreenSession=null;serverNativeLocalPlayer?.destroy();serverNativeLocalPlayer=null;$('#serverVoiceScreenPreview').hidden=true;setServerStatus('Native AV1 unavailable: '+(error?.message||error));return false}finally{if(gen===serverScreenGen)serverScreenStarting=false;renderServerVoiceUI()}
}
async function fallbackServerNativeToWebRtc(){if(!serverNativeScreenSession)return;const previous=screenCodec;serverNativeScreenBypassOnce=true;await stopServerScreenShare();screenCodec='H264';try{await startServerScreenShare()}finally{screenCodec=previous}}
...
async function startServerScreenShare(){
  ...
  try{
    ...
    serverScreenStream=stream;startupStream=null;const preview=$('#serverVoiceScreenPreview');preview.srcObject=stream;preview.hidden=false;preview.muted=true;try{await preview.play()}catch{}track.onended=()=>{if(serverScreenStream===stream)stopServerScreenShare()};
    const starts=[];for(const [peerId,state] of serverPeers){if(!voicePeerAllowed(peerId))continue;starts.push((async()=>{const sender=state.pc.addTrack(track,stream);state.screenSenders.push(sender);applyScreenCodecPreference(state.pc,sender);await configureScreenVideoSender(sender,track,fps);await renegotiateServerPeer(peerId,state)})())}await Promise.allSettled(starts);if(gen!==serverScreenGen||serverScreenStream!==stream)return;
  ...
  }catch(error){try{startupStream?.getTracks().forEach(track=>track.stop())}catch{}if(serverScreenStream){serverScreenStream.getTracks().forEach(track=>track.stop());serverScreenStream=null}cleanupNativeScreenCapture();if(window.pairEnv?.platform==='linux')window.pairEnv.stopLinuxShareAudio?.();if(error?.name!=='NotAllowedError')setServerStatus('Screen share failed: '+(error?.message||error))}
...
async function attachServerScreenAudio(gen,stream){
  ...
  const discard=()=>{for(const [peerId,state,sender] of created){try{state.pc.removeTrack(sender)}catch{}state.screenSenders=state.screenSenders.filter(value=>value!==sender);renegotiateServerPeer(peerId,state).catch(()=>{})}try{audioTrack?.stop()}catch{};try{stream.removeTrack(audioTrack)}catch{};if(window.pairEnv?.platform==='linux')window.pairEnv.stopLinuxShareAudio?.();else cleanupNativeScreenCapture()};if(!audioTrack)return;if(gen!==serverScreenGen||serverScreenStream!==stream){discard();return}
...
function disconnectRoom(){if(pc&&pc._connectTimer){clearTimeout(pc._connectTimer);pc._connectTimer=null}
  ...
  if(screenStatsTimer){clearInterval(screenStatsTimer);screenStatsTimer=null}screenStatsLast=null;screenRecoveryLevel=0;screenBadSamples=0;screenGoodSamples=0;
...
async function startLocalTestCall(){
  ...
  try{
    ...
    dmCallPeerId=dmPeerId||activePeerId;callActive=true;callStart=Date.now();callBtn.textContent='End call';callBtn.title='End local mic test';callBtn.disabled=false;muteBtn.hidden=false;micMuted=false;muteBtn.textContent='Mute';applyMicTransmission();setParticipant(participantYou,true);playSound('ring');callStatus.textContent='Testing microphone locally';callStatus.className='call-status live';
...
async function startCall(){
  ...
  try{
    ...
    dmCallPeerId=dmPeerId||activePeerId;callActive=true;callStart=Date.now();monitorSpeaking('dm-self',localStream);setRemoteCallAudio(true);callBtn.textContent='End call';callBtn.title='End voice call';callBtn.disabled=false;muteBtn.hidden=false;micMuted=false;muteBtn.textContent='Mute';muteBtn.title='Mute microphone';applyMicTransmission();
...
async function endCall(silent){
  ...
  stopSpeakingMonitor('dm-self');stopSpeakingMonitor('dm-friend');
...
async function tuneDisplayTrack(track){
  ...
  try{await track.applyConstraints(constraints)}catch(error){console.warn('[VIDEO] display constraints partially unsupported:',error?.message||error);try{await track.applyConstraints({frameRate:{ideal:fps,max:fps}})}catch{}}
}
async function waitForDisplayFrames(track,timeoutMs=4000){
  ...
  try{
    ...
    const settings=track.getSettings?.()||{},width=settings.width||video.videoWidth||0,height=settings.height||video.videoHeight||0,fps=Math.round(settings.frameRate||0);if(!width||!height)throw new Error('Screen capture returned an empty video target');return{width,height,fps};
  }finally{clearTimeout(timer);video.pause();video.srcObject=null}
}
function orderedScreenCodecs(caps,selectedCodec=screenCodec){
  const automatic=['H264','VP9','VP8','AV1'],requested=selectedCodec==='auto'?automatic:[selectedCodec,...automatic],order=[...new Set(requested.map(name=>name.toUpperCase()))],seen=new Set(),result=[];
...
function targetScreenBitrate(width,height,fps){
  const pixels=Math.max(1,(Number(width)||1920)*(Number(height)||1080)),cadence=Number(fps)===30?.62:1,base=14*(pixels/(1920*1080))*cadence;
  return Math.round(Math.min(screenBitrateMbps,Math.max(4,base))*1000000);
}
async function configureScreenVideoSender(sender,track,fps){
  const settings=track?.getSettings?.()||{},requestedFps=fps===30?30:60,maxBitrate=targetScreenBitrate(settings.width,settings.height,requestedFps);
...
async function recoverOverloadedScreenSender(sender){
  if(!screenActive||screenRecoveryLevel)return;screenRecoveryLevel=1;screenBadSamples=0;screenGoodSamples=0;const track=screenStream?.getVideoTracks?.()[0];try{await configureScreenVideoSender(sender,track,shareFrameRate)}catch{}
  screenStatus.textContent='Sharing · encoder cannot sustain the selected preset'+screenAudioDebug;logCallEvent('Screen encoder overloaded — selected quality preserved');
}
async function restoreScreenSenderQuality(sender){
  if(!screenActive||screenRecoveryLevel<=0)return;screenRecoveryLevel=0;screenGoodSamples=0;const track=screenStream?.getVideoTracks?.()[0];try{await configureScreenVideoSender(sender,track,shareFrameRate)}catch{}
}
function startScreenStats(sender){
  if(screenStatsTimer)clearInterval(screenStatsTimer);screenStatsLast=null;screenRecoveryLevel=0;screenBadSamples=0;screenGoodSamples=0;
  const sample=async()=>{try{
    ...
    screenStatsLast={bytes,frames,totalEncode,at:now};const fps=Math.round(out.framesPerSecond||0),w=out.frameWidth||0,h=out.frameHeight||0,reason=out.qualityLimitationReason||'',requestedFps=shareFrameRate;
    const overloaded=previous&&(reason==='cpu'||encodeMs>1000/Math.max(1,requestedFps)*1.15||(encodedDelta===0&&bytes===previous.bytes));screenBadSamples=overloaded?screenBadSamples+1:0;screenGoodSamples=!overloaded&&fps>=requestedFps*.85?screenGoodSamples+1:0;
    if(screenBadSamples>=2&&!screenRecoveryLevel)await recoverOverloadedScreenSender(sender);else if(screenRecoveryLevel&&screenGoodSamples>=5)await restoreScreenSenderQuality(sender);
    ...
    const status='Sharing'+(w&&h?' · '+w+'×'+h:'')+(fps?' · '+fps+'fps':'')+(mbps!=='…'?' · '+mbps+' Mbps':'')+(codec?.mimeType?' · '+codec.mimeType.replace('video/',''):'')+(encodeMs?' · '+encodeMs.toFixed(1)+'ms encode':'')+network+(screenRecoveryLevel?' · preset under load':'')+screenAudioDebug;screenStatus.textContent=status;screenBtn.title=status;
  }catch{}};
  sample();screenStatsTimer=setInterval(sample,2000);
}
const NATIVE_SCREEN_PACKET=0x4b4e5331,NATIVE_SCREEN_PART=60*1024,NATIVE_SCREEN_BUFFER_HIGH=256*1024,NATIVE_SCREEN_BUFFER_LOW=64*1024,NATIVE_SCREEN_MAX_SEGMENT=4*1024*1024,NATIVE_SCREEN_GAP_WAIT=120,NATIVE_SCREEN_PROTOCOL=2;
function nativeScreenMime(codec='AV1'){return codec.toUpperCase()==='AV1'?'video/webm; codecs="av01.0.13M.08"':''}
...
function createWebCodecsNativeScreenPlayer(video,codec,onError=()=>{},options={}){
  ...
  const allowSoftwareFallback=options.allowSoftwareFallback!==false,enforceLatencyTarget=options.enforceLatencyTarget!==false,fps=Number(options.fps)||60,configuredWidth=Number(options.width)||0,configuredHeight=Number(options.height)||0;
  let canvas,context,decoder=null,config=null,configured=false,destroyed=false,decoderDisabled=false,failureReported=false,latencyExceeded=false,playbackActive=true,decodedFrames=0,paintedFrames=0,firstPaintAt=0,frameWidth=0,frameHeight=0,softwareFallback=false,hardwareUnavailable=false,replay=[],queuedSinceOutput=0,configuredAt=0,lastOutputAt=0,needsKeyframe=true;const arrivalTimes=new Map(),latencySamples=[];
  const closeDecoderSoon=value=>{if(!value)return;queueMicrotask(()=>{try{value.close()}catch{}})};
  const fail=error=>{if(destroyed||failureReported)return;failureReported=true;decoderDisabled=true;const failed=decoder;decoder=null;closeDecoderSoon(failed);const reason=error instanceof Error?error:new Error(String(error||'Native AV1 low-latency decode failed'));queueMicrotask(()=>{if(!destroyed)onError(reason)})};
  ...
  const output=frame=>{
    ...
    if(playbackActive){
      ...
      try{context.drawImage(frame,0,0,canvas.width,canvas.height);const paintedAt=performance.now(),arrivedAt=arrivalTimes.get(frame.timestamp);paintedFrames++;if(!firstPaintAt)firstPaintAt=paintedAt;if(arrivedAt!==undefined){latencySamples.push(paintedAt-arrivedAt);if(latencySamples.length>360)latencySamples.shift();if(enforceLatencyTarget&&softwareFallback&&latencySamples.length>=120&&decodedFrames%15===0){const recent=latencySamples.slice(-60).sort((a,b)=>a-b),p95=recent[Math.ceil(recent.length*.95)-1];if(p95>100){latencyExceeded=true;fail(new Error('AV1 software decode exceeded the 100ms latency target'))}}}}catch(error){fail(error)}
    }
    arrivalTimes.delete(frame.timestamp);
    frame.close();
  };
  const disableLocalDecoder=()=>{hardwareUnavailable=true;decoderDisabled=true;const unavailable=decoder;decoder=null;closeDecoderSoon(unavailable);replay=[];arrivalTimes.clear();drawPreviewUnavailable();return true};
  const startSoftwareDecoder=error=>{
    if(destroyed||decoderDisabled)return false;
    if(!allowSoftwareFallback)return disableLocalDecoder();
    if(softwareFallback){fail(error);return false}
    const previous=decoder;let next=null;softwareFallback=true;
    try{next=makeDecoder('prefer-software');next.configure({...config,hardwareAcceleration:'prefer-software'});decoder=next;closeDecoderSoon(previous);configuredAt=performance.now();lastOutputAt=configuredAt;queuedSinceOutput=0;needsKeyframe=true;for(const frame of replay){if(needsKeyframe&&frame.type!=='key')continue;if(frame.type==='key')needsKeyframe=false;queuedSinceOutput++;decoder.decode(new EncodedVideoChunk(frame))}return true}catch(fallbackError){closeDecoderSoon(next);decoder=previous;fail(fallbackError);return false}
  };
  ...
  try{
    canvas=document.createElement('canvas');canvas.className='native-screen-canvas';canvas.setAttribute('aria-hidden','true');context=canvas.getContext('2d',{alpha:false,desynchronized:true});if(!context)return null;video.after(canvas);video.style.opacity='0';try{video.pause();video.removeAttribute('src');video.srcObject=null}catch{}
    decoder=makeDecoder('prefer-hardware');lastOutputAt=performance.now();
  }catch{return null}
  const stallTimer=setInterval(()=>{if(destroyed||decoderDisabled||!configured||queuedSinceOutput<30)return;const now=performance.now();if(now-Math.max(lastOutputAt,configuredAt)<1500)return;if(!softwareFallback)startSoftwareDecoder(new Error('Hardware AV1 decoder produced no frames'));else fail(new Error('AV1 decoder produced no frames'))},250);
  return{
    mode:'webcodecs',
    append(data){
      ...
      try{
        if(bytes.length>=4&&bytes[0]===0x1f&&bytes[1]===0x43&&bytes[2]===0xb6&&bytes[3]===0x75){
          ...
          config={codec:parser.av1Codec(description).replace('H.','M.'),hardwareAcceleration:'prefer-hardware',optimizeForLatency:true};if(configuredWidth&&configuredHeight){config.codedWidth=configuredWidth;config.codedHeight=configuredHeight;config.displayAspectWidth=configuredWidth;config.displayAspectHeight=configuredHeight}try{decoder.configure(config)}catch(error){if(!startSoftwareDecoder(error))throw error}configured=true;needsKeyframe=true;configuredAt=performance.now();lastOutputAt=configuredAt;
        }
        return true;
      }catch(error){fail(error);return false}
    },
    reset(){if(destroyed||decoderDisabled||!decoder)return false;replay=[];arrivalTimes.clear();latencySamples.length=0;queuedSinceOutput=0;needsKeyframe=true;try{decoder.reset();if(config)decoder.configure({...config,hardwareAcceleration:softwareFallback?'prefer-software':'prefer-hardware'});configured=!!config;configuredAt=performance.now();lastOutputAt=configuredAt;return true}catch(error){fail(error);return false}},
    setActive(active){playbackActive=!!active;if(canvas)canvas.hidden=!playbackActive},
    destroy(){if(destroyed)return;destroyed=true;clearInterval(stallTimer);try{decoder?.close()}catch{}canvas?.remove();video.style.opacity='';try{video.pause();video.srcObject=null;video.removeAttribute('src');video.load()}catch{}},
    stats(){const steady=latencySamples.slice(-120).sort((a,b)=>a-b),p95=steady.length?steady[Math.min(steady.length-1,Math.ceil(steady.length*.95)-1)]:0;return{decodedFrames,paintedFrames,firstPaintAt,width:frameWidth,height:frameHeight,decodeQueueSize:decoderDisabled?0:decoder?.decodeQueueSize||0,softwareFallback,hardwareUnavailable,latencyExceeded,steadyStateP95Ms:p95,latencySamples:steady.length}}
  };
}
function createMseNativeScreenPlayer(video,codec,onError=()=>{}){
  ...
  try{video.pause();video.srcObject=null}catch{}const source=new MediaSource(),url=URL.createObjectURL(source),queue=[];let buffer=null,destroyed=false,queuedBytes=0,cleaning=false,playbackStarted=false,playbackActive=true;video.src=url;video.playbackRate=1;
  const fail=error=>{if(destroyed)return;onError(error instanceof Error?error:new Error(String(error||'Native screen playback failed')))};
  const drain=()=>{if(destroyed||!buffer||buffer.updating)return;try{if(video.currentTime>8&&buffer.buffered.length&&buffer.buffered.start(0)<video.currentTime-5&&!cleaning){cleaning=true;buffer.remove(0,video.currentTime-4);return}const next=queue.shift();if(!next)return;queuedBytes-=next.byteLength;buffer.appendBuffer(next)}catch(error){fail(error)}};
  source.addEventListener('sourceopen',()=>{if(destroyed)return;try{buffer=source.addSourceBuffer(mime);buffer.addEventListener('error',()=>fail(new Error('Native AV1 SourceBuffer failed')));buffer.addEventListener('updateend',()=>{cleaning=false;if(video.buffered.length){const start=video.buffered.start(0),end=video.buffered.end(video.buffered.length-1),lag=end-video.currentTime;if(!playbackActive){video.pause();video.playbackRate=1;if(end-start>2){cleaning=true;buffer.remove(0,Math.max(0,end-1.5));return}}else{if(!playbackStarted){playbackStarted=true;video.currentTime=Math.max(0,end-.08)}else if(lag>1.5)video.currentTime=Math.max(0,end-.18);video.playbackRate=lag>.35&&lag<=1.5?1.03:1;video.play().catch(()=>{})}}drain()});drain()}catch(error){fail(error)}},{once:true});
  return{append(data){if(destroyed)return false;const bytes=data instanceof Uint8Array?data:new Uint8Array(data);if(!bytes.byteLength)return true;const copy=bytes.slice().buffer;if(queuedBytes+copy.byteLength>24*1024*1024){fail(new Error('Native screen decoder fell behind'));return false}queue.push(copy);queuedBytes+=copy.byteLength;drain();return true},setActive(active){playbackActive=!!active;if(destroyed)return;if(!playbackActive){video.pause();video.playbackRate=1;return}if(video.buffered.length){const end=video.buffered.end(video.buffered.length-1);video.currentTime=Math.max(0,end-.08);playbackStarted=true;video.play().catch(()=>{})}},destroy(){if(destroyed)return;destroyed=true;queue.length=0;queuedBytes=0;try{buffer?.abort()}catch{}try{video.pause();video.removeAttribute('src');video.load()}catch{}URL.revokeObjectURL(url)}};
}
function createNativeScreenPlayer(video,codec,onError=()=>{},options={}){return createWebCodecsNativeScreenPlayer(video,codec,onError,options)||createMseNativeScreenPlayer(video,codec,onError)}
function nativeScreenSegmentInfo(data,fps=60){const bytes=data instanceof Uint8Array?data:new Uint8Array(data),cluster=bytes.byteLength>=4&&bytes[0]===0x1f&&bytes[1]===0x43&&bytes[2]===0xb6&&bytes[3]===0x75;if(!cluster)return{kind:'init',key:false};let key=false;try{key=!!window.KnotNativeVideo?.webmAv1Frames?.(bytes,fps).some(frame=>frame.type==='key')}catch{}return{kind:'cluster',key}}
function nativeScreenReceiveState(player,meta={},onGap=()=>{}){return{fragments:new Map(),complete:new Map(),nextSeq:0,pendingBytes:0,player,fps:Number(meta.fps)||60,haveInit:false,latestInit:null,gapSince:0,gapTimer:null,onGap}}
...
function ensureNativeRemoteAudio(){if(nativeRemoteAudio)return nativeRemoteAudio;nativeRemoteAudio=document.createElement('audio');nativeRemoteAudio.autoplay=true;nativeRemoteAudio.hidden=true;document.body.append(nativeRemoteAudio);return nativeRemoteAudio}
...
function beginRemoteNativeScreen(meta,channel){
  cleanupRemoteNativeScreen({keepChannel:true});remoteNativeScreenChannel=channel;remoteScreenExpected=true;remoteScreenSuppressed=false;remoteScreen.hidden=false;remoteScreen.srcObject=null;const requestFallback=()=>{try{if(channel.readyState==='open')channel.send(JSON.stringify({t:'native-screen-fallback'}))}catch{}};
  try{nativeRemotePlayer=createNativeScreenPlayer(remoteScreen,meta.codec||'AV1',requestFallback,meta)}catch(error){screenStatus.textContent=error.message;requestFallback();return false}channel._nativeReceive=nativeScreenReceiveState(nativeRemotePlayer,meta,requestFallback);try{channel.send(JSON.stringify({t:'native-screen-ready',transportVersion:NATIVE_SCREEN_PROTOCOL}))}catch{}screenStatus.textContent='Friend sharing · '+(meta.codec||'AV1')+' · '+(meta.width||'source')+'×'+(meta.height||'source')+' · '+(meta.fps||60)+'fps';updateScreenLayout();return true;
}
...
function appendNativeReceiveEntry(state,entry){if(entry.kind==='init'){state.latestInit=entry.data;if(state.haveInit)return true;state.haveInit=true}return state.player?.append(entry.data)!==false}
function drainNativeScreenReceive(channel){
  ...
  while(state.complete.has(state.nextSeq)){
    ...
    if(!appendNativeReceiveEntry(state,entry)){state.onGap?.(new Error('Native AV1 decoder rejected a segment'));return}
  }
  ...
  if(state.haveInit){if(typeof state.player?.reset!=='function'||state.player.reset()===false){state.onGap?.(new Error('Native AV1 packet loss could not be recovered'));return}}
  else if(!initEntry&&!state.latestInit){state.onGap?.(new Error('Native AV1 initialization was lost'));return}
...
function initializeNativeScreenSender(channel,meta,sessionId,onFallback=()=>{}){
  clearTimeout(channel._nativeProtocolTimer);channel._nativePeerProtocol=0;channel._nativeSend={sessionId,seq:0,init:null,fps:Number(meta.fps)||60,dropping:false,droppedSegments:0,missedKeys:0,congestedSince:0,fallbackRequested:false,onFallback};channel.bufferedAmountLowThreshold=NATIVE_SCREEN_BUFFER_LOW;
...
async function nativeChannelBackpressure(channel,waitMs=0){if(channel.readyState!=='open')return false;if((Number(channel.bufferedAmount)||0)<NATIVE_SCREEN_BUFFER_HIGH)return true;if(!waitMs||typeof channel.addEventListener!=='function')return false;channel.bufferedAmountLowThreshold=NATIVE_SCREEN_BUFFER_LOW;return new Promise(resolve=>{let done=false;const finish=value=>{if(done)return;done=true;clearTimeout(timer);channel.removeEventListener('bufferedamountlow',low);channel.removeEventListener('close',closed);resolve(value)};const low=()=>finish(channel.readyState==='open'&&(Number(channel.bufferedAmount)||0)<NATIVE_SCREEN_BUFFER_HIGH),closed=()=>finish(false),timer=setTimeout(()=>finish(channel.readyState==='open'&&(Number(channel.bufferedAmount)||0)<NATIVE_SCREEN_BUFFER_HIGH),waitMs);channel.addEventListener('bufferedamountlow',low,{once:true});channel.addEventListener('close',closed,{once:true})})}
async function sendNativeScreenSegment(channel,item){
  const data=item.data instanceof Uint8Array?item.data:new Uint8Array(item.data);if(!data.byteLength||data.byteLength>NATIVE_SCREEN_MAX_SEGMENT||channel.readyState!=='open')return false;const total=Math.max(1,Math.ceil(data.byteLength/NATIVE_SCREEN_PART));if(total>128||(!item.admitted&&!await nativeChannelBackpressure(channel,Number(item.waitMs)||0)))return false;try{for(let part=0;part<total;part++){const start=part*NATIVE_SCREEN_PART,end=Math.min(data.byteLength,start+NATIVE_SCREEN_PART),packet=new Uint8Array(12+end-start),view=new DataView(packet.buffer);view.setUint32(0,NATIVE_SCREEN_PACKET);view.setUint32(4,item.seq);view.setUint16(8,part);view.setUint16(10,total);packet.set(data.subarray(start,end),12);channel.send(packet.buffer)}return true}catch{return false}
}
...
function markNativeScreenCongested(channel,state,key){state.dropping=true;state.droppedSegments++;if(!state.congestedSince)state.congestedSince=performance.now();if(key)state.missedKeys++;const legacy=channel._nativePeerProtocol<NATIVE_SCREEN_PROTOCOL;if(legacy||state.missedKeys>=4||performance.now()-state.congestedSince>=5000)requestNativeScreenFallback(channel,state,new Error(legacy?'AV1 congestion requires a compatible receiver':'AV1 connection remained congested'))}
async function sendNativeScreenLiveItem(channel,item){
  ...
  if(item.kind==='init'){state.init=data.slice();if(!await nativeChannelBackpressure(channel,120)){markNativeScreenCongested(channel,state,false);return true}const sent=await sendNativeScreenSegment(channel,{kind:'init',seq:state.seq++,data,admitted:true});if(!sent)markNativeScreenCongested(channel,state,false);return channel.readyState==='open'}
  const key=item.key===true||nativeScreenSegmentInfo(data,state.fps).key;if(state.dropping&&!key){markNativeScreenCongested(channel,state,false);return true}if(!await nativeChannelBackpressure(channel,key?120:0)){markNativeScreenCongested(channel,state,key);return true}
  // Every recovery boundary carries a fresh init segment. Receivers skip the
  // duplicate during normal playback but can immediately reset at this keyframe
  // after SCTP abandons a stale fragment.
  if(key&&state.init){if(!await sendNativeScreenSegment(channel,{kind:'init',seq:state.seq++,data:state.init,admitted:true})){markNativeScreenCongested(channel,state,true);return channel.readyState==='open'}}
  if(!await sendNativeScreenSegment(channel,{kind:'cluster',seq:state.seq++,data,admitted:true})){markNativeScreenCongested(channel,state,key);return channel.readyState==='open'}
  if(key){state.dropping=false;state.missedKeys=0;state.congestedSince=0}return true
}
function selectedNativeDimensions(){const dimensions={720:[1280,720],1080:[1920,1080],1440:[2560,1440],2160:[3840,2160]};return dimensions[Number(shareResolution)]||[3840,2160]}
function targetNativeAv1BitrateKbps(width,height,fps){const pixels=Math.max(1,(Number(width)||1920)*(Number(height)||1080)),cadence=Number(fps)===30?.65:1,targetMbps=Math.max(8,6*(pixels/(1920*1080))*cadence);return Math.round(Math.min(screenBitrateMbps,targetMbps)*1000)}
async function attachNativeShareAudio(gen){
  if(!screenAudioOn||!screenActive||gen!==screenGen)return;const track=await linuxShareAudioTrack();if(!track||!screenActive||gen!==screenGen){try{track?.stop()}catch{};return}nativeScreenAudioStream=new MediaStream([track]);try{track.contentHint='music'}catch{};let sender=null;try{sender=pc.addTrack(track,nativeScreenAudioStream);screenSenders.push(sender);const parameters=sender.getParameters();if(!parameters.encodings?.length)parameters.encodings=[{}];parameters.encodings[0].maxBitrate=192000;parameters.encodings[0].priority='medium';parameters.encodings[0].networkPriority='medium';await sender.setParameters(parameters);if(!await renegotiate())throw new Error('screen audio negotiation did not start');if(!screenActive||gen!==screenGen)throw new Error('screen share ended during audio negotiation');screenAudioDebug=' · sound live';screenStatus.textContent='Sharing · '+(nativeScreenSession?.encoder||'GPU')+' AV1'+screenAudioDebug}catch(error){console.warn('[AUDIO] native share audio failed:',error?.message||error);if(sender){try{pc?.removeTrack(sender)}catch{}screenSenders=screenSenders.filter(value=>value!==sender);renegotiate().catch(()=>{})}try{track.stop()}catch{}nativeScreenAudioStream=null;window.pairEnv.stopLinuxShareAudio?.();if(screenActive&&gen===screenGen){screenAudioDebug=' · sound unavailable';screenStatus.textContent='Sharing · '+(nativeScreenSession?.encoder||'GPU')+' AV1'+screenAudioDebug}}
}
async function pumpNativeScreen(gen,session,channel){
  ...
  if(screenActive&&gen===screenGen)await stopScreenShare();
}
async function startNativeScreenShare(){
  screenStarting=true;const gen=++screenGen;try{const channel=pc.createDataChannel('knot-screen-native',{ordered:true,maxPacketLifeTime:250,priority:'low'});wireNativeScreenChannel(channel);if(!await waitNativeScreenChannel(channel))throw new Error('Native screen channel did not open');const [width,height]=selectedNativeDimensions(),fps=shareFrameRate===30?30:60,session=await window.pairNativeScreen.start({codec:'av1',fps,width,height,bitrateKbps:targetNativeAv1BitrateKbps(width,height,fps),cursor:screenCursor});if(!session||session.error)throw new Error(session?.error||'GPU AV1 capture did not start');nativeScreenSession=session;nativeScreenAnnounced=false;screenActive=true;screenSenders=[];screenAudioDebug=screenAudioOn?' · starting sound capture':' · sound off';screenPreview.hidden=false;screenPreview.muted=true;nativeLocalPlayer=createNativeScreenPlayer(screenPreview,'AV1',()=>{}, {...session,decode:false});initializeNativeScreenSender(channel,{t:'native-screen-meta',codec:'AV1',fps:session.fps,width:session.width,height:session.height,encoder:session.encoder,latencyTargetMs:session.latencyTargetMs},session.id,()=>fallbackNativeScreenToWebRtc());screenBtn.textContent='Stop sharing';screenBtn.title='Stop screen sharing';screenStatus.textContent='Choose a display · starting '+(session.encoder||'GPU')+' AV1…';focusedScreen='local';screenExpanded=false;updateScreenLayout();void pumpNativeScreen(gen,session,channel);return true}catch(error){console.warn('[VIDEO] native screen start failed:',error?.message||error);screenStatus.textContent='Native AV1 unavailable: '+(error?.message||error);if(nativeScreenSession)window.pairNativeScreen?.stop(nativeScreenSession.id);nativeScreenSession=null;try{nativeScreenChannel?.close()}catch{}nativeScreenChannel=null;nativeLocalPlayer?.destroy();nativeLocalPlayer=null;screenPreview.hidden=true;return false}finally{if(gen===screenGen)screenStarting=false}}
async function fallbackNativeScreenToWebRtc(){if(!nativeScreenSession||!screenActive)return;screenStatus.textContent='AV1 playback unavailable · switching to H.264';const previous=screenCodec;nativeScreenBypassOnce=true;await stopScreenShare();screenCodec='H264';try{await startScreenShare()}finally{screenCodec=previous}}
function recoverFromGpuProcessLoss(details={}){
  ...
  if(requested)screenStatus.textContent='GPU video process restarted ('+reason+') · switching the affected AV1 viewer to H.264'
}
window.pairEnv?.onGpuProcessGone?.(recoverFromGpuProcessLoss);
async function startScreenShare(){
  if(screenActive||screenStarting||!pc)return;
  if(!nativeScreenBypassOnce&&window.pairNativeScreen&&window.pairEnv?.platform==='linux'&&['0x10de','0x1002'].includes(window.pairEnv.primaryGpuVendor)&&(screenCodec==='auto'||screenCodec==='AV1')){const info=await window.pairNativeScreen.info();if(info?.supported&&await startNativeScreenShare())return}
  nativeScreenBypassOnce=false;
  ...
  try{
    if(window.pairEnv?.getSources&&!window.pairEnv.useSystemPicker){
      screenStatus.textContent='Choose a screen or window…';const sources=await window.pairEnv.getSources();
      ...
      const id=window.pairEnv.platform==='linux'&&sources.length===1?sources[0].id:await new Promise(resolve=>{
        const modal=document.createElement('div');modal.className='screen-source-modal';const dialog=document.createElement('div');dialog.className='screen-source-dialog';
        dialog.innerHTML='<h3>Select what to share</h3><div class="share-start-options"><label>Resolution<select id="shareResolution"><option value="source">Source</option><option value="2160">4K</option><option value="1440">1440p</option><option value="1080">1080p</option><option value="720">720p</option></select></label><label>Frame rate<select id="shareFrameRate"><option value="60">60 fps</option><option value="30">30 fps</option></select></label><label class="share-audio-option"><input id="shareSystemAudio" type="checkbox" checked /> Share computer sound<span class="share-audio-hint">Desktop apps and games — Knot voice is excluded</span></label></div>';
        const resolution=dialog.querySelector('#shareResolution'),frameRate=dialog.querySelector('#shareFrameRate'),audio=dialog.querySelector('#shareSystemAudio');resolution.value=String(shareResolution);frameRate.value=String(shareFrameRate);audio.checked=screenAudioOn;const grid=document.createElement('div');grid.className='screen-source-grid';
        for(const source of sources){const button=document.createElement('button');button.type='button';button.className='screen-source-option';const image=document.createElement('img');image.src=source.thumbnail;image.alt='';const name=document.createElement('span');name.textContent=source.name;button.append(image,name);button.onclick=()=>{shareResolution=['source','720','1080','1440','2160'].includes(resolution.value)?resolution.value:'1080';shareFrameRate=Number(frameRate.value)===30?30:60;screenAudioOn=audio.checked;syncScreenPreset();ssSet('shareResolution',shareResolution);ssSet('shareFrameRate',String(shareFrameRate));modal.remove();resolve(source.id)};grid.append(button)}
        const cancel=document.createElement('button');cancel.type='button';cancel.className='screen-source-cancel';cancel.textContent='Cancel';cancel.onclick=()=>{modal.remove();resolve(null)};dialog.append(grid,cancel);modal.append(dialog);document.body.append(modal);
      });
      if(!id||gen!==screenGen)return;await window.pairEnv.setPendingSource(id);
    }
    ...
    // A brief local consumer keeps PipeWire alive during graph startup. Pause it
    // immediately afterward unless the sharer opens their own preview, avoiding
    // a second full-resolution compositor workload in Knot's renderer.
    screenPreview.srcObject=stream;screenPreview.hidden=false;try{await screenPreview.play()}catch{};setTimeout(()=>{try{syncLocalScreenPreview()}catch{}},250);
...
async function toggleRemoteFs(){if(screenStage.classList.contains('fs')||document.fullscreenElement===screenStage){exitShareFullscreen();return}if(!screenExpanded)return;try{if(!screenStage.requestFullscreen)throw new Error('element fullscreen unavailable');await screenStage.requestFullscreen()}catch{screenStage.classList.add('fs');document.body.classList.add('screen-fullscreen')}updateScreenLayout()}
