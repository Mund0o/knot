...
function createKnotAccount(usernameElement,passwordElement,status){const input=accountCredentials(usernameElement,passwordElement,status);if(!input)return;input.status.textContent='Creating account…';if(!directorySend({type:'create-account',username:input.username,password:input.password}))input.status.textContent='Knot is offline. Reconnect and try again.'}
async function signInKnotAccount(usernameElement,passwordElement,status){const input=accountCredentials(usernameElement,passwordElement,status);if(!input)return;input.status.textContent='Signing in…';const socket=new WebSocket(directoryAddress()),timer=setTimeout(()=>{try{socket.close()}catch{};input.status.textContent='Sign-in timed out. Try again.'},20000);socket.onopen=()=>socket.send(JSON.stringify({type:'account-login',username:input.username,password:input.password}));socket.onmessage=async event=>{let value;try{value=JSON.parse(event.data)}catch{return}if(value.type==='error'){clearTimeout(timer);input.status.textContent=value.message||'Sign-in failed.';socket.close();return}if(value.type!=='account-session'||value.mode!=='login')return;clearTimeout(timer);const changed=value.userId!==directoryUserId;await Promise.all([ssSet('directoryUserId',value.userId),ssSet('directoryToken',value.token),ssSet('directoryAccountName',value.username),ssSet('accountOnboardingDismissed','yes')]);directoryUserId=value.userId;directoryToken=value.token;directoryAccountName=value.username;if(changed){conversationHistories={};await ssSet('messageHistory','{}');closedDmIds.clear();await ssSet('closedDmIds','[]')}input.status.textContent='Signed in as @'+value.username+'. Reconnecting…';renderAccountSummary();$('#accountDialog')?.close();socket.close();try{directorySocket?.close()}catch{};directorySocket=null;connectDirectory()};socket.onerror=()=>{clearTimeout(timer);input.status.textContent='Could not reach Knot to sign in.'}}
...
async function connectDirectory(){
  ...
  socket.onmessage=event=>{try{
    const value=JSON.parse(event.data);
    if(value.type==='authenticated'){directoryAccountName=value.username||directoryAccountName;if(directoryAccountName)ssSet('directoryAccountName',directoryAccountName);renderAccountSummary();setDirectoryState(true,'Online');directoryProfilePush();if(callActive)publishCallState(true);void maybeShowAccountOnboarding()}
