...
function addMessage(text,mine=false,gif=null,author=null){
  ...
  avatar.className='avatar message-avatar';const authorImage=!mine&&validProfileData(author?.image)?author.image:'';avatar.classList.toggle('has-image',source?source.classList.contains('has-image'):!!authorImage);avatar.style.backgroundImage=source?source.style.backgroundImage:(authorImage?'url("'+authorImage.replace(/"/g,'%22')+'")':'');avatar.style.backgroundSize=source?source.style.backgroundSize:'cover';avatar.style.backgroundPositionX=source?source.style.backgroundPositionX:'50%';avatar.style.backgroundPositionY=source?source.style.backgroundPositionY:'50%';avatar.style.setProperty('--avatar-hue',source?source.style.getPropertyValue('--avatar-hue'):String(avatarHue(author?.id||name)));const letter=document.createElement('span');letter.className='avatar-letter';letter.textContent=name.slice(0,1).toUpperCase()||'?';avatar.append(letter);
  ...
  el.append(avatar,content);messages.append(el);messages.scrollTop=messages.scrollHeight;recordConversationMessage({text,mine,gif:gif?.url?{url:gif.url,thumb:gif.thumb||gif.url}:null,author:mine?null:{id:author?.id||'',name,image:''},time:messageTime});
}
// --- Emoji Picker ------------------------------------------------------------
const EMOJI_CATS=[
...
function directoryProfile(){return {name:profileName,image:profileSharing&&profileAvatar.length<=190*1024?profileAvatar:''}}
directoryProfilePush=()=>{const profile=directoryProfile();directorySend({type:'update-profile',...profile})};
...
function renderFriends(){const list=$('#friendList');if(!list)return;list.replaceChildren();for(const friend of directorySnapshot.friends||[]){const button=document.createElement('button');button.type='button';button.className='friend-entry'+(friend.id===activePeerId&&!activeServerId?' active':'');button.dataset.id=friend.id;const avatar=document.createElement('span');avatar.className='friend-avatar';avatar.style.setProperty('--avatar-hue',String(avatarHue(friend.id)));if(validProfileData(friend.image)){avatar.style.backgroundImage='url("'+friend.image.replace(/"/g,'%22')+'")';avatar.textContent=''}else avatar.textContent=(friend.name||'?').slice(0,1).toUpperCase();const dot=document.createElement('i');dot.classList.toggle('online',!!friend.online);avatar.append(dot);const name=document.createElement('span');name.textContent=friend.name||'Pair user';button.append(avatar,name);button.onclick=()=>selectFriend(friend.id);list.append(button)}if(!list.children.length){const empty=document.createElement('p');empty.className='social-empty';empty.textContent='Create a five-digit friend code to add someone.';list.append(empty)}}
