...
let socialSidebarWidth=238,pendingServerSelection=false;
...
function setSocialSidebarWidth(value,persist=true){socialSidebarWidth=Math.max(190,Math.min(sidebarWidthLimit(),Number(value)||238));$('.app-shell')?.style.setProperty('--social-sidebar-width',socialSidebarWidth+'px');if(persist)ssSet('socialSidebarWidth',String(Math.round(socialSidebarWidth)))}
async function installSidebarLayout(){const toggle=$('#sidebarToggle'),handle=$('#sidebarResize');setSocialSidebarWidth(Number(await ss('socialSidebarWidth'))||238,false);setSocialSidebarCollapsed((await ss('socialSidebarCollapsed'))==='on',false);toggle.onclick=()=>setSocialSidebarCollapsed(!document.body.classList.contains('social-sidebar-collapsed'));let startX=0,startWidth=0;handle.addEventListener('pointerdown',event=>{if(document.body.classList.contains('social-sidebar-collapsed'))return;startX=event.clientX;startWidth=socialSidebarWidth;handle.setPointerCapture(event.pointerId);document.body.classList.add('sidebar-resizing')});handle.addEventListener('pointermove',event=>{if(!handle.hasPointerCapture(event.pointerId))return;setSocialSidebarWidth(startWidth+event.clientX-startX,false)});const finish=event=>{if(handle.hasPointerCapture(event.pointerId))handle.releasePointerCapture(event.pointerId);if(document.body.classList.contains('sidebar-resizing')){document.body.classList.remove('sidebar-resizing');setSocialSidebarWidth(socialSidebarWidth)}};handle.addEventListener('pointerup',finish);handle.addEventListener('pointercancel',finish);handle.addEventListener('dblclick',()=>setSocialSidebarWidth(238));handle.addEventListener('keydown',event=>{if(!['ArrowLeft','ArrowRight','Home'].includes(event.key))return;event.preventDefault();setSocialSidebarWidth(event.key==='Home'?238:socialSidebarWidth+(event.key==='ArrowLeft'?-16:16))});window.addEventListener('resize',()=>setSocialSidebarWidth(socialSidebarWidth,false))}
