...
let conversationHistories={},historySaveTimer=null,serverVoiceStream=null,serverScreenStream=null,serverNativeScreenSession=null,serverNativeLocalPlayer=null,serverNativeScreenAudioStream=null,serverNativeScreenInit=null,serverNativeScreenBypassOnce=false,serverNativeFallbackInFlight=false,serverVoiceMuted=false,serverScreenStarting=false,serverScreenGen=0,joinedVoiceServerId='',joinedVoiceChannelId='',joinedVoiceAt=0,voiceElapsedTimer=null,draggedChannelId='';const serverPeers=new Map();
...
let screenTransceiver=null,screenActive=false,screenStarting=false,screenStream=null,screenGen=0,screenSenders=[],screenStatsTimer=null,screenStatsLast=null,screenRecoveryLevel=0,screenBadSamples=0,screenGoodSamples=0,screenFallbackBitrateCapMbps=0,remoteScreenExpected=false,remoteScreenSuppressed=false,screenAudioDebug='';
let nativeScreenSession=null,nativeScreenChannel=null,remoteNativeScreenChannel=null,nativeLocalPlayer=null,nativeRemotePlayer=null,nativeRemoteAudio=null,nativeScreenBypassOnce=false,nativeScreenFallbackInFlight=false,nativeScreenAnnounced=false,nativeScreenAudioStream=null;
...
async function fallbackServerNativeToWebRtc(){if(serverNativeFallbackInFlight||!serverNativeScreenSession)return;serverNativeFallbackInFlight=true;const previous=screenCodec;try{serverNativeScreenBypassOnce=true;await stopServerScreenShare();screenFallbackBitrateCapMbps=12;screenCodec='H264';await startServerScreenShare()}finally{screenCodec=previous;serverNativeFallbackInFlight=false}}
...
async function startServerScreenShare(){
  ...
  if(!serverNativeScreenBypassOnce)screenFallbackBitrateCapMbps=0;
...
function targetScreenBitrate(width,height,fps){
  ...
  return Math.round(Math.min(screenBitrateMbps,compatibilityCap,Math.max(4,base))*1000000);
}
...
const NATIVE_SCREEN_PACKET=0x4b4e5331,NATIVE_SCREEN_PART=60*1024,NATIVE_SCREEN_BUFFER_HIGH=256*1024,NATIVE_SCREEN_BUFFER_LOW=64*1024,NATIVE_SCREEN_MAX_SEGMENT=4*1024*1024,NATIVE_SCREEN_GAP_WAIT=80,NATIVE_SCREEN_PROTOCOL=2;
...
function createWebCodecsNativeScreenPlayer(video,codec,onError=()=>{},options={}){
  ...
  let canvas=null,context=null,trackGenerator=null,frameWriter=null,pendingFrame=null,presentationBusy=false,presentationGeneration=0,presentationMode='canvas',presentationDroppedFrames=0,decoder=null,config=null,configured=false,destroyed=false,decoderDisabled=false,failureReported=false,latencyExceeded=false,latencyViolationWindows=0,playbackActive=true,decodedFrames=0,paintedFrames=0,firstPaintAt=0,frameWidth=0,frameHeight=0,softwareFallback=false,hardwareUnavailable=false,replay=[],queuedSinceOutput=0,configuredAt=0,lastOutputAt=0,needsKeyframe=true;const arrivalTimes=new Map(),latencySamples=[];
  const closeDecoderSoon=value=>{if(!value)return;queueMicrotask(()=>{try{value.close()}catch{}})};
  const closePendingFrame=()=>{if(!pendingFrame)return;arrivalTimes.delete(pendingFrame.timestamp);try{pendingFrame.close()}catch{}pendingFrame=null};
  const fail=error=>{if(destroyed||failureReported)return;failureReported=true;decoderDisabled=true;closePendingFrame();const failed=decoder;decoder=null;closeDecoderSoon(failed);const reason=error instanceof Error?error:new Error(String(error||'Native AV1 low-latency decode failed'));queueMicrotask(()=>{if(!destroyed)onError(reason)})};
  ...
  const presentGeneratedFrame=frame=>{
    if(destroyed||decoderDisabled||!playbackActive||!frameWriter){arrivalTimes.delete(frame.timestamp);try{frame.close()}catch{};return}
    const timestamp=frame.timestamp,generation=presentationGeneration;presentationBusy=true;let write;
    try{write=frameWriter.write(frame)}catch(error){try{frame.close()}catch{};arrivalTimes.delete(timestamp);presentationBusy=false;fail(error);return}
    Promise.resolve(write).then(()=>{if(!destroyed&&generation===presentationGeneration&&playbackActive)recordPresentation(timestamp,performance.now());else arrivalTimes.delete(timestamp)}).catch(error=>{try{frame.close()}catch{};arrivalTimes.delete(timestamp);if(!destroyed)fail(error)}).finally(()=>{presentationBusy=false;if(destroyed||decoderDisabled){closePendingFrame();return}const next=pendingFrame;pendingFrame=null;if(next)presentGeneratedFrame(next)});
  };
  const output=frame=>{
    ...
    if(playbackActive){
      if(frameWriter){if(presentationBusy){if(pendingFrame){arrivalTimes.delete(pendingFrame.timestamp);try{pendingFrame.close()}catch{};presentationDroppedFrames++}pendingFrame=frame}else presentGeneratedFrame(frame);return}
      if(canvas.width!==frameWidth||canvas.height!==frameHeight){canvas.width=frameWidth;canvas.height=frameHeight}
      try{context.drawImage(frame,0,0,canvas.width,canvas.height);recordPresentation(frame.timestamp,performance.now())}catch(error){fail(error)}
    }
  ...
  const startSoftwareDecoder=error=>{
    ...
    const previous=decoder;let next=null;softwareFallback=true;presentationGeneration++;closePendingFrame();latencySamples.length=0;latencyViolationWindows=0;
  ...
  try{
    ...
    if(typeof MediaStreamTrackGenerator==='function')try{trackGenerator=new MediaStreamTrackGenerator({kind:'video'});frameWriter=trackGenerator.writable.getWriter();video.srcObject=new MediaStream([trackGenerator]);video.autoplay=true;video.playsInline=true;video.style.opacity='';presentationMode='track';video.play().catch(()=>{})}catch{try{frameWriter?.abort()}catch{}try{trackGenerator?.stop()}catch{}frameWriter=null;trackGenerator=null}
    ...
    append(data){
      ...
      try{
        if(bytes.length>=4&&bytes[0]===0x1f&&bytes[1]===0x43&&bytes[2]===0xb6&&bytes[3]===0x75){
          ...
          config={codec:parser.av1Codec(description),hardwareAcceleration:'prefer-hardware',optimizeForLatency:true};if(configuredWidth&&configuredHeight){config.codedWidth=configuredWidth;config.codedHeight=configuredHeight;config.displayAspectWidth=configuredWidth;config.displayAspectHeight=configuredHeight}try{decoder.configure(config)}catch(error){if(!startSoftwareDecoder(error))throw error}configured=true;needsKeyframe=true;configuredAt=performance.now();lastOutputAt=configuredAt;
        }
        return true;
      }catch(error){fail(error);return false}
    },
    reset(){if(destroyed||decoderDisabled||!decoder)return false;presentationGeneration++;closePendingFrame();replay=[];arrivalTimes.clear();latencySamples.length=0;latencyViolationWindows=0;queuedSinceOutput=0;needsKeyframe=true;try{decoder.reset();if(config)decoder.configure({...config,hardwareAcceleration:softwareFallback?'prefer-software':'prefer-hardware'});configured=!!config;configuredAt=performance.now();lastOutputAt=configuredAt;return true}catch(error){fail(error);return false}},
    setActive(active){playbackActive=!!active;if(canvas)canvas.hidden=!playbackActive;try{if(trackGenerator)trackGenerator.enabled=playbackActive}catch{}if(!playbackActive){presentationGeneration++;closePendingFrame();try{video.pause()}catch{}}else if(frameWriter)video.play().catch(()=>{})},
    destroy(){if(destroyed)return;destroyed=true;presentationGeneration++;clearInterval(stallTimer);closePendingFrame();try{decoder?.close()}catch{}try{const aborted=frameWriter?.abort(new Error('AV1 player closed'));aborted?.catch?.(()=>{})}catch{}try{trackGenerator?.stop()}catch{}canvas?.remove();video.style.opacity='';try{video.pause();video.srcObject=null;video.removeAttribute('src');video.load()}catch{}},
    stats(){const steady=latencySamples.slice(-120).sort((a,b)=>a-b),p95=steady.length?steady[Math.min(steady.length-1,Math.ceil(steady.length*.95)-1)]:0;return{decodedFrames,paintedFrames,firstPaintAt,width:frameWidth,height:frameHeight,decodeQueueSize:decoderDisabled?0:decoder?.decodeQueueSize||0,softwareFallback,hardwareUnavailable,latencyExceeded,latencyViolationWindows,steadyStateP95Ms:p95,latencySamples:steady.length,presentationMode,presentationDroppedFrames}}
  };
}
...
function nativeScreenReceiveState(player,meta={},onGap=()=>{}){return{fragments:new Map(),complete:new Map(),nextSeq:0,pendingBytes:0,player,fps:Number(meta.fps)||60,haveInit:false,latestInit:null,gapSince:0,gapTimer:null,fallbackRequested:false,onGap}}
...
function drainNativeScreenReceive(channel){
  ...
  if(!state.haveInit){const init=initEntry?.data||state.latestInit;state.haveInit=true;state.latestInit=init;if(state.player?.append(init)===false){requestNativeReceiveFallback(state,new Error('Native AV1 initialization failed'));return}}
  if(state.player?.append(keyEntry.data)===false){requestNativeReceiveFallback(state,new Error('Native AV1 keyframe recovery failed'));return}
...
async function sendNativeScreenLiveItem(channel,item){
  ...
  if(item.kind==='init'){state.init=data.slice();if(!await nativeChannelBackpressure(channel,100)){state.seq++;markNativeScreenCongested(channel,state,false);return true}const sent=await sendNativeScreenSegment(channel,{kind:'init',seq:state.seq++,data,admitted:true,waitMs:100});if(!sent)markNativeScreenCongested(channel,state,false);return channel.readyState==='open'}
  const key=item.key===true||nativeScreenSegmentInfo(data,state.fps).key;if(state.dropping&&!key){state.seq++;markNativeScreenCongested(channel,state,false);return true}if(!await nativeChannelBackpressure(channel,key?100:0)){state.seq++;markNativeScreenCongested(channel,state,key);return true}
  ...
  if(key&&state.init){if(!await sendNativeScreenSegment(channel,{kind:'init',seq:state.seq++,data:state.init,admitted:true,waitMs:100})){markNativeScreenCongested(channel,state,true);return channel.readyState==='open'}}
  if(!await sendNativeScreenSegment(channel,{kind:'cluster',seq:state.seq++,data,admitted:true,waitMs:key?100:0})){markNativeScreenCongested(channel,state,key);return channel.readyState==='open'}
...
function targetNativeAv1BitrateKbps(width,height,fps){const pixels=Math.max(1,(Number(width)||1920)*(Number(height)||1080)),cadence=Number(fps)===30?.65:1,targetMbps=Math.max(8,2.5*(pixels/(1920*1080))*cadence);return Math.round(Math.min(screenBitrateMbps,targetMbps)*1000)}
...
async function startScreenShare(){
  ...
  if(!nativeScreenBypassOnce)screenFallbackBitrateCapMbps=0;
