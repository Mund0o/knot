...
let conversationHistories={},historySaveTimer=null,serverVoiceStream=null,joinedVoiceChannelId='',joinedVoiceAt=0,voiceElapsedTimer=null,draggedChannelId='';const serverPeers=new Map();
...
function refreshVoiceElapsed(){document.querySelectorAll('[data-voice-joined]').forEach(node=>{node.textContent=voiceElapsed(node.dataset.voiceJoined)})}
function scheduleVoiceElapsed(){clearInterval(voiceElapsedTimer);if(document.querySelector('[data-voice-joined]')){refreshVoiceElapsed();voiceElapsedTimer=setInterval(refreshVoiceElapsed,1000)}}
function renderVoiceParticipants(channelId,container){for(const entry of voiceChannelEntries(channelId)){const member=directorySnapshot.members?.[entry.id]||(entry.id===directoryUserId?directorySnapshot.self:null);if(!member)continue;const row=document.createElement('div');row.className='voice-channel-member';const avatar=document.createElement('span');avatar.className='friend-avatar';paintDirectoryAvatar(avatar,member);const name=document.createElement('span');name.textContent=(member.name||'Pair user')+(entry.id===directoryUserId?' (you)':'');const elapsed=document.createElement('time');elapsed.dataset.voiceJoined=String(entry.joinedAt||Date.now());row.append(avatar,name,elapsed);container.append(row)}}
function moveChannel(sourceId,targetId,after){const server=activeServer(),source=server?.channels.find(channel=>channel.id===sourceId),target=server?.channels.find(channel=>channel.id===targetId);if(!server||!source||!target||source.type!==target.type||source.id===target.id)return;const channels=server.channels.filter(channel=>channel.id!==sourceId),targetIndex=channels.findIndex(channel=>channel.id===targetId);channels.splice(targetIndex+(after?1:0),0,source);server.channels=channels;renderChannels();directorySend({type:'reorder-channels',serverId:server.id,channelIds:channels.map(channel=>channel.id)})}
function createChannelRow(server,channel){const item=document.createElement('div');item.className='channel-item'+(channel.id===activeChannelId?' active':'');item.dataset.id=channel.id;item.dataset.type=channel.type;item.draggable=true;const button=document.createElement('button');button.type='button';button.className='channel-entry '+channel.type;button.textContent=channel.name;button.title=channel.type==='voice'?'Click to select · double-click to join':'Open #'+channel.name;button.onclick=()=>selectServerChannel(server.id,channel.id);if(channel.type==='voice')button.ondblclick=async event=>{event.preventDefault();await selectServerChannel(server.id,channel.id);await joinServerVoice()};const controls=document.createElement('span');controls.className='channel-row-controls';const drag=document.createElement('span');drag.className='channel-drag';drag.textContent='⠇';drag.title='Drag to reorder';const remove=document.createElement('button');remove.type='button';remove.className='channel-remove';remove.textContent='×';remove.title='Delete channel';remove.setAttribute('aria-label','Delete '+channel.name);remove.onclick=event=>{event.stopPropagation();remove.disabled=true;directorySend({type:'delete-channel',serverId:server.id,channelId:channel.id})};controls.append(drag,remove);item.append(button,controls);if(channel.type==='voice'){const participants=document.createElement('div');participants.className='voice-channel-members';renderVoiceParticipants(channel.id,participants);item.append(participants)}item.ondragstart=event=>{draggedChannelId=channel.id;item.classList.add('dragging');event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/plain',channel.id)};item.ondragend=()=>{draggedChannelId='';document.querySelectorAll('.channel-item').forEach(row=>row.classList.remove('dragging','drop-before','drop-after'))};item.ondragover=event=>{const source=server.channels.find(value=>value.id===draggedChannelId);if(!source||source.type!==channel.type||source.id===channel.id)return;event.preventDefault();const after=event.clientY>item.getBoundingClientRect().top+item.offsetHeight/2;item.classList.toggle('drop-before',!after);item.classList.toggle('drop-after',after)};item.ondrop=event=>{event.preventDefault();const after=item.classList.contains('drop-after');moveChannel(draggedChannelId,channel.id,after)};return item}
function renderChannels(){const server=activeServer(),textList=$('#textChannelList'),voiceList=$('#voiceChannelList');if(!server||!textList||!voiceList)return;$('#serverPanelTitle').textContent=server.name;textList.replaceChildren();voiceList.replaceChildren();for(const channel of server.channels||[])(channel.type==='voice'?voiceList:textList).append(createChannelRow(server,channel));renderServerMembers();scheduleVoiceElapsed()}
function renderServerMembers(){const server=activeServer(),list=$('#serverMemberList'),panel=$('#serverMemberPanel');if(!server||!list||!panel)return;const members=(server.members||[]).map(id=>directorySnapshot.members?.[id]||(id===directoryUserId?directorySnapshot.self:null)).filter(Boolean).sort((a,b)=>Number(b.online)-Number(a.online)||(a.name||'').localeCompare(b.name||''));$('#serverMemberCount').textContent=String(members.length);list.replaceChildren();for(const member of members){const button=document.createElement('button');button.type='button';button.className='friend-entry';button.disabled=member.id===directoryUserId;const avatar=document.createElement('span');avatar.className='friend-avatar';paintDirectoryAvatar(avatar,member);const dot=document.createElement('i');dot.classList.toggle('online',!!member.online);avatar.append(dot);const copy=document.createElement('span');copy.className='member-copy';const name=document.createElement('strong');name.textContent=(member.name||'Pair user')+(member.id===directoryUserId?' (you)':'');const status=document.createElement('small');status.textContent=member.online?'Online':'Offline';copy.append(name,status);button.append(avatar,copy);if(member.id!==directoryUserId)button.onclick=()=>selectFriend(member.id);list.append(button)}panel.hidden=false}
...
async function selectServerChannel(serverId,channelId){const server=directorySnapshot.servers?.find(item=>item.id===serverId),channel=server?.channels?.find(item=>item.id===channelId);if(!server||!channel)return;if(joinedVoiceChannelId&&joinedVoiceChannelId!==channelId)stopServerVoice();activeServerId=serverId;activeChannelId=channelId;activePeerId='';roomTitle.textContent=channel.name;$('#chatTitle').textContent=channel.name;$('#roomContextLabel').textContent=server.name.toUpperCase();$('#chatModePill').textContent=channel.type==='voice'?'VOICE':'P2P MESH';messageInput.placeholder='Message #'+channel.name;openConversation('server:'+serverId+':'+channelId);renderChannels();if(channel.type==='voice'&&!serverVoiceStream){setServerStatus('Double-click '+channel.name+' to join voice');callBtn.textContent='Join voice'}else setServerStatus('Connecting to online server members…');syncServerMesh();messageInput.disabled=false;messageForm.querySelector('.send').disabled=false;fileInput.disabled=true}
function serverOnlineMembers(){const server=activeServer();if(!server)return[];const voiceIds=serverVoiceStream?new Set(voiceChannelEntries(joinedVoiceChannelId).map(entry=>entry.id)):null;return (server.members||[]).filter(id=>id!==directoryUserId&&directorySnapshot.members?.[id]?.online&&(!voiceIds||voiceIds.has(id)))}
...
function stopServerVoice(){const serverId=activeServerId,channelId=joinedVoiceChannelId;if(channelId&&serverId)directorySend({type:'voice-state',serverId,channelId,joined:false});if(serverVoiceStream){serverVoiceStream.getTracks().forEach(track=>track.stop());serverVoiceStream=null}joinedVoiceChannelId='';joinedVoiceAt=0;clearInterval(voiceElapsedTimer);closeServerMesh();if(activeServerId){callBtn.textContent=activeChannel()?.type==='voice'?'Join voice':'Start call';callStatus.textContent='Voice off';callStatus.className='call-status';renderChannels()}}
...
function installChannelDialog(){const dialog=$('#channelDialog'),form=$('#channelForm'),input=$('#newChannelName'),kind=$('#channelDialogKind'),status=$('#channelDialogStatus'),submit=form.querySelector('.primary');let channelType='text';const open=type=>{channelType=type==='voice'?'voice':'text';kind.textContent=channelType.toUpperCase()+' CHANNEL';input.placeholder=channelType==='voice'?'New voice':'new-channel';input.value=channelType==='voice'?'New voice':'new-channel';status.textContent='';submit.disabled=false;dialog.showModal();setTimeout(()=>input.select(),0)};$('#addTextChannel').onclick=()=>open('text');$('#addVoiceChannel').onclick=()=>open('voice');$('#closeChannelDialog').onclick=()=>dialog.close();dialog.addEventListener('click',event=>{const box=dialog.getBoundingClientRect();if(event.target===dialog&&(event.clientX<box.left||event.clientX>box.right||event.clientY<box.top||event.clientY>box.bottom))dialog.close()});form.onsubmit=event=>{event.preventDefault();const server=activeServer(),name=cleanClientName(input.value,channelType==='voice'?'New voice':'new-channel');if(!server||!directorySend({type:'create-channel',serverId:server.id,channelType,name})){status.textContent='Pair is offline. Reconnect before creating a channel.';return}pendingChannelCreation={serverId:server.id,type:channelType,beforeIds:new Set(server.channels.map(channel=>channel.id))};status.textContent='Creating '+name+'…';submit.disabled=true}}
