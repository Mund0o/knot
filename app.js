...
function updateDirectorySnapshot(snapshot){
  ...
  directorySnapshot=snapshot;const conversationIds=new Set([...snapshot.friends,...snapshot.groupDms].map(item=>item.id));let closedChanged=false,unreadChanged=false;for(const id of [...closedDmIds])if(!conversationIds.has(id)){closedDmIds.delete(id);closedChanged=true}for(const id of Object.keys(unreadDmCounts))if(!conversationIds.has(id)){delete unreadDmCounts[id];unreadChanged=true}for(const id of [...pendingGroupEnvelopes.keys()])if(!snapshot.groupDms.some(group=>group.id===id))pendingGroupEnvelopes.delete(id);if(closedChanged)persistClosedDms();if(unreadChanged)persistUnreadDms();void serverTextMembershipSync().then(async()=>{const group=groupDm(activeGroupDmId);if(group&&!await serverTextKey(group)){await requestServerTextKey(group)}});renderServers();renderFriends();renderUnreadBadges();syncActiveDmTransport();if(!activePeerId&&!activeServerId&&!activeGroupDmId)showFriendsLanding();
...
async function pumpNativeScreen(gen,session,channel){
  ...
  while(screenActive&&gen===screenGen&&nativeScreenSession?.id===session.id){
    ...
    for(const item of queued){
      ...
      if(!await sendNativeScreenLiveItem(channel,item)){queued.length=0;break}
