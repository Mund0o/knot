...
async function ensureServerPeer(peerId,context={type:'server',serverId:activeServerId,channelId:activeChannelId}){if(serverPeers.has(peerId))return serverPeers.get(peerId);const connection=new RTCPeerConnection({iceServers:ICE_SERVERS}),state={pc:connection,channel:null,candidates:[],audio:null,context};serverPeers.set(peerId,state);connection.onicecandidate=event=>{if(event.candidate)directorySend({type:'signal',peerId,context:state.context,payload:{kind:'candidate',candidate:event.candidate.toJSON()}})};connection.onconnectionstatechange=()=>{if(['failed','closed'].includes(connection.connectionState)){try{state.audio?.remove()}catch{}serverPeers.delete(peerId)}};connection.ondatachannel=event=>{state.channel=event.channel;wireServerChannel(peerId,state.channel,state.context.serverId)};connection.ontrack=event=>{if(event.track.kind!=='audio'||!voicePeerAllowed(peerId))return;const audio=document.createElement('audio');audio.autoplay=true;audio.srcObject=event.streams[0]||new MediaStream([event.track]);audio.hidden=true;document.body.append(audio);state.audio=audio;audio.play().catch(()=>{})};if(voicePeerAllowed(peerId))for(const track of serverVoiceStream.getTracks())connection.addTrack(track,serverVoiceStream);if(directoryUserId<peerId){state.channel=connection.createDataChannel('pair-server-chat');wireServerChannel(peerId,state.channel,state.context.serverId);const offer=await connection.createOffer();await connection.setLocalDescription(offer);directorySend({type:'signal',peerId,context:state.context,payload:{kind:'offer',sdp:connection.localDescription.sdp}})}return state}
