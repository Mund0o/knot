...
const SEND_WINDOW=64*1024*1024;
...
function wire(){
  ...
  if(files){files.binaryType='arraybuffer';files.bufferedAmountLowThreshold=Math.max(4*1024*1024,SEND_WINDOW*.5);files.onmessage=event=>{receiveQueue=receiveQueue.then(()=>onFileFrame(event)).catch(()=>{})};files.onopen=()=>setStatus('Connected directly',true)}
}
