...
function refreshVoiceElapsed(){document.querySelectorAll('[data-voice-joined]').forEach(node=>{node.textContent=voiceElapsed(node.dataset.voiceJoined)});if(joinedVoiceChannelId){const value=voiceElapsed(joinedVoiceAt);$('#serverVoiceDockTime').textContent=value;$('#serverVoiceStageTime').textContent=value}}
...
queueMicrotask(()=>{installDirectoryUI();installChannelDialog()});
