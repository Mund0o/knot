...
function monitorSpeaking(key,stream){
  ...
  try{
    ...
    const source=speakingAudioCtx.createMediaStreamSource(new MediaStream([track])),analyser=speakingAudioCtx.createAnalyser(),samples=new Float32Array(256),monitor={source,analyser,timer:null,speaking:false,noiseFloor:.003,holdUntil:0};
    analyser.fftSize=256;analyser.smoothingTimeConstant=0;source.connect(analyser);
...
function beginServerNativeScreen(peerId,state,meta,channel){
  ...
  const fallback=()=>{try{if(channel.readyState==='open')channel.send(JSON.stringify({t:'native-screen-fallback',serverId:state.context.serverId}))}catch{}};try{state.nativeScreenPlayer=createNativeScreenPlayer(video,meta.codec||'AV1',fallback)}catch(error){fallback();clearServerNativeScreen(state,{keepChannel:true});setServerStatus(error.message);return false}channel._nativeReceive={fragments:new Map(),complete:new Map(),nextSeq:0,player:state.nativeScreenPlayer};renderServerVoiceUI();return true
}
...
async function ensureServerPeer(peerId,context={type:'server',serverId:activeServerId,channelId:activeChannelId}){
  ...
  connection.ontrack=event=>{if(!voicePeerAllowed(peerId))return;const stream=event.streams[0]||new MediaStream([event.track]);if(event.track.kind==='video'){addServerScreenVideo(peerId,state,event.track,stream);return}if(state.nativeScreenAudioExpected&&state.nativeScreenPlayer){stream._knotPeerId=peerId;addServerNativeScreenAudio(state,event.track,stream);return}const screenAudio=stream.getVideoTracks().length>0||!!(state.screenStreamId&&stream.id===state.screenStreamId);if(screenAudio){if(state.screen&&state.screen.srcObject!==stream)try{state.screen.srcObject.addTrack(event.track)}catch{}renderServerShareExperience();return}addServerVoiceAudio(peerId,state,event.track,stream)};
  if(voicePeerAllowed(peerId)){
    for(const track of serverVoiceStream.getTracks())connection.addTrack(track,serverVoiceStream);
...
function announceServerNativeChannel(channel){if(channel.readyState!=='open'||!serverNativeScreenSession)return false;channel._nativeSendSeq=0;channel._nativeInitSent=false;channel.send(JSON.stringify({t:'native-screen-meta',serverId:joinedVoiceServerId,codec:'AV1',fps:serverNativeScreenSession.fps,width:serverNativeScreenSession.width,height:serverNativeScreenSession.height,audio:false}));return true}
...
async function startServerNativeScreenShare(){
  serverScreenStarting=true;const gen=++serverScreenGen;renderServerVoiceUI();try{const [width,height]=selectedNativeDimensions(),session=await window.pairNativeScreen.start({codec:'av1',fps:shareFrameRate===30?30:60,width,height,bitrateKbps:Math.round(screenBitrateMbps*1000),cursor:screenCursor});if(!session||session.error)throw new Error(session?.error||'NVENC capture did not start');serverNativeScreenSession=session;serverNativeScreenInit=null;const preview=$('#serverVoiceScreenPreview');preview.hidden=false;preview.muted=true;serverNativeLocalPlayer=createNativeScreenPlayer(preview,'AV1',error=>setServerStatus('Local AV1 preview failed: '+error.message));await Promise.all([...serverPeers].filter(([peerId])=>voicePeerAllowed(peerId)).map(([peerId,state])=>ensureServerNativeChannel(peerId,state)));if(gen!==serverScreenGen||serverNativeScreenSession?.id!==session.id)return true;setServerStatus('Choose a display · starting NVENC AV1…',true);serverFocusedShareId=directoryUserId;renderServerVoiceUI();void pumpServerNativeScreen(gen,session);return true}catch(error){if(serverNativeScreenSession)window.pairNativeScreen?.stop(serverNativeScreenSession.id);serverNativeScreenSession=null;serverNativeLocalPlayer?.destroy();serverNativeLocalPlayer=null;$('#serverVoiceScreenPreview').hidden=true;setServerStatus('Native AV1 unavailable: '+(error?.message||error));return false}finally{if(gen===serverScreenGen)serverScreenStarting=false;renderServerVoiceUI()}
}
...
async function startServerScreenShare(){
  ...
  if(!serverNativeScreenBypassOnce&&window.pairNativeScreen&&window.pairEnv?.platform==='linux'&&window.pairEnv.primaryGpuVendor==='0x10de'&&(screenCodec==='auto'||screenCodec==='AV1')){const info=await window.pairNativeScreen.info();if(info?.supported&&await startServerNativeScreenShare())return}serverNativeScreenBypassOnce=false;
...
function createNativeScreenPlayer(video,codec,onError=()=>{}){
...
function beginRemoteNativeScreen(meta,channel){
  ...
  try{nativeRemotePlayer=createNativeScreenPlayer(remoteScreen,meta.codec||'AV1',requestFallback)}catch(error){screenStatus.textContent=error.message;requestFallback();return false}channel._nativeReceive={fragments:new Map(),complete:new Map(),nextSeq:0,player:nativeRemotePlayer};screenStatus.textContent='Friend sharing · '+(meta.codec||'AV1')+' · '+(meta.width||'source')+'×'+(meta.height||'source')+' · '+(meta.fps||60)+'fps';updateScreenLayout();return true;
}
...
async function startScreenShare(){
  ...
  if(!nativeScreenBypassOnce&&window.pairNativeScreen&&window.pairEnv?.platform==='linux'&&window.pairEnv.primaryGpuVendor==='0x10de'&&(screenCodec==='auto'||screenCodec==='AV1')){const info=await window.pairNativeScreen.info();if(info?.supported&&await startNativeScreenShare())return}
...
function updateScreenLayout(){
  const hasLocal=!screenPreview.hidden,hasRemote=!remoteScreen.hidden,fullscreen=!!document.fullscreenElement||screenStage.classList.contains('fs');if(!hasRemote&&focusedScreen==='remote')focusedScreen='local';if(!hasLocal&&focusedScreen==='local')focusedScreen='remote';if(!hasLocal&&!hasRemote)screenExpanded=false;if(focusedScreen==='remote'&&remoteScreenSuppressed)screenExpanded=false;
...
function exitShareFullscreen({collapse=false,fromNativeEvent=false}={}){const hadNative=nativeShareFullscreen;nativeShareFullscreen=false;screenStage.classList.remove('fs');document.body.classList.remove('screen-fullscreen');try{if(document.fullscreenElement)document.exitFullscreen().catch(()=>{})}catch{}if(hadNative&&!fromNativeEvent)try{window.pairEnv?.toggleFullscreen?.()}catch{}if(collapse)screenExpanded=false;updateScreenLayout()}
function toggleRemoteFs(){if(screenStage.classList.contains('fs')||document.fullscreenElement||nativeShareFullscreen){exitShareFullscreen();return}if(!screenExpanded)return;screenStage.classList.add('fs');document.body.classList.add('screen-fullscreen');nativeShareFullscreen=!!window.pairEnv?.toggleFullscreen;window.pairEnv?.toggleFullscreen?.();updateScreenLayout()}
...
window.pairEnv?.onFullscreenChange?.(isFullscreen=>{if(isFullscreen){nativeShareFullscreen=true;return}if(nativeShareFullscreen||screenStage.classList.contains('fs')){nativeShareFullscreen=false;screenStage.classList.remove('fs');document.body.classList.remove('screen-fullscreen');updateScreenLayout()}});document.addEventListener('fullscreenchange',()=>{const is=!!document.fullscreenElement;document.body.classList.toggle('screen-fullscreen',is);if(!is)screenStage.classList.remove('fs');updateScreenLayout()});document.addEventListener('keydown',event=>{if(event.key!=='Escape')return;if(!shareContextMenu.hidden){hideShareContextMenu();return}if(document.fullscreenElement||screenStage.classList.contains('fs')||nativeShareFullscreen)exitShareFullscreen();else if(screenExpanded)returnToSharePreview()});
