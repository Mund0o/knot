...
async function ensurePersistentDmPeer(peerId,{initiate=directoryUserId<peerId}={}){
  ...
  connection.onconnectionstatechange=()=>{if(state.closing)return;if(['failed','closed'].includes(connection.connectionState)){if(dmPeerId===peerId&&directFileId)closeTcpLane();closePersistentDmPeer(peerId,{retry:true})}else if(activePeerId===peerId)syncActiveDmTransport()};
  if(initiate){
