...
function monitorRemoteScreenDecode(receiver,track,requestFallback){
  let latencyTargetMs=45;const applyLatencyTarget=value=>{latencyTargetMs=value;try{receiver.playoutDelayHint=value/1000}catch{}try{if('jitterBufferTarget'in receiver)receiver.jitterBufferTarget=value}catch{}};applyLatencyTarget(latencyTargetMs);
  ...
  const sample=async()=>{if(sampleInFlight||finished)return;sampleInFlight=true;try{
    ...
    previousBytes=bytes;previousFrames=frames;previousLost=lost;previousFreezes=freezes;previousJitterDelay=jitterDelay;previousJitterCount=jitterCount;if(playoutMs)recordMetric('screen.playout_ms',playoutMs,{codec:String(codec?.mimeType||'unknown').replace('video/','').toLowerCase()});if(pressure){stableWindows=0;if(latencyTargetMs<80)applyLatencyTarget(80)}else if(decoded>0&&++stableWindows>=3&&latencyTargetMs>45)applyLatencyTarget(45);
...
let screenCursor='always',screenContentHint='motion',screenBitrateMbps=20,screenCodec='auto',shareResolution='source',shareFrameRate=60,screenAudioOn=true;
...
for(const [theme,name,summary,title] of THEME_CONCEPTS){
  ...
  const button=document.createElement('button');button.className='theme-option theme-concept';button.type='button';button.dataset.theme=theme;button.title=title;
...
function normalizeDirectorySnapshotWire(snapshot){
  ...
  return{...snapshot,self:normalizeDirectoryProfile(snapshot.self),friends:(snapshot.friends||[]).map(normalizeDirectoryProfile),servers:(snapshot.servers||[]).map(normalizeDirectoryEntity),groupDms:(snapshot.groupDms||[]).map(normalizeDirectoryEntity),members}
}
...
async function restoreAccountProfile(value,userId,{keepLocalLegacy=false}={}){
  ...
  profileName=normalizeProfileName(value.name,'Knot user');profileAvatar=directoryMediaUrl(value.image);profileFrame=normalizeFrame(value.frame);
...
async function signInKnotAccount(usernameElement,passwordElement,status){
  ...
  socket.onmessage=async event=>{
    ...
    if(changed){conversationHistories={};conversationRenderState=null;conversationLoadGeneration++;closedDmIds.clear();await ssSet('closedDmIds','[]');unreadDmCounts={};await ssSet('unreadDmCounts','{}');directoryRevision=0;directorySnapshot={friends:[],servers:[],groupDms:[],members:{},voiceStates:{}};renderUnreadBadges();renderServers();renderFriends()}
...
function updateDirectorySnapshot(snapshot){
  ...
  snapshot=normalizeDirectorySnapshotWire(snapshot);const revision=Number(snapshot?.revision);if(Number.isSafeInteger(revision)&&revision>=0){if(revision<directoryRevision)return;directoryRevision=revision}
...
const NATIVE_SCREEN_PACKET=0x4b4e5331,NATIVE_SCREEN_PART=60*1024,NATIVE_SCREEN_BUFFER_HIGH=256*1024,NATIVE_SCREEN_BUFFER_LOW=96*1024,NATIVE_SCREEN_MAX_SEGMENT=4*1024*1024,NATIVE_SCREEN_BUFFER_HARD=NATIVE_SCREEN_MAX_SEGMENT+1024*1024,NATIVE_SCREEN_GAP_WAIT=60,NATIVE_SCREEN_PROTOCOL=2,NATIVE_SCREEN_STALE_MS=150,NATIVE_SCREEN_KEY_WAIT_MS=100;
...
function createMseNativeScreenPlayer(video,codec,onError=()=>{},options={}){
  ...
  // Keep the 160 ms emergency boundary above the 150 ms operating goal: a
  // tighter forced cutoff caused visible drops while measured p95 stayed low.
