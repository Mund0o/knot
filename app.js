...
async function busSafeSend(data){if(directFileId&&window.pairDirectFile){try{await window.pairDirectFile.send(directFileId,data);return}catch{directFileId=''}}let retries=0;for(;;){const bus=fileBus();if(!bus)throw new Error('no file channel');
...
function renderTransferSetupStatus(){if(!transferSetupStatus)return;if(relayVoiceMode){transferSetupStatus.textContent='File transfer is unavailable: this call is using the low-bandwidth voice relay.';return}if(fileTransportMode==='webrtc'){transferSetupStatus.textContent='WebRTC-only is selected. No TCP port-forward is needed.';return}if(directFileId){transferSetupStatus.textContent='Fast direct TCP file lane is connected.';return}if(fileBus()&&pc?.connectionState==='connected'){transferSetupStatus.textContent='Direct WebRTC file connection is ready. TCP will be tested when you send a file.';return}transferSetupStatus.textContent='No direct file connection yet. Open the chat with your friend and wait for “Connected directly.”'}
function saveTcpSettings(){const port=validTcpPort(tcpListenPortInput?.value);if(!port){tcpListenPortInput.value=String(tcpFilePort);return}tcpFilePort=port;fileTransportMode=['auto','webrtc','tcp'].includes(fileTransportSetting?.value)?fileTransportSetting.value:'auto';ssSet('tcpListenPort',String(port));ssSet('fileTransport',fileTransportMode);renderTransferSetupStatus()}
