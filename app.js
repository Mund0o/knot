...
function stopWatchingRemoteShare(){
  ...
  try{remoteScreen.pause();remoteScreen.muted=true}catch{}
...
let conversationHistories={},historySaveTimer=null,serverVoiceStream=null,serverScreenStream=null,serverVoiceMuted=false,serverScreenStarting=false,joinedVoiceServerId='',joinedVoiceChannelId='',joinedVoiceAt=0,voiceElapsedTimer=null,draggedChannelId='';const serverPeers=new Map();
...
function serverShareVideo(peerId){if(peerId===directoryUserId)return serverScreenStream?$('#serverVoiceScreenPreview'):null;return serverPeers.get(peerId)?.screen||null}
function watchServerShare(peerId){const video=serverShareVideo(peerId);if(!video)return;serverSuppressedShares.delete(peerId);serverFocusedShareId=peerId;renderServerShareExperience();video.muted=peerId===directoryUserId;video.play().catch(()=>{})}
...
function renderServerShareExperience(){
  ...
  for(const video of screens.querySelectorAll('video')){const peerId=video.id==='serverVoiceScreenPreview'?directoryUserId:video.dataset.peerId||'';video.hidden=!active||video!==active;if(!video.dataset.shareMenu){video.dataset.shareMenu='1';video.addEventListener('contextmenu',event=>showShareContextMenu(event,{label:peerId===directoryUserId?'Your stream':(directoryUser(peerId)?.name||'Stream'),stopWatching:()=>stopWatchingServerShare(peerId)}));video.addEventListener('dblclick',()=>video.requestFullscreen?.().catch(()=>{}))}}
...
screenVol.oninput=()=>{const v=Math.max(0,Math.min(100,Number(screenVol.value)||0))/100;remoteScreen.volume=v;remoteScreen.muted=v===0;ssSet('screenVol',String(v))};enableRangeDrag(screenVol);
...
const screenViewBar=document.createElement('div');screenViewBar.className='screen-view-bar';screenViewBar.innerHTML='<button type="button" data-screen-volume title="Stream volume" aria-label="Stream volume">♫</button><button type="button" data-screen-fullscreen title="Fullscreen" aria-label="Fullscreen">⛶</button>';screenVideos.after(screenViewBar);
...
function watchDmShare(kind){const available=kind==='local'?!screenPreview.hidden:!remoteScreen.hidden;if(!available)return;if(kind==='remote'){remoteScreenSuppressed=false;if(remoteScreen.volume>0)remoteScreen.muted=false}focusedScreen=kind;screenExpanded=true;updateScreenLayout()}
function syncScreenPlayback(){if(!screenPreview.hidden&&screenPreview.srcObject&&screenPreview.readyState>=2){if(screenExpanded&&focusedScreen==='local')screenPreview.play().catch(()=>{});else screenPreview.pause()}if(!remoteScreen.hidden&&remoteScreen.srcObject){if(screenExpanded&&focusedScreen==='remote'&&!remoteScreenSuppressed){if(remoteScreen.volume>0)remoteScreen.muted=false;remoteScreen.play().catch(()=>{})}else{remoteScreen.pause();remoteScreen.muted=true}}}
...
screenViewBar.onclick=event=>{if(event.target.closest('[data-screen-volume]'))showShareContextMenu(event,{label:'Friend’s stream',volume:true,stopWatching:stopWatchingRemoteShare});else if(event.target.closest('[data-screen-fullscreen]'))toggleRemoteFs()};
