...
let screenCursor='always',screenContentHint='motion',screenBitrateMbps=12,screenCodec='auto',shareResolution='1080',shareFrameRate=60;
...
async function configureScreenVideoSender(sender,track,fps,viewers=1){
  ...
  const parameters=sender.getParameters();if(!parameters.encodings?.length)parameters.encodings=[{}];const encoding=parameters.encodings[0],scale=[1,1.5,2][Math.max(0,Math.min(2,screenRecoveryLevel))]||1;encoding.maxBitrate=Math.round(maxBitrate/(scale>1?Math.pow(scale,.45):1));delete encoding.minBitrate;encoding.maxFramerate=requestedFps;encoding.scaleResolutionDownBy=scale;encoding.priority='medium';encoding.networkPriority='low';parameters.degradationPreference='maintain-framerate';await sender.setParameters(parameters);
}
async function recoverOverloadedScreenSender(sender){
  if(!screenActive||screenRecoveryLevel>=2)return;screenRecoveryLevel++;screenBadSamples=0;screenGoodSamples=0;const track=screenStream?.getVideoTracks?.()[0];try{await configureScreenVideoSender(sender,track,shareFrameRate)}catch{}
  const label=screenRecoveryLevel===1?'temporarily scaling to protect 60 fps':'temporarily scaling to 1080p to protect 60 fps';screenStatus.textContent='Sharing · '+label+screenAudioDebug;logCallEvent('Screen load detected — '+label);
}
async function restoreScreenSenderQuality(sender){
  if(!screenActive||screenRecoveryLevel<=0)return;screenRecoveryLevel--;screenGoodSamples=0;screenBadSamples=0;const track=screenStream?.getVideoTracks?.()[0];try{await configureScreenVideoSender(sender,track,shareFrameRate)}catch{}
}
function startScreenStats(sender){
  ...
  const sample=async()=>{try{
    ...
    const overloaded=previous&&(reason==='cpu'||reason==='bandwidth'||encodeMs>1000/Math.max(1,requestedFps)*1.1||(fps>0&&fps<requestedFps*.72)||(encodedDelta===0&&bytes===previous.bytes));screenBadSamples=overloaded?screenBadSamples+1:0;screenGoodSamples=!overloaded&&fps>=requestedFps*.88?screenGoodSamples+1:0;
    if(screenBadSamples>=2&&screenRecoveryLevel<2)await recoverOverloadedScreenSender(sender);else if(screenRecoveryLevel&&screenGoodSamples>=8)await restoreScreenSenderQuality(sender);
    ...
    const status='Sharing'+(w&&h?' · '+w+'×'+h:'')+(fps?' · '+fps+'fps':'')+(mbps!=='…'?' · '+mbps+' Mbps':'')+(codec?.mimeType?' · '+codec.mimeType.replace('video/',''):'')+(encodeMs?' · '+encodeMs.toFixed(1)+'ms encode':'')+network+(screenRecoveryLevel?' · smoothness recovery '+screenRecoveryLevel:'')+screenAudioDebug;screenStatus.textContent=status;screenBtn.title=status;
  }catch{}};
  sample();screenStatsTimer=setInterval(sample,2000);
}
const NATIVE_SCREEN_PACKET=0x4b4e5331,NATIVE_SCREEN_PART=60*1024,NATIVE_SCREEN_BUFFER_HIGH=320*1024,NATIVE_SCREEN_BUFFER_LOW=64*1024,NATIVE_SCREEN_MAX_SEGMENT=4*1024*1024,NATIVE_SCREEN_GAP_WAIT=80,NATIVE_SCREEN_PROTOCOL=2,NATIVE_SCREEN_STALE_MS=300;
...
function createWebCodecsNativeScreenPlayer(video,codec,onError=()=>{},options={}){
  ...
  let canvas=null,context=null,trackGenerator=null,frameWriter=null,presentationQueue=[],presentationTimer=null,presentationClockTimestamp=null,presentationClockAt=0,presentationGeneration=0,presentationMode='canvas',presentationDroppedFrames=0,renderedFrames=0,lastRenderedAt=0,decoder=null,config=null,configured=false,destroyed=false,decoderDisabled=false,failureReported=false,latencyExceeded=false,latencyViolationWindows=0,playbackActive=true,decodedFrames=0,paintedFrames=0,firstPaintAt=0,frameWidth=0,frameHeight=0,softwareFallback=preferSoftware,hardwareUnavailable=false,replay=[],queuedSinceOutput=0,configuredAt=0,lastOutputAt=0,needsKeyframe=true;const arrivalTimes=new Map(),latencySamples=[],renderIntervals=[];
  ...
  const presentFrame=frame=>{
    ...
    if(frameWriter){try{Promise.resolve(frameWriter.write(frame)).then(()=>{if(!destroyed&&generation===presentationGeneration&&playbackActive)recordPresentation(timestamp,performance.now());else arrivalTimes.delete(timestamp)}).catch(error=>{arrivalTimes.delete(timestamp);if(!destroyed)fail(error)})}catch(error){try{frame.close()}catch{}arrivalTimes.delete(timestamp);fail(error)};return}
  ...
  const schedulePresentation=()=>{
    if(presentationTimer||destroyed||decoderDisabled||!playbackActive||!presentationQueue.length)return;
...
function createMseNativeScreenPlayer(video,codec,onError=()=>{},options={}){
  ...
  const configuredWidth=Number(options.width)||0,configuredHeight=Number(options.height)||0,targetLag=.045,startLag=.025;
  let source=null,url='',buffer=null,queue=[],queuedBytes=0,latestInit=null,destroyed=false,failed=false,cleaning=false,playbackStarted=false,playbackActive=true,recoverAtLiveEdge=false,pipelineGeneration=0,renderedFrames=0,firstPaintAt=0,lastRenderedAt=0,powerKnown=false,powerEfficient=false;const renderIntervals=[],latencySamples=[];
  ...
  const liveRange=()=>{try{if(!video.buffered.length)return null;return{start:video.buffered.start(0),end:video.buffered.end(video.buffered.length-1)}}catch{return null}};
  const synchronizePlayback=()=>{
    const range=liveRange();if(!range)return;
    if(!playbackActive){video.pause();video.playbackRate=1;if(range.end-range.start>.6&&!buffer?.updating){cleaning=true;buffer.remove(range.start,Math.max(range.start,range.end-.4))}return}
    if(recoverAtLiveEdge){recoverAtLiveEdge=false;video.currentTime=Math.max(range.start,range.end-targetLag);playbackStarted=true}
    ...
    const lag=range.end-video.currentTime;if(lag>.25||lag<.008)video.currentTime=Math.max(range.start,range.end-targetLag);
    const corrected=range.end-video.currentTime;video.playbackRate=corrected>.09?1.1:corrected>.06?1.04:corrected<.025?.98:1;try{source?.setLiveSeekableRange?.(range.start,range.end)}catch{}video.play().catch(()=>{});
  };
  const drain=()=>{
    ...
    try{
      ...
      const next=queue.shift();if(!next)return;queuedBytes-=next.byteLength;buffer.appendBuffer(next)
  ...
  return{
    ...
    reset(){if(destroyed||failed||!latestInit)return false;queue.length=0;queuedBytes=0;recoverAtLiveEdge=true;return true},
    setActive(active){playbackActive=!!active;if(destroyed)return;if(!playbackActive){video.pause();video.playbackRate=1}else{const range=liveRange();if(range){video.currentTime=Math.max(range.start,range.end-targetLag);playbackStarted=true}video.play().catch(()=>{})}},
...
function createNativeScreenPlayer(video,codec,onError=()=>{},options={}){
  ...
  const replayIntoBackend=()=>{for(const entry of replay){const token=generation;if(backend?.append(entry.data,entry.info)===false&&token===generation){handleBackendFailure(token,new Error('Native AV1 fallback decoder rejected buffered video'));return false}}return !!backend};
...
async function sendNativeScreenLiveItem(channel,item){
  ...
  if(item.kind==='init'){state.init=data.slice();if(!await nativeChannelBackpressure(channel,100)){markNativeScreenCongested(channel,state,false);return true}const seq=state.seq,sent=await sendNativeScreenSegment(channel,{kind:'init',seq,data,admitted:true});state.seq++;if(!sent)markNativeScreenCongested(channel,state,false);return channel.readyState==='open'}
  ...
  if(!await nativeChannelBackpressure(channel,key?120:0)){markNativeScreenCongested(channel,state,key,frameCount);return true}
...
function targetNativeAv1BitrateKbps(width,height,fps,viewers=1){const pixels=Math.max(1,(Number(width)||1920)*(Number(height)||1080)),cadence=Number(fps)===30?.62:1,ratio=pixels/(1920*1080),targetMbps=Math.max(2.5,3.5*Math.pow(ratio,.62)*cadence),uploadBudget=Math.max(2,screenBitrateMbps/Math.max(1,Number(viewers)||1));return Math.round(Math.min(uploadBudget,targetMbps)*1000)}
...
function updateScreenLayout(){
  const hasLocal=!screenPreview.hidden,hasRemote=!remoteScreen.hidden,fullscreen=[screenPreview,remoteScreen].includes(document.fullscreenElement)||screenPreview.classList.contains('fs')||remoteScreen.classList.contains('fs');if(!hasRemote&&focusedScreen==='remote')focusedScreen='local';if(!hasLocal&&focusedScreen==='local')focusedScreen='remote';if(!hasLocal&&!hasRemote)screenExpanded=false;if(focusedScreen==='remote'&&remoteScreenSuppressed)screenExpanded=false;
...
function exitShareFullscreen({collapse=false}={}){nativeShareFullscreen=false;screenPreview.classList.remove('fs');remoteScreen.classList.remove('fs');document.body.classList.remove('screen-fullscreen');try{if([screenPreview,remoteScreen].includes(document.fullscreenElement))document.exitFullscreen().catch(()=>{})}catch{}if(collapse)screenExpanded=false;updateScreenLayout()}
async function toggleRemoteFs(){const target=focusedScreen==='local'?screenPreview:remoteScreen;if(target.classList.contains('fs')||document.fullscreenElement===target){exitShareFullscreen();return}if(!screenExpanded||target.hidden)return;try{if(!target.requestFullscreen)throw new Error('element fullscreen unavailable');await target.requestFullscreen()}catch{target.classList.add('fs');document.body.classList.add('screen-fullscreen')}updateScreenLayout()}
...
document.addEventListener('fullscreenchange',()=>{const is=[screenPreview,remoteScreen].includes(document.fullscreenElement);document.body.classList.toggle('screen-fullscreen',is);if(!is){screenPreview.classList.remove('fs');remoteScreen.classList.remove('fs')}updateScreenLayout()});document.addEventListener('keydown',event=>{if(event.key!=='Escape')return;if(!shareContextMenu.hidden){hideShareContextMenu();return}if([screenPreview,remoteScreen].includes(document.fullscreenElement)||screenPreview.classList.contains('fs')||remoteScreen.classList.contains('fs'))exitShareFullscreen();else if(screenExpanded)returnToSharePreview()});
