...
let pc,chat,files,role,sharedKey,directFileKey=null,directFileId='',sendQueue=Promise.resolve(),receiveQueue=Promise.resolve(),pairSignalBusy=false,pairReplyAccepted=false;let CHUNK=1024*1024;const MAX=200*1024**3;
let directoryTrustedConnection=false,recordConversationMessage=()=>{},directoryProfilePush=()=>{},syncFileAttachmentUi=()=>{};
...
async function safeSend(data){return busSafeSend(data)}
// Apply backpressure to the direct WebRTC file channel so its send buffer
// remains bounded even during very large transfers.
const FILE_DRAIN_TIMEOUT=45000,busDrains=new Map();function awaitBusDrain(bus,afterSendError=false){if(!bus||bus!==fileBus())return Promise.resolve(false);const low=sendWindow*.5,starting=bus.bufferedAmount,goal=afterSendError?Math.min(low,Math.max(0,starting-Math.max(CHUNK,1024*1024))):low;if(bus.bufferedAmount<=goal)return Promise.resolve(true);let waiters=busDrains.get(bus);if(!waiters){waiters=new Set();busDrains.set(bus,waiters)}return new Promise(resolve=>{let done=false;const finish=ok=>{if(done)return;done=true;clearInterval(timer);clearTimeout(timeout);try{bus.removeEventListener('bufferedamountlow',h)}catch{};waiters.delete(h);resolve(ok)};const h=()=>{if(bus!==fileBus())finish(false);else if(bus.bufferedAmount<=goal)finish(true)};const timer=setInterval(h,30);const timeout=setTimeout(()=>finish(false),FILE_DRAIN_TIMEOUT);try{bus.addEventListener('bufferedamountlow',h)}catch{};waiters.add(h)})}
async function busSafeSend(data){if(directFileId&&window.pairDirectFile){try{await window.pairDirectFile.send(directFileId,data);return}catch{directFileId='';refreshTransferRouteUi()}}let retries=0;for(;;){const bus=fileBus();if(!bus)throw new Error('no file channel');
...
function setStatus(text,on=false){statusText.textContent=text;$('.connection').classList.toggle('connected',on);if(connectCard)connectCard.hidden=on;if(addFriendBtn)addFriendBtn.disabled=on;  if(on){const negotiated=pc?.sctp?.maxMessageSize||16*1024*1024;CHUNK=Math.min(4*1024*1024,Math.max(64*1024,negotiated-4096));messageInput.disabled=false;messageForm.querySelector('.send').disabled=false;fileInput.disabled=relayVoiceMode;syncFileAttachmentUi();$('#leaveRoom').hidden=false;$('#hostRoom').hidden=true;$('#joinRoom').hidden=true;callBtn.disabled=false;if(!connectSoundDone){playSound('connect');connectSoundDone=true}}else{messageInput.disabled=true;messageForm.querySelector('.send').disabled=true;fileInput.disabled=true;syncFileAttachmentUi();callBtn.disabled=true;endCall(true);queueMicrotask(()=>{if(activePeerId&&!activeServerId)syncActiveDmTransport()})}}
...
(function initChatExtras(){
  ...
  const syncFileAttachment=()=>{const available=!fileInput.disabled;fileOpt.disabled=!available;fileOpt.title=available?'':'File transfer is unavailable until your friend is online';plusBtn.classList.toggle('file-unavailable',!available)};
  ...
  Object.defineProperty(messageInput,'disabled',{set(v){this._disabled=v;if(v){this.setAttribute('disabled','')}else{this.removeAttribute('disabled')}sendBtn.disabled=v;emojiBtn.disabled=v;gifBtn.disabled=v},get(){return this._disabled!==false}});
  messageInput.disabled=orig;syncFileAttachment();
})();
...
function wire(){
  ...
  if(files){files.binaryType='arraybuffer';sendWindow=MAX_SEND_WINDOW;files.bufferedAmountLowThreshold=Math.max(4*1024*1024,sendWindow*.5);files.onmessage=event=>{receiveQueue=receiveQueue.then(()=>onFileFrame(event)).catch(()=>{})};files.onopen=()=>setStatus('Connected directly',true)}
}
...
let tcpFilePort=8787,fileTransportMode='auto';const tcpLaneWait=new Map();
...
if(window.pairDirectFile){window.pairDirectFile.onFrame((id,data)=>{if(!directFileId){directFileId=id;refreshTransferRouteUi()}if(id!==directFileId)return;const bytes=data instanceof ArrayBuffer?data:Uint8Array.from(data||[]).buffer;receiveQueue=receiveQueue.then(()=>onFileFrame({data:bytes})).catch(()=>{})});window.pairDirectFile.onClose(id=>{if(id===directFileId){directFileId='';refreshTransferRouteUi()}})}
...
function setupPeer(){
  ...
  peer.onconnectionstatechange=()=>{if(pc!==peer)return;const state=peer.connectionState;if(state==='connected'){if(dmConnectingPeerId===dmPeerId)dmConnectingPeerId='';screenBtn.disabled=relayVoiceMode;if(peer._connectTimer){clearTimeout(peer._connectTimer);peer._connectTimer=connectTimer=null}if(callActive)publishCallState(true);if(!wasEverConnected){wasEverConnected=true;if(reconnectCall){reconnectCall=false;if(localStream){localStream.getTracks().forEach(t=>t.stop());localStream=null}callActive=false;startCall()}}else{setStatus(relayVoiceMode?'Voice relay active · files and screen share stay P2P':'Connected directly',true);friendLeftNotified=false}}if(['failed','disconnected','closed'].includes(state)){if(callActive)publishCallState(false);if(dmConnectingPeerId===dmPeerId)dmConnectingPeerId='';screenBtn.disabled=true;abortScreenSharePicker();if(screenActive||screenStarting||screenStream||nativeScreenSession)void stopScreenShare(true);else screenGen++;if(peer._connectTimer){clearTimeout(peer._connectTimer);peer._connectTimer=connectTimer=null}applyRemoteCallState(false);setStatus(state)}if(state==='connecting'){pairHint.textContent=(relayVoiceMode?'Connecting low-bandwidth voice relay':'Negotiating peer connection')+' (ICE '+(peer.iceConnectionState||'')+')…';armConnectTimeout()}};peer.oniceconnectionstatechange=()=>{if(pc!==peer)return;if(peer.iceConnectionState==='failed'){pairHint.textContent=relayVoiceMode?'Voice relay failed. Text will keep working, but this network cannot reach the relay.':'Direct peer connection failed; retrying before the low-bandwidth voice relay.'}else if(peer.iceConnectionState==='checking'||peer.iceConnectionState==='connected'){pairHint.textContent=(relayVoiceMode?'Connecting voice relay':'Negotiating peer connection')+' (ICE '+(peer.iceConnectionState||'')+')…'}};peer.ondatachannel=e=>{if(e.channel.label==='chat')chat=e.channel;else if(!relayVoiceMode)files=e.channel;wire()};
...
async function sendFile(file,retryId){try{if(file.size>MAX)return alert('This file is larger than 200 GiB.');if(!fileBus()){if(LOCAL_TEST_MODE){const preview=transfer(file.name,file.size,'out');preview.querySelector('.transfer-status').textContent='Local preview — not sent';preview.querySelector('.bar i').style.width='100%';return}return alert('Connect first, then send a file.')}const el=transfer(file.name,file.size,'out');const cancelBtn=el.querySelector('.cancel-btn'),retryBtn=el.querySelector('.retry-btn');cancelBtn.hidden=false;retryBtn.hidden=true;const seq=retryId||++fileSeq;const ctrl={abort:false,remoteCancelled:false};sendAbort.set(seq,ctrl);outTransfers.set(seq,el);cancelBtn.onclick=()=>{const aw=acceptWait.get(seq);if(aw){acceptWait.delete(seq);aw.reject(new Error('Cancelled'))}ctrl.abort=true;cancelBtn.hidden=true;void safeSend(JSON.stringify({t:'cancel',seq,by:'sender'})).catch(()=>{})};sendQueue=sendQueue.then(async()=>{let t0=0;try{const usingTcp=await prepareTcpLane(),lane=usingTcp?'tcp':'webrtc';setTransferRoute(el,lane);const meta=await seal(JSON.stringify({name:file.name,size:file.size,type:file.type,seq,transport:lane}));await safeSend(JSON.stringify({t:'start',v:meta}));
  ...
  if(ctrl.abort)throw new Error('Cancelled');t0=performance.now();
  ...
  // disk writer and memory remain bounded to CRYPTO_AHEAD chunks plus the
  ...
  while(cryptoJobs.length){
    ...
    if(!ctrl.abort){await safeSend(JSON.stringify({t:'end',seq}));el.querySelector('.transfer-status').textContent='Sent';el.querySelector('.transfer-speed').textContent='';el.querySelector('.transfer-eta').textContent='';setPeerPct(el,100);cancelBtn.hidden=true;closeTransferCard(el)}sendAbort.delete(seq);}catch(e){const aw=acceptWait.get(seq);if(aw){acceptWait.delete(seq);if(!ctrl.abort)aw.reject(e)}sendAbort.delete(seq);if(ctrl.remoteCancelled){el.querySelector('.transfer-status').textContent='Friend cancelled'}else if(ctrl.abort||(e&&e.message==='Cleared')||(e&&e.message==='disconnected')){el.querySelector('.transfer-status').textContent='Cancelled'}else if(e&&e.message==='rejected'){const s=el.querySelector('.transfer-status');s.textContent='Declined by friend';s.classList.add('declined')}else{const s=el.querySelector('.transfer-status');s.textContent='Failed: '+(e?.message||e);s.classList.add('failed')}el.querySelector('.transfer-speed').textContent='';el.querySelector('.transfer-eta').textContent='';cancelBtn.hidden=true;retryBtn.hidden=false;retryBtn.onclick=()=>{el.remove();sendFile(file);};closeTransferCard(el);try{await safeSend(JSON.stringify({t:'end',seq,cancelled:true}))}catch{}}outTransfers.delete(seq);}).catch(()=>{});}catch{}}
...
function enqueueChunk(buf){
  ...
  if(hdr.t!=='c')return;
  const seq=hdr.s||0;if(isCancelledOffer(seq))return;
...
async function onFileFrame(e,offerInBackground=false){
  ...
  if(o.t==='tcp-prepare'){if(!window.pairDirectFile||!directFileKey||typeof o.token!=='string')return;const port=tcpFilePort;try{const listening=await window.pairDirectFile.listen(port);if(!listening?.ok)throw new Error(listening?.error||'Could not listen');if(!await window.pairDirectFile.register(o.token,directFileKey))throw new Error('Could not authorize TCP connection');await safeSend(JSON.stringify({t:'tcp-ready',token:o.token,port:listening.port}))}catch{await safeSend(JSON.stringify({t:'tcp-unavailable',token:o.token})).catch(()=>{})}return}
  if(o.t==='tcp-ready'||o.t==='tcp-unavailable'){const wait=tcpLaneWait.get(o.token);if(wait){tcpLaneWait.delete(o.token);o.t==='tcp-ready'?wait.resolve(Number(o.port)||8787):wait.reject(new Error('Friend TCP listener unavailable'))}return}
  ...
  try{
  if(o.t==='start'){let meta;try{meta=JSON.parse(dec.decode(await open(o.v)))}catch{try{await safeSend(JSON.stringify({t:'reject',seq:0}))}catch{};return}const seq=Number(meta.seq);if(!Number.isSafeInteger(seq)||seq<1||!Number.isSafeInteger(meta.size)||meta.size<0||meta.size>MAX||typeof meta.name!=='string'||meta.name.length>255){try{await safeSend(JSON.stringify({t:'reject',seq:0}))}catch{};return}if(takeCancelledOffer(seq))return;const accepted=await showAcceptCard(meta,seq);if(!accepted){dropPending(seq);await safeSend(JSON.stringify({t:'reject',seq}));return}if(takeCancelledOffer(seq)){dropPending(seq);return}const t={...meta,seq,received:0,wire:0,el:transfer(meta.name,meta.size,'in',meta.transport),startTime:performance.now(),frames:[],parts:[],lastSeen:false,abort:false,done:Promise.resolve(),writeError:null,saveMode:'mem',writer:null,stuck:null};t.writeQueue=makeWriteQueue(t);activeTransfers.set(seq,t);const cancelBtn=t.el.querySelector('.cancel-btn');cancelBtn.hidden=false;cancelBtn.onclick=()=>{if(t.abort)return;t.abort=true;rememberCancelledOffer(seq);if(t.saveMode==='pair')window.pairSave?.cancel?.().catch?.(()=>{});if(t.writer)t.writer.abort?.().catch?.(()=>{});activeTransfers.delete(seq);cancelBtn.hidden=true;const s=t.el.querySelector('.transfer-status');s.textContent='Cancelled';closeTransferCard(t.el);void safeSend(JSON.stringify({t:'cancel',seq,by:'receiver'})).catch(()=>{})};let saveErr=null;if(window.pairSave){try{const r=await window.pairSave.start(meta.name);if(r&&r.ok){t.saveMode='pair'}else saveErr='Save dialog declined'}catch(e){saveErr=e.message}}else if(window.showSaveFilePicker){try{const handle=await showSaveFilePicker({suggestedName:meta.name});t.writer=await handle.createWritable();t.saveMode='fileAccess'}catch(e){saveErr=e.message}}else saveErr='No save method available';if(t.abort||takeCancelledOffer(seq)){t.abort=true;if(t.saveMode==='pair')try{await window.pairSave.cancel()}catch{}if(t.writer)try{await t.writer.abort()}catch{}activeTransfers.delete(seq);return}if(saveErr&&meta.size>5*1024*1024){t.abort=true;try{await safeSend(JSON.stringify({t:'reject',seq}))}catch{};const s=t.el.querySelector('.transfer-status');s.textContent='Failed: '+saveErr;s.classList.add('failed');cancelBtn.hidden=true;closeTransferCard(t.el);activeTransfers.delete(seq);return}if(t.saveMode==='mem'&&meta.size>4*1024*1024*1024){alert('No disk streaming available for files over 4 GB. The transfer will fail.');t.abort=true;try{await safeSend(JSON.stringify({t:'reject',seq}))}catch{};const s=t.el.querySelector('.transfer-status');s.textContent='Failed: File too large for memory mode';s.classList.add('failed');cancelBtn.hidden=true;closeTransferCard(t.el);activeTransfers.delete(seq);return}
  ...
  const held=pendingFrames.get(seq);if(held){for(const p of held){t.frames.push(p.frame);t.wire+=p.len;if(p.last)t.lastSeen=true}dropPending(seq)}
           t.done=processIncoming(t);t.done.catch(e=>{if(t.el){const s=t.el.querySelector('.transfer-status');if(s&&!s.classList.contains('failed')&&!s.classList.contains('declined')){s.textContent='Failed: '+(e?.message||e);s.classList.add('failed');t.el.querySelector('.cancel-btn').hidden=true;closeTransferCard(t.el)}t.abort=true;rememberCancelledOffer(t.seq);activeTransfers.delete(t.seq);void safeSend(JSON.stringify({t:'cancel',seq:t.seq,by:'receiver'})).catch(()=>{})}});}else if(o.t==='cancel'){const outgoing=sendAbort.get(o.seq);if(outgoing){outgoing.abort=true;outgoing.remoteCancelled=o.by==='receiver'}const ac=acceptCards.get(o.seq);if(ac)try{ac(false)}catch{};acceptCards.delete(o.seq);pendingFrames.delete(o.seq);const t=activeTransfers.get(o.seq);if(t){t.abort=true;if(t.saveMode==='pair')window.pairSave?.cancel?.().catch?.(()=>{});if(t.writer)t.writer.abort?.().catch?.(()=>{});const s=t.el?.querySelector('.transfer-status');if(s){s.textContent=o.by==='receiver'?'Friend cancelled':'Sender cancelled';s.classList.add('declined')}t.el?.querySelector('.cancel-btn')&&(t.el.querySelector('.cancel-btn').hidden=true);closeTransferCard(t.el);activeTransfers.delete(o.seq)}}else if(o.t==='progress'){const el=outTransfers.get(o.seq)||(activeTransfers.get(o.seq)&&activeTransfers.get(o.seq).el);if(el)setPeerPct(el,o.p|0);return}else if(o.t==='reject'){const t=activeTransfers.get(o.seq);const aw=acceptWait.get(o.seq);if(aw){acceptWait.delete(o.seq);aw.reject(new Error('rejected'))}if(t){t.abort=true;if(t.saveMode==='pair')try{await window.pairSave.cancel()}catch{}if(t.writer)try{await t.writer.abort()}catch{}const s=t.el.querySelector('.transfer-status');s.textContent='Declined';s.classList.add('declined');t.el.querySelector('.cancel-btn').hidden=true;closeTransferCard(t.el);activeTransfers.delete(o.seq)}}else if(o.t==='accept'){const aw=acceptWait.get(o.seq);if(aw){acceptWait.delete(o.seq);aw.resolve()}}else if(o.t==='end'){const t=activeTransfers.get(o.seq);if(!t){const ac=acceptCards.get(o.seq);if(ac)try{ac(false)}catch{};return}acceptWait.delete(o.seq);if(o.cancelled||t.abort){const senderCancelled=!!o.cancelled&&!t.abort;t.abort=true;if(t.saveMode==='pair')try{await window.pairSave.cancel()}catch{}if(t.writer)try{await t.writer.abort()}catch{}if(senderCancelled){const s=t.el.querySelector('.transfer-status');s.textContent='Sender cancelled';s.classList.add('declined');t.el.querySelector('.cancel-btn').hidden=true;closeTransferCard(t.el);activeTransfers.delete(o.seq)}return}try{await t.done;if(t.saveMode==='fileAccess')await t.writer.close();else if(t.saveMode==='pair')await window.pairSave.end();else{const a=document.createElement('a');a.href=URL.createObjectURL(new Blob(t.parts||[],{type:t.type}));a.download=t.name;a.click()}t.el.querySelector('.transfer-status').textContent='Received';t.el.querySelector('.transfer-speed').textContent='';t.el.querySelector('.transfer-eta').textContent='';t.el.querySelector('.cancel-btn').hidden=true;setPeerPct(t.el,100);closeTransferCard(t.el)}catch(e){if(t.saveMode==='pair')try{await window.pairSave.cancel()}catch{}if(t.writer)try{await t.writer.abort()}catch{}const s=t.el.querySelector('.transfer-status');s.textContent='Save failed: '+(e?.message||e);s.classList.add('failed');t.el.querySelector('.cancel-btn').hidden=true;closeTransferCard(t.el)}finally{activeTransfers.delete(o.seq)}}
  }catch{}}
...
function enableLocalTestControls(){if(!LOCAL_TEST_MODE)return;messageInput.disabled=false;messageForm.querySelector('.send').disabled=false;fileInput.disabled=false;syncFileAttachmentUi();callBtn.disabled=false;screenBtn.disabled=false;statusText.textContent='Local test mode';pairHint.textContent='Test mode is on — messages stay on this device until you pair with a friend.'}
...
function renderAccountSummary(){const summary=$('#accountSummary'),username=$('#accountUsername');if(summary)summary.textContent=directoryAccountName?'Signed in as @'+directoryAccountName+'. This identity and its friend list can now be recovered on another computer.':'This identity currently exists only on this device. Create an account to recover your friends after reinstalling or changing operating systems.';if(username&&directoryAccountName)username.value=directoryAccountName}
...
function syncActiveDmTransport(){
  ...
  fileInput.disabled=relayVoiceMode||!friend?.online||directorySocket?.readyState!==WebSocket.OPEN;syncFileAttachmentUi();
...
function renderUnreadBadges(){
  ...
  if(dock){
    ...
    // This is a notification dock, not a second permanent DM list. A friend
    // appears here only while their conversation has unread messages.
    const friends=(directorySnapshot.friends||[]).filter(friend=>unreadDmCount(friend.id)>0);
    for(const friend of friends){
      ...
      const avatar=document.createElement('span');avatar.className='unread-dm-rail-avatar';paintDirectoryAvatar(avatar,friend);avatar.style.backgroundSize='cover';avatar.style.backgroundPosition='center';button.append(avatar);const count=document.createElement('span');count.className='unread-dm-rail-badge';count.textContent=unread>99?'99+':String(unread);count.setAttribute('aria-hidden','true');button.append(count);button.onclick=()=>selectFriend(friend.id);dock.append(button);
    }
  }
...
function closeDm(id){closedDmIds.add(id);persistClosedDms();if(activePeerId===id){activePeerId='';showFriendsLanding()}renderFriends()}
...
async function ensureServerPeer(peerId,context={type:'server',serverId:activeServerId,channelId:activeChannelId}){
  if(serverPeers.has(peerId)){const existing=serverPeers.get(peerId);await activateServerPeerMedia(peerId,existing);return existing}
  ...
  connection.ontrack=event=>{if(!voicePeerAllowed(peerId)){state.pendingTracks.push(event);event.track.addEventListener?.('ended',()=>{state.pendingTracks=state.pendingTracks.filter(item=>item!==event)},{once:true});return}attachServerIncomingTrack(peerId,state,event)};
...
function updateDirectorySnapshot(snapshot){
  ...
  const joinedServer=joinedVoiceServerId?directorySnapshot.servers?.find(server=>server.id===joinedVoiceServerId):null;if(serverVoiceStream&&joinedVoiceChannelId&&!joinedServer?.channels?.some(channel=>channel.id===joinedVoiceChannelId))stopServerVoice();if(activeServerId){const server=activeServer();if(activeChannelId&&!server?.channels?.some(channel=>channel.id===activeChannelId)){const fallback=server?.channels?.find(channel=>channel.type==='text')||server?.channels?.[0];if(fallback)selectServerChannel(server.id,fallback.id)}else renderChannels()}if(serverVoiceStream){syncServerMesh();flushPendingServerTracks();renderServerVoiceUI()}
...
async function automaticPair(kind,explicitRoom='',expectedPeerId=''){
  ...
  reconnectCall=callActive;if(pc||signaling){disconnectRoom();syncActiveDmTransport()}
...
async function setupNativeScreenCapture(){
  ...
  try{
    ...
    unsubFormat=window.pairCapture.onFormat?.(fmt=>{if(!isCurrent())return;if(fmt?.available===false)captureFailure='WASAPI process-loopback format unavailable';else formatReady=true});
    if(!isCurrent()){dispose(false);try{ctx.close()}catch{};return null}window.pairCapture.start();
    // Wait for either initial PCM or a confirmed output format. An idle route
    // intentionally has no PCM yet but must remain attached for later sound.
    const deadline=Date.now()+2500;
    while(isCurrent()&&!addonData&&!formatReady&&!captureFailure&&Date.now()<deadline)await new Promise(r=>setTimeout(r,40));
    if(!isCurrent()||captureFailure||(!addonData&&!formatReady)){
