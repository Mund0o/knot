...
let localStream=null,micMuted=false,callActive=false,callStart=0,callTimerId=null,callStarting=false,callGen=0,reconnectCall=false,dmVoiceFocused=true;
...
let directorySocket=null,directoryReconnect=null,directoryBackoff=1000,directoryUserId='',directoryToken='',directorySnapshot={friends:[],servers:[],members:{},voiceStates:{}},activePeerId='',activeServerId='',activeChannelId='',activeConversationKey='',historyRendering=false;
let conversationHistories={},historySaveTimer=null,serverVoiceStream=null,serverScreenStream=null,serverVoiceMuted=false,serverVoiceFocused=true,serverScreenStarting=false,joinedVoiceChannelId='',joinedVoiceAt=0,voiceElapsedTimer=null,draggedChannelId='';const serverPeers=new Map();
...
function renderVoiceParticipants(channelId,container){for(const entry of voiceChannelEntries(channelId)){const member=directorySnapshot.members?.[entry.id]||(entry.id===directoryUserId?directorySnapshot.self:null);if(!member)continue;const row=document.createElement('div');row.className='voice-channel-member';const avatar=document.createElement('span');avatar.className='friend-avatar';paintDirectoryAvatar(avatar,member);const name=document.createElement('span');name.textContent=(member.name||'Knot user')+(entry.id===directoryUserId?' (you)':'');const elapsed=document.createElement('time');elapsed.dataset.voiceJoined=String(entry.joinedAt||Date.now());row.append(avatar,name,elapsed);container.append(row)}}
...
function renderServerVoiceUI(){const server=activeServer(),channel=server?.channels?.find(item=>item.id===joinedVoiceChannelId),connected=!!(serverVoiceStream&&channel);const dock=$('#serverVoiceDock'),stage=$('#serverVoiceStage');dock.hidden=!connected;stage.hidden=!connected||!serverVoiceFocused;document.body.classList.toggle('server-voice-connected',connected);if(!connected)return;$('#serverVoiceDockChannel').textContent=channel.name;$('#serverVoiceStageChannel').textContent=channel.name;const members=$('#serverVoiceStageMembers');members.replaceChildren();for(const entry of voiceChannelEntries(channel.id)){const member=directorySnapshot.members?.[entry.id]||(entry.id===directoryUserId?directorySnapshot.self:null);if(!member)continue;const card=document.createElement('div');card.className='server-stage-member';const avatar=document.createElement('span');avatar.className='server-stage-avatar';paintDirectoryAvatar(avatar,member);const name=document.createElement('strong');name.textContent=(member.name||'Knot user')+(entry.id===directoryUserId?' (you)':'');card.append(avatar,name);members.append(card)}const muted=serverVoiceMuted;for(const id of ['serverVoiceMute','serverStageMute']){const button=$('#'+id);button.classList.toggle('active',muted);button.setAttribute('aria-label',muted?'Unmute microphone':'Mute microphone');button.title=muted?'Unmute microphone':'Mute microphone'}const sharing=!!serverScreenStream;for(const id of ['serverVoiceShare','serverStageShare']){const button=$('#'+id);button.classList.toggle('active',sharing);button.setAttribute('aria-label',sharing?'Stop screen sharing':'Share screen');button.title=sharing?'Stop screen sharing':'Share screen';button.disabled=serverScreenStarting}$('#serverVoiceFocus').classList.toggle('active',serverVoiceFocused);refreshVoiceElapsed()}
function renderDmVoiceUI(){const dock=$('#dmVoiceDock');if(!dock)return;const connected=!activeServerId&&(callActive||friendInCall);dock.hidden=!connected;document.body.classList.toggle('dm-call-connected',connected);document.body.classList.toggle('dm-call-collapsed',connected&&!dmVoiceFocused);if(!connected){document.body.classList.remove('dm-call-collapsed');return}$('#dmVoiceDockName').textContent=friendName||'Direct call';$('#dmVoiceDockTime').textContent=voiceElapsed(callStart||Date.now());$('#dmVoiceMute').classList.toggle('active',micMuted);$('#dmVoiceMute').disabled=!callActive;$('#dmVoiceShare').classList.toggle('active',screenActive||screenStarting);$('#dmVoiceShare').disabled=!callActive||!pc;$('#dmVoiceFocus').classList.toggle('active',dmVoiceFocused)}
...
function closeServerMesh(){for(const state of serverPeers.values()){try{state.channel?.close()}catch{}try{state.pc.close()}catch{}for(const audio of state.audios||[])try{audio.remove()}catch{}try{state.screen?.remove()}catch{}}serverPeers.clear()}
...
async function startLocalTestCall(){
  ...
  try{
    ...
    callActive=true;callStart=Date.now();dmVoiceFocused=true;callBtn.textContent='End call';callBtn.title='End local mic test';callBtn.disabled=false;muteBtn.hidden=false;micMuted=false;muteBtn.textContent='Mute';applyMicTransmission();setParticipant(participantYou,true);playSound('ring');callStatus.textContent='Testing microphone locally';callStatus.className='call-status live';
...
async function startCall(){
  ...
  try{
    ...
    callActive=true;callStart=Date.now();dmVoiceFocused=true;setRemoteCallAudio(true);callBtn.textContent='End call';callBtn.title='End voice call';callBtn.disabled=false;muteBtn.hidden=false;micMuted=false;muteBtn.textContent='Mute';muteBtn.title='Mute microphone';applyMicTransmission();
