...
function speakingTargets(key){if(key==='dm-self')return[participantYou.querySelector('.avatar'),$('#sidebarProfileAvatar')].filter(Boolean);if(key==='dm-friend')return[participantFriend.querySelector('.avatar'),activePeerId?document.querySelector(`.friend-entry[data-id="${activePeerId}"] .friend-avatar`):null].filter(Boolean);const id=key.startsWith('server:')?key.slice(7):'';return id?[...document.querySelectorAll(`[data-speaking-id="${id}"]`)]:[]}
...
function renderDmVoiceUI(){const dock=$('#dmVoiceDock');if(!dock)return;const connected=!activeServerId&&(callActive||friendInCall);dock.hidden=!connected;document.body.classList.toggle('dm-call-connected',connected);if(!connected)return;$('#dmVoiceDockName').textContent=friendName||'Direct call';$('#dmVoiceDockTime').textContent=voiceElapsed(callStart||Date.now());$('#dmVoiceMute').classList.toggle('active',micMuted);$('#dmVoiceMute').disabled=!callActive;$('#dmVoiceShare').classList.toggle('active',screenActive||screenStarting);$('#dmVoiceShare').disabled=!callActive||!pc}
...
async function startCall(){
  ...
  try{
    ...
    callActive=true;callStart=Date.now();monitorSpeaking('dm-self',localStream);setRemoteCallAudio(true);callBtn.textContent='End call';callBtn.title='End voice call';callBtn.disabled=false;muteBtn.hidden=false;micMuted=false;muteBtn.textContent='Mute';muteBtn.title='Mute microphone';applyMicTransmission();
