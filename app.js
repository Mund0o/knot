...
function updateDirectorySnapshot(snapshot){const profilesChanged=profileSnapshotSignature(directorySnapshot)!==profileSnapshotSignature(snapshot);directorySnapshot=snapshot;renderFriends();renderServers();if(activePeerId){const friend=directoryUser(activePeerId);if(friend)applyFriendProfile(friend)}if(activeServerId){renderChannels();syncServerMesh()}if(profilesChanged&&activeConversationKey)openConversation(activeConversationKey)}
