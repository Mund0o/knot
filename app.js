...
function stopSpeakingMonitor(key){const monitor=speakingMonitors.get(key);if(!monitor)return;clearInterval(monitor.timer);try{monitor.source.disconnect()}catch{}speakingMonitors.delete(key);paintSpeaking(key,false);if(!speakingMonitors.size&&speakingAudioCtx){try{speakingAudioCtx.close()}catch{}speakingAudioCtx=null}}
function monitorSpeaking(key,stream){if(stream?.getVideoTracks?.().length)return;stopSpeakingMonitor(key);const track=stream?.getAudioTracks?.()[0];if(!track)return;try{if(!speakingAudioCtx)speakingAudioCtx=new (window.AudioContext||window.webkitAudioContext)({sampleRate:48000});if(speakingAudioCtx.state==='suspended')speakingAudioCtx.resume().catch(()=>{});const source=speakingAudioCtx.createMediaStreamSource(new MediaStream([track])),analyser=speakingAudioCtx.createAnalyser(),samples=new Float32Array(512),monitor={source,timer:null,speaking:false};analyser.fftSize=512;analyser.smoothingTimeConstant=.32;source.connect(analyser);let holdUntil=0;monitor.timer=setInterval(()=>{analyser.getFloatTimeDomainData(samples);let energy=0;for(const sample of samples)energy+=sample*sample;const loud=Math.sqrt(energy/samples.length)>.018;if(loud)holdUntil=performance.now()+220;const speaking=loud||performance.now()<holdUntil;if(speaking!==monitor.speaking){monitor.speaking=speaking;paintSpeaking(key,speaking)}},80);speakingMonitors.set(key,monitor);track.addEventListener?.('ended',()=>stopSpeakingMonitor(key),{once:true})}catch{paintSpeaking(key,false)}}
...
function clearRemoteScreenShare(status='Not sharing'){
  ...
  queueMicrotask(()=>{try{returnToSharePreview()}catch{try{updateScreenLayout()}catch{}}});
}
function stopWatchingRemoteShare(){
  if(remoteScreen.hidden&&!remoteScreen.srcObject)return;
  remoteScreenSuppressed=true;
  try{remoteScreen.srcObject?.getTracks?.().forEach(track=>{track.enabled=false})}catch{}
  remoteScreen.srcObject=null;remoteScreen.hidden=true;screenStatus.textContent='Screen share hidden locally';
  try{returnToSharePreview()}catch{try{updateScreenLayout()}catch{}}
}
function receiveDirectMessage(message){
  const peerId=dmPeerId||dmCallPeerId||activePeerId,key=peerId?'dm:'+peerId:'';
...
function wire(){
  if(chat){
    chat.onopen=()=>{setStatus('Connected directly',true);announceProfile()};
    chat.onmessage=async event=>{
      try{
        ...
        if(value.t==='call-ring'){
          dmCallPeerId=dmPeerId||activePeerId;friendLeftNotified=false;setParticipant(participantFriend,true);renderCallPeerProfile();logCallEvent('Friend joined the call');playSound('ring');setRemoteCallAudio(callActive);return;
        }
        if(value.t==='call-end'){
          setParticipant(participantFriend,false);stopSpeakingMonitor('dm-friend');clearRemoteScreenShare('Friend left the call');if(!callActive)dmCallPeerId='';if(!friendLeftNotified){friendLeftNotified=true;playSound('leave')}logCallEvent('Friend left the call');callStatus.textContent='Friend left the call';callStatus.className='call-status';renderDmVoiceUI();return;
        }
...
function setupPeer(){
  ...
  pc.ontrack=e=>{logCallEvent('Diag: ontrack kind='+e.track.kind);try{const stream=e.streams[0]||new MediaStream([e.track]);
    ...
    if(e.track.kind==='audio'&&bindsToScreen){
      if(remoteScreenSuppressed){e.track.enabled=false;return}
...
function monitorRemoteScreenDecode(receiver,track){
  ...
  const sample=async()=>{try{
    ...
    if(stalls>=2&&/video\/AV1/i.test(codec?.mimeType||'')&&chat?.readyState==='open'){
      screenStatus.textContent='AV1 decoder stalled — switching to H.264';send({t:'screen-codec-fallback'});stop();
    }
  }catch{}};
...
let conversationHistories={},historySaveTimer=null,serverVoiceStream=null,serverScreenStream=null,serverVoiceMuted=false,serverScreenStarting=false,joinedVoiceChannelId='',joinedVoiceAt=0,voiceElapsedTimer=null,draggedChannelId='';const serverPeers=new Map();
...
function showFriends({expand=true}={}){if(expand)setSocialSidebarCollapsed(false);if(serverVoiceStream||joinedVoiceChannelId)stopServerVoice();activeServerId='';activeChannelId='';if(!dmCallOngoing()){callBtn.textContent='Start call';callStatus.textContent='Voice off';callStatus.className='call-status'}$('#friendsNavigation').hidden=false;$('#serverNavigation').hidden=true;$('#serverMemberPanel').hidden=true;document.body.classList.remove('server-view');document.querySelectorAll('#serverList .rail-button').forEach(button=>button.classList.remove('active'));$('#homeButton').classList.add('active');closeServerMesh();if(!activePeerId)showFriendsLanding();renderDmVoiceUI()}
async function selectFriend(id,{connect=true}={}){const friend=directoryUser(id);if(!friend)return;const backgroundCall=dmCallOngoing()&&dmPeerId&&id!==dmPeerId;showFriends();activePeerId=id;applyFriendProfile(friend);openConversation('dm:'+id);document.querySelectorAll('.friend-entry').forEach(button=>button.classList.toggle('active',button.dataset.id===id));if(backgroundCall){messageInput.disabled=true;fileInput.disabled=true;setStatus('Viewing history — your call stays connected');pairHint.textContent='Your call with '+(directoryUser(dmCallPeerId)?.name||'your friend')+' is still active. Return to that DM to send messages or share files.';renderCallPeerProfile();return}if(pc&&dmPeerId===id){messageInput.disabled=chat?.readyState!=='open';fileInput.disabled=files?.readyState!=='open';setStatus('Connected directly',true);renderCallPeerProfile();return}if(pc)disconnectRoom();setStatus(friend.online?'Online — connecting directly':'Offline');if(connect&&friend.online){const session=clientHex(16);directorySend({type:'connect',peerId:id,session,context:{type:'dm'}});await automaticPair('host',session,id);applyFriendProfile(friend);renderCallPeerProfile()}else if(!friend.online){messageInput.disabled=true;messageForm.querySelector('.send').disabled=true;fileInput.disabled=true;pairHint.textContent=friendName+' is offline. Knot will show them online as soon as they open the app.'}}
...
async function ensureServerPeer(peerId,context={type:'server',serverId:activeServerId,channelId:activeChannelId}){if(serverPeers.has(peerId))return serverPeers.get(peerId);const connection=new RTCPeerConnection({iceServers:ICE_SERVERS}),state={pc:connection,channel:null,candidates:[],audios:[],screen:null,screenSenders:[],context};serverPeers.set(peerId,state);connection.onicecandidate=event=>{if(event.candidate)directorySend({type:'signal',peerId,context:state.context,payload:{kind:'candidate',candidate:event.candidate.toJSON()}})};connection.onconnectionstatechange=()=>{if(['failed','closed'].includes(connection.connectionState)){stopSpeakingMonitor('server:'+peerId);state.audios.forEach(audio=>audio.remove());try{state.screen?.remove()}catch{}serverPeers.delete(peerId)}};connection.ondatachannel=event=>{state.channel=event.channel;wireServerChannel(peerId,state.channel,state.context.serverId)};connection.ontrack=event=>{if(!voicePeerAllowed(peerId))return;if(event.track.kind==='video'){const video=document.createElement('video');video.autoplay=true;video.playsInline=true;video.srcObject=event.streams[0]||new MediaStream([event.track]);video.dataset.peerId=peerId;$('#serverVoiceScreens').append(video);state.screen=video;renderServerVoiceUI();video.play().catch(()=>{});event.track.onended=()=>{video.remove();if(state.screen===video)state.screen=null;renderServerVoiceUI()};return}const audio=document.createElement('audio');audio.autoplay=true;audio.srcObject=event.streams[0]||new MediaStream([event.track]);audio.hidden=true;document.body.append(audio);state.audios.push(audio);monitorSpeaking('server:'+peerId,audio.srcObject);audio.play().catch(()=>{});event.track.onended=()=>{stopSpeakingMonitor('server:'+peerId);audio.remove();state.audios=state.audios.filter(item=>item!==audio)}};if(voicePeerAllowed(peerId)){for(const track of serverVoiceStream.getTracks())connection.addTrack(track,serverVoiceStream);if(serverScreenStream)for(const track of serverScreenStream.getTracks())state.screenSenders.push(connection.addTrack(track,serverScreenStream))}if(directoryUserId<peerId){state.channel=connection.createDataChannel('pair-server-chat');wireServerChannel(peerId,state.channel,state.context.serverId);const offer=await connection.createOffer();await connection.setLocalDescription(offer);directorySend({type:'signal',peerId,context:state.context,payload:{kind:'offer',sdp:connection.localDescription.sdp}})}return state}
async function handleServerSignal(message){const context=message.context||{},payload=message.payload||{};if(context.serverId!==activeServerId)return;const state=await ensureServerPeer(message.from,context),connection=state.pc;if(payload.kind==='offer'){await connection.setRemoteDescription({type:'offer',sdp:payload.sdp});const answer=await connection.createAnswer();await connection.setLocalDescription(answer);directorySend({type:'signal',peerId:message.from,context:state.context,payload:{kind:'answer',sdp:connection.localDescription.sdp}});for(const candidate of state.candidates.splice(0))await connection.addIceCandidate(candidate)}else if(payload.kind==='answer'){await connection.setRemoteDescription({type:'answer',sdp:payload.sdp});for(const candidate of state.candidates.splice(0))await connection.addIceCandidate(candidate)}else if(payload.kind==='candidate'){if(connection.remoteDescription)await connection.addIceCandidate(payload.candidate);else state.candidates.push(payload.candidate)}}
function closeServerMesh(){for(const [peerId,state] of serverPeers){stopSpeakingMonitor('server:'+peerId);try{state.channel?.close()}catch{}try{state.pc.close()}catch{}for(const audio of state.audios||[])try{audio.remove()}catch{}try{state.screen?.remove()}catch{}}serverPeers.clear()}
...
async function startServerScreenShare(){
  ...
  try{
    ...
    for(const [peerId,state] of serverPeers){if(!voicePeerAllowed(peerId))continue;for(const mediaTrack of serverScreenStream.getTracks()){const sender=state.pc.addTrack(mediaTrack,serverScreenStream);state.screenSenders.push(sender);if(mediaTrack.kind==='video'){applyScreenCodecPreference(state.pc,sender);configureScreenVideoSender(sender,mediaTrack,fps).catch(()=>{})}}renegotiateServerPeer(peerId,state).catch(()=>{})}
  }catch(error){if(serverScreenStream){serverScreenStream.getTracks().forEach(track=>track.stop());serverScreenStream=null}cleanupNativeScreenCapture();if(window.pairEnv?.platform==='linux')window.pairEnv.stopLinuxShareAudio?.();if(error?.name!=='NotAllowedError')setServerStatus('Screen share failed: '+(error?.message||error))}
  finally{serverScreenStarting=false;renderServerVoiceUI()}
}
async function stopServerScreenShare(){const stream=serverScreenStream;if(!stream)return;serverScreenStream=null;stream.getTracks().forEach(track=>track.stop());if(window.pairEnv?.platform==='linux')window.pairEnv.stopLinuxShareAudio?.();else cleanupNativeScreenCapture();const preview=$('#serverVoiceScreenPreview');preview.srcObject=null;preview.hidden=true;for(const [peerId,state] of serverPeers){for(const sender of state.screenSenders||[])try{state.pc.removeTrack(sender)}catch{}state.screenSenders=[];renegotiateServerPeer(peerId,state).catch(()=>{})}renderServerVoiceUI()}
async function joinServerVoice(){const channel=activeChannel();if(!channel||channel.type!=='voice')return callStatus.textContent='Select a voice channel first.';if(dmCallOngoing()){callStatus.textContent='Leave your direct call before joining server voice.';return}if(serverVoiceStream&&joinedVoiceChannelId===channel.id)return;stopServerVoice();try{serverVoiceStream=await navigator.mediaDevices.getUserMedia(microphoneConstraints());serverVoiceMuted=false;serverVoiceStream.getAudioTracks().forEach(track=>track.enabled=true);joinedVoiceChannelId=channel.id;joinedVoiceAt=Date.now();monitorSpeaking('server:'+directoryUserId,serverVoiceStream);directorySend({type:'voice-state',serverId:activeServerId,channelId:channel.id,joined:true});callStatus.textContent='Joined '+channel.name;callStatus.className='call-status live';callBtn.textContent='Leave voice';renderChannels();syncServerMesh()}catch(error){callStatus.textContent='Could not join voice: '+(error?.message||error);callStatus.className='call-status';syncServerMesh();renderServerVoiceUI()}}
function stopServerVoice(){stopSpeakingMonitor('server:'+directoryUserId);const serverId=activeServerId,channelId=joinedVoiceChannelId;if(channelId&&serverId)directorySend({type:'voice-state',serverId,channelId,joined:false});if(serverScreenStream)stopServerScreenShare();if(serverVoiceStream){serverVoiceStream.getTracks().forEach(track=>track.stop());serverVoiceStream=null}joinedVoiceChannelId='';joinedVoiceAt=0;serverVoiceMuted=false;clearInterval(voiceElapsedTimer);closeServerMesh();renderServerVoiceUI();if(activeServerId){callBtn.textContent=activeChannel()?.type==='voice'?'Join voice':'Start call';callStatus.textContent='Voice off';callStatus.className='call-status';renderChannels()}}
...
async function automaticPair(kind,explicitRoom='',expectedPeerId=''){
  ...
  role=kind;directoryTrustedConnection=!!expectedPeerId;activePeerId=expectedPeerId||activePeerId;dmPeerId=expectedPeerId||activePeerId;const baseAddress=PAIR_SIGNAL_SERVER; const room=String(explicitRoom||$('#roomCode').value).trim().toUpperCase();
...
async function configureScreenVideoSender(sender,track,fps){
  const settings=track?.getSettings?.()||{},requestedFps=fps===30?30:60,recoveryFps=screenRecoveryLevel?Math.min(30,requestedFps):requestedFps,scale=screenRecoveryLevel>=2?1.5:1,maxBitrate=targetScreenBitrate(settings.width,settings.height,recoveryFps);
  const parameters=sender.getParameters();if(!parameters.encodings?.length)parameters.encodings=[{}];const encoding=parameters.encodings[0];encoding.maxBitrate=maxBitrate;delete encoding.minBitrate;encoding.maxFramerate=recoveryFps;encoding.scaleResolutionDownBy=scale;parameters.degradationPreference=screenContentHint==='detail'?'maintain-resolution':'balanced';await sender.setParameters(parameters);
}
async function recoverOverloadedScreenSender(sender){
  ...
  const mode=screenRecoveryLevel===1?'30fps':'reduced render scale';screenStatus.textContent='Sharing · performance mode: '+mode+screenAudioDebug;logCallEvent('Screen encoder overloaded — enabled '+mode);
}
async function restoreScreenSenderQuality(sender){
  if(!screenActive||screenRecoveryLevel<=0)return;screenRecoveryLevel--;screenGoodSamples=0;const track=screenStream?.getVideoTracks?.()[0];try{await configureScreenVideoSender(sender,track,shareFrameRate)}catch{}
}
function startScreenStats(sender){
  ...
  const sample=async()=>{try{
    ...
    screenStatsLast={bytes,frames,totalEncode,at:now};const fps=Math.round(out.framesPerSecond||0),w=out.frameWidth||0,h=out.frameHeight||0,reason=out.qualityLimitationReason||'',requestedFps=screenRecoveryLevel?Math.min(30,shareFrameRate):shareFrameRate;
    ...
    if(screenBadSamples>=2)await recoverOverloadedScreenSender(sender);else if(screenRecoveryLevel&&screenGoodSamples>=5)await restoreScreenSenderQuality(sender);
    ...
    const status='Sharing'+(w&&h?' · '+w+'×'+h:'')+(fps?' · '+fps+'fps':'')+(mbps!=='…'?' · '+mbps+' Mbps':'')+(codec?.mimeType?' · '+codec.mimeType.replace('video/',''):'')+(encodeMs?' · '+encodeMs.toFixed(1)+'ms encode':'')+network+(screenRecoveryLevel?' · adaptive performance':'')+screenAudioDebug;screenStatus.textContent=status;screenBtn.title=status;
  }catch{}};
...
const localScreenTile=makeScreenTile(screenPreview,'You', 'local'),remoteScreenTile=makeScreenTile(remoteScreen,'Friend', 'remote'),stopWatchButton=document.createElement('button');stopWatchButton.type='button';stopWatchButton.className='screen-stop-watch';stopWatchButton.textContent='Stop watching';stopWatchButton.title='Stop watching this screen share';stopWatchButton.setAttribute('aria-label','Stop watching screen share');stopWatchButton.onclick=event=>{event.stopPropagation();stopWatchingRemoteShare()};remoteScreenTile.append(screenVolWrap,stopWatchButton);screenVideos.replaceChildren(localScreenTile,remoteScreenTile);
