...
async function setupNativeScreenCapture(){
  ...
  // Path 3: raw audio is safe only when there is no Pair playback to remove.
  // With an incoming Pair voice/screen track, forwarding the loopback would
  // echo that person back to them. Prefer no share audio over leaking Pair's
  // audio when both cancellers are unavailable.
  if(refStream&&refStream.getAudioTracks().length){console.warn('[AEC] no clean path available; Pair playback will not be forwarded');return null}
  console.log('[AEC] using raw track (no Pair playback to exclude)');
  return rawTrack;
}
