...
async function webRtcSafeSend(bus,data,session){let retries=0;for(;;){if(!bus||bus.readyState!=='open'||session&&!liveFileSession(session))throw new Error('disconnected');
  ...
  // Native AV1 share uses the same SCTP association; a 48 MiB file window
...
function askPeerNetBudget(){
  try{send({t:'net-budget-ask'})}catch{}
  for(const state of serverPeers.values()){
    if(state.closing||state.channel?.readyState!=='open')continue;
    try{state.channel.send(JSON.stringify({t:'net-budget-ask',serverId:state.context.serverId}))}catch{}
  }
}
...
function directoryMediaUrl(value){
  ...
  if(typeof value!=='string')return '';if(value==='file:v1')return localProfileAvatarUrl();
  if(value.length<=MAX_PROFILE_DATA&&/^data:image\/(?:png|jpeg|gif|webp);base64,[a-z0-9+/=]+$/i.test(value))return value;
...
function validProfileData(data){return data==='file:v1'||!!directoryMediaUrl(data)}
function localProfileAvatarUrl(){return 'knot-avatar://local/profile?v='+(Number(profileAvatarGeneration)||1)}
function blobProfileSrc(data){
  if(typeof data!=='string'||!data.startsWith('data:image/'))return '';
  const comma=data.indexOf(',');if(comma<0)return '';
  try{
    const binary=atob(data.slice(comma+1).replace(/\s+/g,'')),bytes=new Uint8Array(binary.length);
...
function setAvatar(el,data){
  if(!el)return;
  let clip=el.querySelector(':scope > .avatar-photo-clip');
  let img=clip?.querySelector('.avatar-photo')||el.querySelector(':scope > .avatar-photo');
  const clear=()=>{el.classList.remove('has-image');el.style.backgroundImage='';clip?.remove();if(img&&img.parentElement!==clip)img.remove()};
  if(data==='file:v1'||(typeof data==='string'&&(data.startsWith('knot-avatar:')||data.startsWith('blob:')||data.startsWith('data:image/')))){
    const src=data==='file:v1'?localProfileAvatarUrl():data.startsWith('data:image/')&&data.length>1800000?blobProfileSrc(data):data.startsWith('data:image/')?directoryMediaUrl(data):data;
    if(!src){clear();return}
    el.style.backgroundImage='';
    if(!clip){clip=document.createElement('span');clip.className='avatar-photo-clip';el.prepend(clip)}
    if(!img){img=document.createElement('img');img.className='avatar-photo';img.alt='';clip.append(img)}
    else if(img.parentElement!==clip)clip.append(img);
  ...
  const safe=directoryMediaUrl(data);
  if(!safe){clear();return}
  clip?.remove();
  img?.remove();
  el.classList.add('has-image');
  el.style.backgroundImage='url("'+safe.replace(/"/g,'%22')+'")';
}
...
function setAvatarFrame(el,frame){if(!el)return;const f=normalizeFrame(frame);el.style.backgroundSize=f.zoom+'% auto';el.style.backgroundPositionX=f.x+'%';el.style.backgroundPositionY=f.y+'%';const img=el.querySelector('.avatar-photo');if(img){img.style.transform='none';img.style.objectFit='cover';img.style.objectPosition='50% 50%';img.style.maxWidth='none';img.style.maxHeight='none';img.style.inset='auto';img.style.right='auto';img.style.bottom='auto';img.style.width=f.zoom+'%';img.style.height=f.zoom+'%';img.style.left=(f.x*(1-f.zoom/100))+'%';img.style.top=(f.y*(1-f.zoom/100))+'%'}}
...
function openSettings(showPhotoEditor=false){settingsReturnFocus=document.activeElement;connectCard.open=false;settingsPanel.hidden=false;document.body.classList.add('settings-open');syncPanelBackdrop();if(showPhotoEditor){openSettingsTab('profile');const photo=settingsAvatar.querySelector('.avatar-photo');profileEditor.hidden=!(profileAvatar&&photo&&photo.naturalWidth>0)}setTimeout(()=>$('#closeSettings')?.focus(),0)}
...
function renderAccountSummary(){
  ...
  if(summary){
    if(directoryAccountName&&signed)summary.textContent='Signed in as @'+directoryAccountName+' on this computer. Friends and servers restore from this account.';
    else if(directoryAccountName)summary.textContent='This computer knows @'+directoryAccountName+', but the saved session is missing. Sign in again — your photo stays here.';
    else summary.textContent='This identity currently exists only on this device. Create an account to recover your profile and friends after reinstalling or changing operating systems.';
  }
  if(username&&directoryAccountName)username.value=directoryAccountName;
  const create=$('#createAccount'),signin=$('#signInAccount');
  if(create)create.hidden=!!directoryAccountName;
  if(signin)signin.hidden=!!(directoryAccountName&&signed);
}
function accountShouldRemember(){return $('#authRemember')?.checked!==false}
async function persistAccountSession({userId=directoryUserId,token=directoryToken,username='',remember=true}={}){
  if(!/^[a-f0-9]{32}$/.test(userId||'')||!/^[a-f0-9]{64}$/.test(token||''))throw new Error('Knot could not save the account session on this device.');
  directoryUserId=userId;directoryToken=token;directoryAccountName=username;transientDirectorySession=false;
  const writes=await Promise.all([ssSet('rememberAccount','yes'),ssSet('directoryUserId',userId),ssSet('directoryToken',token),ssSet('directoryAccountName',username),ssSet('accountOnboardingDismissed','yes')]);
  if(writes.some(ok=>ok===false))throw new Error('Knot could not save the account session on this device.');
}
async function loadDirectoryIdentity(){
  ...
  if(validId||accountName||storedToken)return{userId:validId?savedId:'',token:validToken?savedToken:'',accountName,needsLogin:true,minted:false};
  transientDirectorySession=true;
  return{userId:clientHex(16),token:clientHex(32),accountName:'',needsLogin:false,minted:false};
}
...
async function restoreAccountProfile(value,userId,{keepLocalLegacy=false}={}){
  ...
  const incomingImage=directoryMediaUrl(value.image),sidecarPhoto=profileAvatar==='file:v1',localPhoto=sidecarPhoto||(typeof profileAvatar==='string'&&profileAvatar.startsWith('data:image/')),incomingIsRemote=!!incomingImage&&!String(incomingImage).startsWith('data:image/')&&incomingImage!==localProfileAvatarUrl(),keepLocalPhoto=sidecarPhoto||(localPhoto&&(!incomingImage||incomingIsRemote));
  profileName=normalizeProfileName(value.name,'Knot user');if(!keepLocalPhoto){profileAvatar=incomingImage;profileAvatarGeneration++}profileFrame=normalizeFrame(value.frame);
  ...
  const persist=[ssSet('profileName',profileName),ssSet('profileFrame',JSON.stringify(profileFrame)),ssSet('profilePhotoMode',profileAvatar?(keepLocalPhoto?'custom':'account'):'none')];
  if(!sidecarPhoto&&!(keepLocalPhoto&&profileAvatar==='file:v1'))persist.push(ssSet('profileAvatar',profileAvatar||null));
...
async function deviceIdentity(){if(deviceIdentityPromise)return deviceIdentityPromise;deviceIdentityPromise=(async()=>{let saved;try{saved=JSON.parse(await ss('deviceIdentityPrivate')||'null')}catch{};try{if(validDevicePublicKey(saved)&&typeof saved.d==='string'){const privateKey=await crypto.subtle.importKey('jwk',saved,{name:'ECDH',namedCurve:'P-256'},true,['deriveBits']);const publicKey=await crypto.subtle.importKey('jwk',{kty:'EC',crv:'P-256',x:saved.x,y:saved.y,ext:true},{name:'ECDH',namedCurve:'P-256'},true,[]);return{privateKey,publicKey}}}catch{}let stored=false;try{stored=!!await ssHas('deviceIdentityPrivate')}catch{}if(stored)throw new Error('Knot could not read the saved device key on this computer.');const generated=await crypto.subtle.generateKey({name:'ECDH',namedCurve:'P-256'},true,['deriveBits']);const privateJwk=await crypto.subtle.exportKey('jwk',generated.privateKey);await ssSet('deviceIdentityPrivate',JSON.stringify(privateJwk));return generated})().catch(error=>{deviceIdentityPromise=null;throw error});return deviceIdentityPromise}
...
async function avatarThumbnail(source,{limit,dimension,quality}){
  ...
  const src=source==='file:v1'?localProfileAvatarUrl():source;
  if(source!=='file:v1'&&source.length<=limit)return source;
  const image=await new Promise((resolve,reject)=>{const value=new Image();value.crossOrigin='anonymous';value.onload=()=>resolve(value);value.onerror=()=>reject(new Error('Could not prepare profile image'));value.src=src});
...
async function linuxShareAudioTrack(){
  ...
  try{
    ...
    let attached=false;
    unsubError=window.pairEnv.onLinuxShareAudioError?.(message=>{if(!isCurrent())return;captureError=String(message||'capture failed');console.warn('[AUDIO] PipeWire capture error:',captureError);if(attached&&outputTrack?.readyState==='live'){try{outputTrack.stop()}catch{};screenAudioDebug=' · sound capture stopped';if(screenActive)screenStatus.textContent='Sharing'+screenAudioDebug;setReservedScreenAudioTrack(null).catch(()=>{});for(const state of serverPeers.values())setServerScreenAudioTrack(state,null).catch(()=>{});queueMicrotask(()=>cleanupNativeScreenCapture(captureOwner))}});
    ...
    for(let routeAttempt=1;routeAttempt<=3;routeAttempt++){
      captureError='';
      if(routeAttempt>1){
        try{await window.pairEnv.stopLinuxShareAudio?.()}catch{}
...
function paintShareSurfaceDark(el){
  ...
  try{
    el.style.backgroundColor='#050609';
    el.style.colorScheme='dark';
    if(el.tagName==='VIDEO'){
      el.style.zIndex=el.style.zIndex||'0';
      if(!el.getAttribute('poster'))el.setAttribute('poster',"data:image/svg+xml;charset=utf-8,"+encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="16" height="9"><rect width="16" height="9" fill="#050609"/></svg>'));
    }else if(el.classList?.contains('native-screen-canvas'))el.style.zIndex='1';
  }catch{}
  return el;
}
...
function prepareShareSurface(video){
  ...
  video.playsInline=true;video.setAttribute('playsinline','');paintShareSurfaceDark(video);
  if(shareVideoCoveredByCanvas(video)||video.classList.contains('native-screen-waiting')||!shareVideoHasPicture(video)){holdShareVideo(video);armShareVideoReveal(video)}
  return video;
}
function playShareVideo(video){if(!shareVideoHasMedia(video))return;try{video.play().catch(()=>{})}catch{}}
...
function attachNativeScreenSurface(video){
  ...
  let canvas=null,context=null,host=video.parentElement,createdHost=false,destroyed=false,live=false;
  try{
    // Grid parents (server stage) would place a sibling canvas in the next cell
    // instead of covering the video. Wrap those in a positioned host first.
    if(host&&!host.classList.contains('screen-tile')&&!host.classList.contains('native-screen-host')){
      const wrap=document.createElement('div');wrap.className='native-screen-host';paintShareSurfaceDark(wrap);host.insertBefore(wrap,video);wrap.append(video);host=wrap;createdHost=true;
    }
    canvas=host?.querySelector?.(':scope > .native-screen-canvas')||null;
    if(!canvas){canvas=document.createElement('canvas');canvas.className='native-screen-canvas';canvas.setAttribute('aria-hidden','true');video.after(canvas)}
...
function createWebCodecsNativeScreenPlayer(video,codec,onError=()=>{},options={}){
  ...
  let trackGenerator=null,frameWriter=null,frameWriterBusy=false,presentationQueue=[],presentationTimer=null,presentationRaf=0,presentationClockTimestamp=null,presentationClockAt=0,presentationGeneration=0,presentationMode='canvas',presentationDroppedFrames=0,renderedFrames=0,lastRenderedAt=0,decoder=null,config=null,configured=false,destroyed=false,decoderDisabled=false,failureReported=false,latencyExceeded=false,latencyViolationWindows=0,playbackActive=true,decodedFrames=0,paintedFrames=0,firstPaintAt=0,frameWidth=0,frameHeight=0,softwareFallback=preferSoftware,hardwareUnavailable=false,replay=[],queuedSinceOutput=0,configuredAt=0,lastOutputAt=0,needsKeyframe=true,displayMaxW=0,displayMaxH=0;const arrivalTimes=new Map(),latencySamples=[],renderIntervals=[];
...
function createNativeScreenPlayer(video,codec,onError=()=>{},options={}){
  ...
  function handleBackendFailure(token,error){if(destroyed||finalFailure||token!==generation)return;lastError=error;backendFailures++;const failed=backend;backend=null;try{failed?.destroy()}catch{}surface.cover();if(!startBackend(backendIndex+1)){reportFinal(lastError);return}replayIntoBackend()}
...
function bindReservedRemoteScreenAudio(){
...
function makeScreenTile(video,label,kind){const tile=document.createElement('article');tile.className='screen-tile '+kind;tile.dataset.screenTile=kind;paintShareSurfaceDark(tile);prepareShareSurface(video);const name=document.createElement('span');name.className='screen-tile-name';name.textContent=label;tile.append(video,name);return tile}
...
function syncScreenPlayback(){
  ...
  if(!screenPreview.hidden&&localAvailable){if(localSelected&&visible)playShareVideo(screenPreview);else try{screenPreview.pause()}catch{}}
  ...
  if(!remoteScreen.hidden&&remoteAvailable){
    ...
    if(hear){
      ...
      if(selected&&visible)playShareVideo(remoteScreen);else try{remoteScreen.pause()}catch{}
