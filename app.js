...
let pc,chat,files,role,sharedKey,directFileKey=null,directFileId='',sendQueue=Promise.resolve(),receiveQueue=Promise.resolve(),pairSignalBusy=false,pairReplyAccepted=false;let CHUNK=1024*1024,fileSessionEpoch=0,remoteFileProtocol=1;const MAX=200*1024**3,MAX_DIRECT_CONTROL_BYTES=512*1024,MAX_DIRECT_PROFILE_DATA=128*1024,MAX_FILE_CONTROL_BYTES=64*1024;
...
function addMessage(text,mine=false,gif=null,author=null){
  const stayAtLatest=mine||(!historyRendering&&messages.scrollHeight-messages.scrollTop-messages.clientHeight<96);
  $('.empty')?.remove();
  ...
  el.append(avatar,content);messages.append(el);if(stayAtLatest){messages.scrollTop=messages.scrollHeight;el.querySelectorAll('img,video').forEach(media=>media.addEventListener('load',()=>{messages.scrollTop=messages.scrollHeight},{once:true}))}recordConversationMessage({text,mine,gif:gif?.url?{url:gif.url,thumb:gif.thumb||gif.url,fallbackUrl:gif.fallbackUrl||null,emoji:gif.emoji===true}:null,author:mine?null:{id:author?.id||'',name,image:'',frame:authorFrame},time:messageTime});
}
// --- Emoji Picker ------------------------------------------------------------
const EMOJI_CATS=[
...
function normalizedTransferLane(value){return value==='tcp'?'tcp':value==='webrtc'?'webrtc':'waiting'}
function transferLaneText(value){const lane=normalizedTransferLane(value);return lane==='tcp'?'TCP DIRECT · port '+tcpFilePort:lane==='webrtc'?'WEBRTC DIRECT · fallback':'CHECKING TCP ROUTE…'}
...
function monitorRemoteScreenDecode(receiver,track,requestFallback){
  ...
  let previousBytes=0,previousFrames=0,stalls=0,finished=false,sampleInFlight=false;
...
fileInput.onchange=async()=>{const selected=[...fileInput.files],targetPeerId=activePeerId;fileInput.value='';if(!selected.length)return;
  ...
  if(targetPeerId&&(!fileBus()||dmPeerId!==targetPeerId)&&!LOCAL_TEST_MODE){try{pairHint.textContent='Connecting directly for file transfer…';await ensureDmMediaConnection(targetPeerId,{requireFileChannel:true});if(!await waitForDirectFileChannel(targetPeerId))throw new Error('Direct file connection timed out. Try again after both people reopen the chat.');if(activePeerId!==targetPeerId)throw new Error('File selection cancelled because you changed conversations');pairHint.textContent='Direct file connection ready.'}catch(error){pairHint.textContent=error?.message||'Could not connect for file transfer';return}}const session=currentFileSession(targetPeerId||undefined);if(!session&&!LOCAL_TEST_MODE){pairHint.textContent='The direct file connection is not ready. Try again.';return}selected.forEach(file=>sendFile(file,session,targetPeerId));};
...
openSettingsTab=function(name){
  ...
  if(name==='emojis'&&!emojiAttributionsLoaded){
    ...
    (async()=>{
      ...
      statsEl.textContent='Showing all '+details.length.toLocaleString()+' Emoji.gg creator credits required by the catalog’s CC BY 4.0 licenses.';
      const frag=document.createDocumentFragment();
...
async function restoreAccountProfile(value,userId,{keepLocalLegacy=false}={}){
  ...
  profileName=normalizeProfileName(value.name,'Knot user');profileAvatar=validProfileData(value.image)?value.image:'';profileFrame=normalizeFrame(value.frame);
  ...
  await Promise.all([ssSet('profileName',profileName),ssSet('profileAvatar',profileAvatar||null),ssSet('profileFrame',JSON.stringify(profileFrame)),ssSet('profilePhotoMode',profileAvatar?'account':'none')]);
  return true;
}
...
async function signInKnotAccount(usernameElement,passwordElement,status){
  ...
  socket.onopen=()=>{if(!finished&&generation===accountAuthGeneration)socket.send(JSON.stringify({type:'account-challenge',username:input.username}));else try{socket.close()}catch{}};
  socket.onmessage=async event=>{
    ...
    if(changed){clearTimeout(historySaveTimer);historySaveTimer=null;conversationHistories={};await ssSet('messageHistory','{}');closedDmIds.clear();await ssSet('closedDmIds','[]');unreadDmCounts={};await ssSet('unreadDmCounts','{}');directorySnapshot={friends:[],servers:[],groupDms:[],members:{},voiceStates:{}};renderUnreadBadges();renderServers();renderFriends()}
...
function syncActiveDmTransport(){
  ...
  const fileBlockedByOtherCall=inBackgroundCall;fileInput.disabled=relayVoiceMode||fileBlockedByOtherCall||!friend?.online||directorySocket?.readyState!==WebSocket.OPEN;fileInput.dataset.unavailableReason=fileBlockedByOtherCall?'End the call with '+(directoryUser(dmCallPeerId)?.name||'your friend')+' before sending a file to this DM.':relayVoiceMode?'Files need a direct connection and are unavailable on the voice relay.':!friend?.online?'Your friend must be online to receive a file.':'File transfer is unavailable while signaling is offline.';syncFileAttachmentUi();
...
async function receiveRelayText(value){
  ...
  if(value.scope==='dm'){
    if(!(directorySnapshot.friends||[]).some(friend=>friend.id===from))return;const key=await relayPairKey(from),payload=await openRelay(key,value.cipher,relayAad('dm',id,from,directoryUserId));if(!payload)return;if(!rememberRelayMessage(id)){directorySend({type:'relay-ack',id});return}receivePersistentDmMessage(from,readChatPayload(payload));directorySend({type:'relay-ack',id});return
  }
...
function boundedConversationHistoryJson(){
  let serialized=JSON.stringify(conversationHistories);if(serialized.length<=HISTORY_PERSIST_LIMIT)return serialized;
  // Keep the newest history across every DM/channel inside the main-process
  // setting limit. Without a global bound, one long conversation eventually
  // made every later save fail, silently freezing history at an old snapshot.
  for(let pass=0;pass<3&&serialized.length>HISTORY_PERSIST_LIMIT;pass++){
    const candidates=[];for(const [key,list] of Object.entries(conversationHistories))if(Array.isArray(list))for(let index=0;index<list.length;index++)candidates.push({key,index,time:Number(list[index]?.time)||0,size:JSON.stringify(list[index]).length+1});
    candidates.sort((a,b)=>a.time-b.time||a.key.localeCompare(b.key)||a.index-b.index);const removeCounts=new Map();let excess=serialized.length-HISTORY_PERSIST_LIMIT+64*1024;
    for(const item of candidates){removeCounts.set(item.key,(removeCounts.get(item.key)||0)+1);excess-=item.size;if(excess<=0)break}
    if(!removeCounts.size)break;for(const [key,count] of removeCounts){const list=conversationHistories[key];if(!Array.isArray(list))continue;list.splice(0,Math.min(count,list.length));if(!list.length)delete conversationHistories[key]}serialized=JSON.stringify(conversationHistories);
  }
  return serialized;
}
function scheduleHistorySave(){clearTimeout(historySaveTimer);historySaveTimer=setTimeout(()=>ssSet('messageHistory',boundedConversationHistoryJson()),180)}
...
function openConversation(key){saveConversationDraft();activeConversationKey=key;historyRendering=true;messages.replaceChildren();const list=conversationHistories[key]||[];for(const item of list){const current=item.author?.id?directoryUser(item.author.id):null;addMessage(item.text,!!item.mine,item.gif,item.author?{...item.author,...current,time:item.time}:{time:item.time})}historyRendering=false;if(!list.length){const empty=document.createElement('div');empty.className='empty';empty.innerHTML='<span>✦</span><p>Messages are encrypted on this device. Local history stays on your devices.</p>';messages.append(empty)}restoreConversationDraft(key);scrollConversationToLatest()}
...
function goHomeWithoutLeavingCall(){const keepDm=dmCallOngoing(),keepServer=!!serverVoiceStream;showFriends();if(keepDm){syncVoiceStage();renderDmVoiceUI()}if(keepServer){syncServerMesh();renderServerVoiceUI()}}
function installCallSafeHomeButton(){const home=$('#homeButton');if(!home)return;home.onclick=()=>{const peer=activePeerId;goHomeWithoutLeavingCall();if(peer)selectFriend(peer,{connect:false})}}
...
function wireServerChannel(peerId,channel,serverId=activeServerId){channel.onopen=()=>{if(serverVoiceStream)setServerStatus('Connected directly to '+serverPeers.size+' voice peer'+(serverPeers.size===1?'':'s'),true)};channel.onmessage=event=>{try{if(typeof event.data!=='string'||event.data.length>64*1024)return;const value=JSON.parse(event.data),state=serverPeers.get(peerId);if(!state||value?.serverId&&value.serverId!==serverId)return;if(value?.t==='screen-codec-fallback'){switchServerScreenCodec(peerId,state,compatibilityScreenCodec()).catch(()=>{});return}if(value?.t==='server-screen-end')clearServerScreenVideo(state)}catch{}}}
...
function targetServerVoiceBitrate(){return Math.max(48000,Math.min(128000,Math.floor(512000/serverMediaPeerCount())))}
async function configureServerVoiceSender(sender){
  if(!sender)return;try{const parameters=sender.getParameters();if(!parameters.encodings?.length)parameters.encodings=[{}];parameters.encodings[0].maxBitrate=targetServerVoiceBitrate();parameters.encodings[0].priority='high';parameters.encodings[0].networkPriority='high';await sender.setParameters(parameters)}catch{}
}
function rebalanceServerMediaBudgets(){const viewers=serverMediaPeerCount();for(const state of serverPeers.values()){if(state.closing)continue;void configureServerVoiceSender(state.voiceSender);for(const sender of state.screenSenders||[])if(sender.track?.kind==='video')void configureScreenVideoSender(sender,sender.track,shareFrameRate,viewers)}}
function addServerVoiceAudio(peerId,state,track,stream){
  const trackId=track?.id||'';state.audioTrackIds||=new Set();if(trackId&&state.audioTrackIds.has(trackId))return;if(trackId)state.audioTrackIds.add(trackId);const audio=document.createElement('audio');audio.autoplay=true;audio.srcObject=stream;audio.hidden=true;document.body.append(audio);state.audios.push(audio);applyMediaElementOutput(audio).catch(()=>{});monitorSpeaking('server:'+peerId,stream);audio.play().catch(()=>{});track.onended=()=>{stopSpeakingMonitor('server:'+peerId);if(trackId)state.audioTrackIds.delete(trackId);audio.remove();state.audios=state.audios.filter(item=>item!==audio)};
}
...
function addServerScreenVideo(peerId,state,track,stream){
  ...
  const receiver=state.pc.getReceivers().find(value=>value.track===track);if(receiver)state.screenDecodeStop=monitorRemoteScreenDecode(receiver,track,()=>{if(state.channel?.readyState!=='open')return false;state.channel.send(JSON.stringify({t:'screen-codec-fallback',serverId:state.context.serverId}));return true});
...
async function activateServerPeerMedia(peerId,state){
  ...
  for(const track of serverVoiceStream.getTracks()){state.voiceSender=state.pc.addTrack(track,serverVoiceStream);await configureServerVoiceSender(state.voiceSender)}
...
function recoverServerPeer(peerId,state){if(!state||state.closing||serverPeers.get(peerId)!==state)return;const context={...state.context};closeServerPeer(peerId,state);if(voicePeerAllowed(peerId)&&directoryUser(peerId)?.online)setTimeout(()=>{if(voicePeerAllowed(peerId)&&directoryUser(peerId)?.online&&!serverPeers.has(peerId))ensureServerPeer(peerId,context).catch(()=>{})},150)}
function closeServerPeer(peerId,state=serverPeers.get(peerId)){if(!state||state.closing)return;state.closing=true;state.offerGeneration++;clearTimeout(state.disconnectTimer);state.disconnectTimer=null;stopSpeakingMonitor('server:'+peerId);try{state.channel?.close()}catch{}clearServerNativeScreen(state);clearServerScreenVideo(state);try{state.nativeSendChannel?.close()}catch{}try{state.pc.close()}catch{}for(const audio of state.audios||[])try{audio.remove()}catch{}state.audios=[];state.pendingTracks=[];state.candidates=[];if(serverPeers.get(peerId)===state)serverPeers.delete(peerId);queueMicrotask(rebalanceServerMediaBudgets)}
...
function queueServerNativeItem(channel,item){
  ...
  if(item.kind==='init'){for(let index=queue.length-1;index>=0;index--)if(queue[index].kind==='init')queue.splice(index,1)}
  if(item.kind==='cluster'&&item.key){for(let index=queue.length-1;index>=0;index--)if(queue[index].kind==='cluster')dropQueuedServerNativeItem(channel,queue.splice(index,1)[0])}
  if(queue.length>=4&&item.kind==='cluster'){for(let index=queue.length-1;index>=0;index--)if(queue[index].kind==='cluster')dropQueuedServerNativeItem(channel,queue.splice(index,1)[0]);dropQueuedServerNativeItem(channel,item);return true}
  if(queue.length>=4)return true;queue.push(item);drainServerNativeQueue(channel);return true
}
...
function applyDirectoryProfileUpdate(value){
  const profile=value?.profile,id=String(profile?.id||'').toLowerCase();if(!/^[a-f0-9]{32}$/.test(id))return;
...
function applyDirectoryEntityUpdate(value){
  const entity=value?.entity,id=String(entity?.id||'').toLowerCase(),members=Array.isArray(entity?.members)?entity.members:[],channels=Array.isArray(entity?.channels)?entity.channels:[];if(!/^[a-f0-9]{32}$/.test(id)||!members.includes(directoryUserId)||!channels.every(channel=>/^[a-f0-9]{32}$/.test(String(channel?.id||''))))return;
...
async function connectDirectory(){
  const generation=++directoryConnectGeneration;clearTimeout(directoryReconnect);directoryReconnect=null;await profileSettingsReady;if(generation!==directoryConnectGeneration)return;
  if(!directoryStateRestored){const saved=await Promise.all([ss('directoryUserId'),ss('directoryToken'),ss('messageHistory'),ss('directoryAccountName'),ss('closedDmIds'),ss('unreadDmCounts')]);if(generation!==directoryConnectGeneration)return;const keepTransient=transientDirectorySession&&/^[a-f0-9]{32}$/.test(directoryUserId)&&/^[a-f0-9]{64}$/.test(directoryToken);if(!keepTransient){directoryUserId=/^[a-f0-9]{32}$/.test(saved[0]||'')?saved[0]:clientHex(16);directoryToken=/^[a-f0-9]{64}$/.test(saved[1]||'')?saved[1]:clientHex(32);directoryAccountName=/^[a-z0-9][a-z0-9_.-]{2,23}$/.test(saved[3]||'')?saved[3]:'';if(directoryUserId!==saved[0])await ssSet('directoryUserId',directoryUserId);if(directoryToken!==saved[1])await ssSet('directoryToken',directoryToken)}try{const parsed=JSON.parse(saved[2]||'{}');if(parsed&&typeof parsed==='object')conversationHistories=parsed}catch{}try{const closed=JSON.parse(saved[4]||'[]');if(Array.isArray(closed))closedDmIds=new Set(closed.filter(id=>/^[a-f0-9]{32}$/.test(id)))}catch{}try{const unread=JSON.parse(saved[5]||'{}');if(unread&&typeof unread==='object'&&!Array.isArray(unread))unreadDmCounts=Object.fromEntries(Object.entries(unread).filter(([id,count])=>/^[a-f0-9]{32}$/.test(id)&&Number(count)>0).map(([id,count])=>[id,Math.min(9999,Math.floor(Number(count)))]))}catch{}directoryStateRestored=true;renderUnreadBadges();renderAccountSummary()}
  ...
  socket.onopen=async()=>{if(directorySocket!==socket)return;directoryBackoff=1000;try{const profile=await directoryProfile();if(directorySocket===socket&&socket.readyState===WebSocket.OPEN)socket.send(JSON.stringify({type:'hello',userId:directoryUserId,token:directoryToken,...profile}))}catch(error){console.warn('directory profile',error);if(directorySocket===socket&&socket.readyState===WebSocket.OPEN)socket.send(JSON.stringify({type:'hello',userId:directoryUserId,token:directoryToken,name:profileName}))}};
  socket.onmessage=event=>{try{
    if(directorySocket!==socket)return;
    const value=JSON.parse(event.data);
    if(value.type==='authenticated'){directoryAccountName=value.username||directoryAccountName;if(directoryAccountName){ssSet('directoryAccountName',directoryAccountName);const connectedMessage='Signed in as @'+directoryAccountName+'. Connected.';if($('#accountStatus'))$('#accountStatus').textContent=connectedMessage;if($('#authStatus'))$('#authStatus').textContent=connectedMessage}renderAccountSummary();setDirectoryState(true,'Online');directoryProfilePush();if(callActive)publishCallState(true);void maybeShowAccountOnboarding()}
  ...
  socket.onclose=event=>{if(directorySocket!==socket)return;directorySocket=null;if(event.code===1008&&/authenticat|account|credential|session/i.test(event.reason||'')){setDirectoryState(false,'Sign in required');ssSet('directoryToken',null);const dialog=$('#accountDialog');if(dialog&&!dialog.open)dialog.showModal();$('#authSigninTab')?.click();if($('#authStatus'))$('#authStatus').textContent='Your saved session expired or was revoked. Sign in again.';return}setDirectoryState(false,'Offline — retrying');directoryReconnect=setTimeout(()=>{if(!directorySocket)void connectDirectory()},directoryBackoff);directoryBackoff=Math.min(30000,directoryBackoff*2)};socket.onerror=()=>{if(directorySocket===socket)setDirectoryState(false,'Connection error')};
}
...
function installGroupDmDialog(){
  ...
  const paintStatus=()=>{const selected=selectedIds().length,group=groupDm(targetGroupId),limit=mode==='add'?Math.max(0,10-(group?.members?.length||0)):9,minimum=mode==='add'?1:2;status.textContent=selected>limit?'That would exceed the 10-person group limit.':selected<minimum?(mode==='add'?'Choose at least one friend.':'Choose at least two friends.'):selected+' selected · '+(limit-selected)+' spot'+(limit-selected===1?'':'s')+' left';submit.disabled=selected<minimum||selected>limit;};
...
async function configureScreenVideoSender(sender,track,fps,viewers=1){
  const settings=track?.getSettings?.()||{},requestedFps=fps===30?30:60,maxBitrate=Math.max(350000,Math.round(targetScreenBitrate(settings.width,settings.height,requestedFps)/Math.max(1,Number(viewers)||1)));
...
function startScreenStats(sender){
  ...
  let sampleInFlight=false;const sample=async()=>{if(sampleInFlight)return;sampleInFlight=true;try{
    ...
    const limitation=reason&&reason!=='none'?` · ${reason} limited (resolution locked)`:'';const status='Sharing'+(w&&h?' · '+w+'×'+h:'')+(fps?' · '+fps+'fps':'')+(mbps!=='…'?' · '+mbps+' Mbps':'')+(codec?.mimeType?' · '+codec.mimeType.replace('video/',''):'')+(encodeMs?' · '+encodeMs.toFixed(1)+'ms encode':'')+network+limitation+(friendWatchingScreen?' · friend watching':'')+screenAudioDebug;screenStatus.textContent=status;screenBtn.title=status;
  }catch{}finally{sampleInFlight=false}};
...
function createMseNativeScreenPlayer(video,codec,onError=()=>{},options={}){
  ...
  // retain roughly six 60-fps frames of decode headroom instead of buffering a second
  // hidden multi-frame mux burst. Playback remains exactly 1x (no tearing);
  // only a genuine >160 ms live-edge debt is discarded at a frame boundary.
  const configuredWidth=Number(options.width)||0,configuredHeight=Number(options.height)||0,targetLag=.095,startLag=.075,hardCatchupLag=.16;
...
function settleNativeScreenReady(channel,ready){clearTimeout(channel?._nativeProtocolTimer);if(!channel||channel._nativeReadySettled)return;channel._nativeReadySettled=true;channel._nativeReadyResolve?.(!!ready);channel._nativeReadyResolve=null}
