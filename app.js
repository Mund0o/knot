...
let screenCursor='always',screenContentHint='motion',screenBitrateMbps=12,screenCodec='auto',shareResolution='1080',shareFrameRate=60,screenAudioOn=true;
...
function openScreenSharePicker({sources=[],qualityOnly=false}={}){
  ...
  const available=(Array.isArray(sources)?sources:[]).map(source=>({...source,type:shareSourceType(source)})),draft={source:null,resolution:['source','720','1080','1440','2160'].includes(shareResolution)?shareResolution:'1080',fps:shareFrameRate===30?30:60,audio:!!screenAudioOn},counts={application:available.filter(source=>source.type==='application').length,screen:available.filter(source=>source.type==='screen').length};
...
function commitScreenShareChoice(choice){if(!choice)return;shareResolution=['source','720','1080','1440','2160'].includes(choice.resolution)?choice.resolution:'1080';shareFrameRate=Number(choice.fps)===30?30:60;screenAudioOn=!!choice.audio;syncScreenAudioToggle();ssSet('shareResolution',shareResolution);ssSet('shareFrameRate',String(shareFrameRate));ssSet('shareSystemAudio',screenAudioOn?'on':'off')}
