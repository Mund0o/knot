...
assert(clean.p95Ms<=110,`clean-link p95 ${clean.p95Ms.toFixed(1)} ms exceeded the 110 ms target`);
...
assert(mesh.maxQueueMs<=260,'real-time stale-frame bound exceeded the 260 ms latency cap');
const summarize=results=>({conditions:results.length,under110:results.filter(item=>item.p95Ms<=110).length,under180:results.filter(item=>item.p95Ms<=180).length,dropped:results.reduce((sum,item)=>sum+item.droppedFrames,0),worstP95:Math.max(...results.map(item=>item.p95Ms))});
...
console.log('PASS deterministic network-conditioned latency benchmark preserves 110/260 ms targets without forcing them');
