...
async function main() {
  ...
  await window.webContents.executeJavaScript(`(()=>{
    ...
    const senderChannel=senderPc.createDataChannel('knot-native-transport',{ordered:true,maxPacketLifeTime:250,priority:'low'});
    ...
    let player,video,localPlayer,localVideo,decoderError='',firstSegmentAt=0,firstFrameAt=0,renderedFrames=0,chain=Promise.resolve(),nativeFallbacks=0,maxBufferedAmount=0,maxHeap=performance.memory?.usedJSHeapSize||0,minHeap=maxHeap,receiverWired=false,senderOpened=false;
    ...
    window._nativeTransportReady=new Promise((resolve,reject)=>{
      ...
      senderChannel.onmessage=event=>{if(typeof event.data!=='string')return;const value=JSON.parse(event.data);if(value.t==='native-screen-ready'){senderChannel._nativePeerProtocol=Number(value.transportVersion)||0;clearTimeout(senderChannel._nativeProtocolTimer)}};
  ...
  const session = service.start({ codec: 'av1', fps: 60, width: 3840, height: 2160, bitrateKbps: 24000, captureSource: monitor });
  let firstClusterAt = 0,clusters = 0,segments = 0,maxSegmentBytes = 0;
  ...
  while (Date.now()<startupDeadline || (firstClusterAt && Date.now()-firstClusterAt<enduranceMs)) {
    ...
    if (item.data) {
      window.webContents.send('knot-native-segment', { kind: item.kind, data: new Uint8Array(item.data) });
      segments++;maxSegmentBytes=Math.max(maxSegmentBytes,item.data.length);
  ...
  const metrics = { ...result, durationSeconds: enduranceMs/1000, clusters, segments, maxSegmentBytes, gpuProcessGone };
  ...
  if (!result.softwareFallback) {
    assert(result.firstFrameMs <= 100, `hardware first frame took ${result.firstFrameMs.toFixed(0)}ms (target: 100ms)`);
    assert(result.steadyStateP95Ms > 0 && result.steadyStateP95Ms <= 100, `hardware steady-state p95 was ${result.steadyStateP95Ms.toFixed(0)}ms (target: 100ms)`);
  }
  ...
  assert.strictEqual(result.channelLifetime, 250, 'native channel is not partially reliable');
  assert.strictEqual(result.nativeFallbacks, 0, 'healthy loopback transport requested H.264 fallback');
