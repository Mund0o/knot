...
async function main() {
  ...
  await window.webContents.executeJavaScript(`(()=>{
    ...
    const senderChannel=senderPc.createDataChannel('knot-native-transport',{ordered:false,maxPacketLifeTime:100,priority:'low'});
  ...
  assert.strictEqual(result.channelLifetime, 100, 'native channel does not abandon stale packets within the latency budget');
  assert.strictEqual(result.channelOrdered, false, 'native channel still has cross-frame head-of-line blocking');
  assert.strictEqual(result.presentationMode, 'track', 'receiver copied every 4K frame through a renderer canvas');
