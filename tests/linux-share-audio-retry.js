...
assert(preloadSource.includes('onLinuxShareAudioDebug') && rendererSource.includes('function logShareAudio') && mainSource.includes('function debugLinuxShareAudio'), 'Share-audio diagnostics are missing from the call log');
...
assert(mainSource.includes('pair_share_hold_') && mainSource.includes("['move-sink-input', id, original]") && mainSource.includes('function linuxShareReturnInput') && mainSource.includes('function isKnotPlaybackStream'), 'Linux loopback still connects unmuted to the real speakers');
