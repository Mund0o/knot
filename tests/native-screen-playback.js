...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const result = await window.webContents.executeJavaScript(`(async()=>{
      const fs=require('fs'),wait=ms=>new Promise(resolve=>setTimeout(resolve,ms)),bytes=fs.readFileSync(${JSON.stringify(path.resolve(input))}),mime='video/webm; codecs="av01.0.13H.08"';
      ...
      await video.play();await new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(new Error('no decoded AV1 frame')),5000);if(video.requestVideoFrameCallback)video.requestVideoFrameCallback(()=>{clearTimeout(timer);resolve()});else video.addEventListener('loadeddata',()=>{clearTimeout(timer);resolve()},{once:true})});const firstFrameMs=performance.now()-started,capabilities=await navigator.mediaCapabilities.decodingInfo({type:'media-source',video:{contentType:mime,width:3840,height:2160,bitrate:40000000,framerate:60}}),qualityStart=video.getVideoPlaybackQuality?.()||{},measuredAt=performance.now();await wait(2000);const measuredSeconds=(performance.now()-measuredAt)/1000,quality=video.getVideoPlaybackQuality?.()||{},rendered=Math.max(0,(quality.totalVideoFrames||0)-(qualityStart.totalVideoFrames||0)),dropped=Math.max(0,(quality.droppedVideoFrames||0)-(qualityStart.droppedVideoFrames||0));return{supported:true,width:video.videoWidth,height:video.videoHeight,firstFrameMs,capabilities,decoded:quality.totalVideoFrames||0,dropped:quality.droppedVideoFrames||0,renderFps:rendered/measuredSeconds,measurementDropped:dropped,buffered:video.buffered.length?video.buffered.end(video.buffered.length-1):0};
    })()`, true);
    console.log(JSON.stringify(result, null, 2));
    if (result.width !== 3840 || result.height !== 2160 || result.decoded < 90 || result.renderFps < 50 || result.measurementDropped > 6) {
