...
try {
  ...
  assert(rendererSource.includes("fmt?.available!==false&&fmt?.isolated===true") && rendererSource.includes("captureFailure='isolated WASAPI process-loopback format unavailable'") && rendererSource.includes('captureFailure||!formatReady') && rendererSource.includes('handshake<=3') && rendererSource.includes('else if(fmt?.available!==false)'), 'renderer can attach Windows desktop sound without a confirmed isolated route');
