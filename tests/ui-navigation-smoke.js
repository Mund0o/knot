...
if (!mainSource.includes("appendSwitch('hardware-video-device-path', primaryGpu.renderNode)") || !mainSource.includes("primaryGpuCard: selectedPrimaryGpu?.card") || !mainSource.includes("appendSwitch('force-high-performance-gpu')") || !gpuSource.includes('.filter(candidate => !candidate.integrated)') || !gpuSource.includes("env.DRI_PRIME = selector") || !gpuSource.includes("env.__VK_LAYER_NV_optimus = 'NVIDIA_only'")) {
