...
if (!mainSource.includes('applyGpuAccelerationPolicy(app') || !mainSource.includes("primaryGpuCard: selectedPrimaryGpu?.card") || !accelerationSource.includes("switches.set('hardware-video-device-path', gpu.renderNode)") || !accelerationSource.includes("['force-high-performance-gpu', '']") || !accelerationSource.includes("['disable-software-rasterizer', '']") || !gpuSource.includes('.filter(candidate => !candidate.integrated)') || !gpuSource.includes("env.DRI_PRIME = selector") || !gpuSource.includes("env.__VK_LAYER_NV_optimus = 'NVIDIA_only'") || !gpuSource.includes("env.LIBVA_DRIVER_NAME = 'nvidia'")) {
...
if (!rendererSource.includes("parameters.degradationPreference='maintain-resolution'") || !rendererSource.includes("encoding.networkPriority='high'") || !rendererSource.includes("screenAudio=stream.getVideoTracks().length>0") || !rendererSource.includes("t:'screen-codec-fallback',serverId:state.context.serverId")) {
...
if (!rendererSource.includes('allowSoftwareFallback:false') || !rendererSource.includes('startSoftwareDecoder') || rendererSource.includes('window.pairEnv?.toggleFullscreen')) {
  throw new Error('Native AV1 can still overload the sender, black-screen without retrying, or fullscreen the whole app');
}
if (!rendererSource.includes('targetNativeAv1BitrateKbps(width,height,fps)') || !rendererSource.includes("createDataChannel('knot-screen-native',{ordered:true,priority:'low'})")) {
