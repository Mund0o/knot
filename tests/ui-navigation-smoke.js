...
if (!rendererSource.includes("parameters.degradationPreference='maintain-framerate'") || !rendererSource.includes("encoding.networkPriority='low'") || !rendererSource.includes("p.encodings[0].networkPriority='high'") || !rendererSource.includes("screenAudio=stream.getVideoTracks().length>0") || !rendererSource.includes("t:'screen-codec-fallback',serverId:state.context.serverId")) {
...
if (!rendererSource.includes('targetNativeAv1BitrateKbps(width,height,fps)') || !rendererSource.includes("createDataChannel('knot-screen-native',nativeScreenChannelOptions())") || !rendererSource.includes('maxRetransmits:1') || !rendererSource.includes('NATIVE_SCREEN_BUFFER_HIGH=320*1024')) {
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const result = await window.webContents.executeJavaScript(`(async()=>{
      ...
      let senderParameters={encodings:[{}]};const sender={getParameters:()=>structuredClone(senderParameters),setParameters:async value=>{senderParameters=structuredClone(value)}},screenTrack={getSettings:()=>({width:3840,height:2160})};screenRecoveryLevel=0;await configureScreenVideoSender(sender,screenTrack,60);assert(senderParameters.degradationPreference==='maintain-framerate'&&senderParameters.encodings[0].maxFramerate===60&&senderParameters.encodings[0].scaleResolutionDownBy===1&&senderParameters.encodings[0].priority==='medium'&&senderParameters.encodings[0].networkPriority==='low'&&senderParameters.encodings[0].maxBitrate===(window.pairEnv?.platform==='linux'?16004877:19698311)&&!('minBitrate' in senderParameters.encodings[0]),'screen sender does not prioritize smooth 4K60 while yielding network priority to voice');
      ...
      const workletContext=new AudioContext({sampleRate:48000});await workletContext.audioWorklet.addModule(new URL('screen-audio-worklet.js',location.href));const workletNode=new AudioWorkletNode(workletContext,'knot-screen-audio',{numberOfInputs:0,numberOfOutputs:1,outputChannelCount:[2]}),workletDest=workletContext.createMediaStreamDestination();workletNode.connect(workletDest);workletNode.port.postMessage(new Float32Array(4096));assert(workletDest.stream.getAudioTracks().length===1,'Linux screen audio worklet did not create a stereo output track');workletNode.disconnect();await workletContext.close();
      screenActive=true;screenStream={getVideoTracks:()=>[screenTrack]};screenRecoveryLevel=0;await recoverOverloadedScreenSender(sender);assert(screenRecoveryLevel===1&&senderParameters.encodings[0].maxFramerate===60&&senderParameters.encodings[0].scaleResolutionDownBy===1.5,'first overload recovery did not protect frame cadence');await recoverOverloadedScreenSender(sender);assert(screenRecoveryLevel===2&&senderParameters.encodings[0].maxFramerate===60&&senderParameters.encodings[0].scaleResolutionDownBy===2,'repeated overload did not scale to a sustainable 1080p60');screenActive=false;screenStream=null;screenRecoveryLevel=0;
