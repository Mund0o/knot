...
async function main() {
  ...
  assert(sender.maxBufferedAmount<=maxSegmentBytes+320*1024,`bounded AV1 uplink queued ${(sender.maxBufferedAmount/1024).toFixed(0)} KiB`);
  ...
  if(receiver.backend==='webcodecs'&&!receiver.softwareFallback&&receiver.steadyStateP95Ms)assert(receiver.steadyStateP95Ms<=100,`receiver AV1 processing p95 reached ${receiver.steadyStateP95Ms.toFixed(0)}ms`);
  assert(receiver.decodedFrames>=Math.floor(durationMs/1000*35),`receiver decoded only ${receiver.decodedFrames} frames`);
  assert(receiver.paintedFrames>=Math.floor(durationMs/1000*30),`receiver displayed only ${receiver.paintedFrames} frames`);
