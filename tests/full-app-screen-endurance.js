...
async function main() {
  ...
  session = service.start({ codec: 'av1', fps: 60, width: 3840, height: 2160, bitrateKbps: 10000, captureSource: monitor });
  ...
  let firstClusterAt=0,clusters=0,segments=0,bytes=0,maxSegmentBytes=0;
  ...
  while(Date.now()<startupDeadline||(firstClusterAt&&Date.now()-firstClusterAt<durationMs)){
    ...
    if(item.data){const payload={kind:item.kind,key:item.key,data:new Uint8Array(item.data)};await sendFullSegment(senderWindow,payload);segments++;bytes+=item.data.length;maxSegmentBytes=Math.max(maxSegmentBytes,item.data.length);if(item.kind==='cluster'){clusters++;if(!firstClusterAt)firstClusterAt=Date.now()}}
  ...
  clearInterval(healthTimer);clearInterval(processTimer);service.stop(session.id);session=null;senderWindow.webContents.send('knot-full-finish');await wait(3000);
  ...
  const metrics={durationSeconds:durationMs/1000,simulatedUplinkMbps:uplinkMbps,sourceSegments:segments,sourceClusters:clusters,sourceBytes:bytes,maxSegmentBytes,sender,receiver,processes:byType,crashes,unresponsive};
  ...
  assert.strictEqual(sender.channelLifetime,100,'full-app AV1 channel retained stale packets beyond 100ms');
  assert(sender.maxBufferedAmount<=384*1024,`bounded AV1 uplink queued ${(sender.maxBufferedAmount/1024).toFixed(0)} KiB`);
  assert(sender.droppedSegments>0,'constrained full-app uplink never exercised stale-frame dropping');
  ...
  assert.strictEqual(receiver.presentationMode,'track','receiver copied every 4K frame through the app renderer');
  assert.strictEqual(receiver.latencyExceeded,false,'receiver remained above the AV1 100ms processing target');
  if(receiver.steadyStateP95Ms)assert(receiver.steadyStateP95Ms<=100,`receiver AV1 processing p95 reached ${receiver.steadyStateP95Ms.toFixed(0)}ms`);
  assert(receiver.decodedFrames>=Math.floor(durationMs/1000*35),`receiver decoded only ${receiver.decodedFrames} frames`);
  assert(receiver.paintedFrames>=Math.floor(receiver.decodedFrames*.95),`receiver painted ${receiver.paintedFrames}/${receiver.decodedFrames} frames`);
